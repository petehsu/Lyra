...
describe("workspace app service", () => {
  test("type guards identify app ids", () => {
    ...
    expect(isAiPanelAppId("ai-panel")).toBe(true);
    expect(isAiPanelAppId("file-manager")).toBe(false);
    ...
    expect(isNotificationCenterAppId("ai-panel")).toBe(false);
  });
  ...
  test("renders workspace app icon node", () => {
    ...
    const aiPanelIcon = renderWorkspaceAppIcon("ai-panel", "ai-panel-default");
