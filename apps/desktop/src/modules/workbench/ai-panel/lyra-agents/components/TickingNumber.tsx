...
export function TickingNumber({
  ...
  return (
    <span className={`lyra-agents-tick tick-${direction}`}>
      <span key={display} className="lyra-agents-tick-current">
        {display}
      </span>
      {ghost !== null && (
        <span key={`ghost-${ghost.id}`} className="lyra-agents-tick-ghost">
          {ghost.value}
        </span>
      )}
    </span>
