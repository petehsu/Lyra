...
describe("ChatView progressive message window", () => {
  ...
  test("shows a sticky previous user message anchor while scrolling history", async () => {
    ...
    await waitFor(() => {
      expect(container.querySelector(".chat-thread-anchor-text")).toHaveTextContent("Message 19");
    });

    fireEvent.click(container.querySelector(".chat-thread-anchor-button") as HTMLButtonElement);
  ...
  test("hides the sticky anchor once its message is visible at the top", async () => {
    ...
    await waitFor(() => {
      expect(container.querySelector(".chat-thread-anchor-text")).not.toBeInTheDocument();
    });
  });
});
