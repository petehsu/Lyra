...
const requiredMethods: readonly (keyof TerminalNativeBindings)[] = [
...
const resolveNativeFileNames = (): readonly string[] => {
  if (process.platform === "win32") {
    return ["lyra_terminal_napi.dll", "lyra_terminal_napi.node"];
  }

  if (process.platform === "darwin") {
    return ["liblyra_terminal_napi.dylib", "lyra_terminal_napi.node"];
  }

  return ["liblyra_terminal_napi.so", "lyra_terminal_napi.node"];
};
...
export const resolveNativeCandidates = (cwd: string): readonly string[] => {
  ...
  const explicit = process.env.LYRA_TERMINAL_NATIVE_LIB;
...
const validateBindings = (value: unknown): value is TerminalNativeBindings => {
...
const loadFromPath = (candidatePath: string): TerminalNativeBindings | null => {
...
export const loadTerminalNativeBindings = (): TerminalNativeLoadResult => {
  const cwd = process.cwd();
  const candidates = resolveNativeCandidates(cwd);

  let lastError = "terminal native addon not found";
