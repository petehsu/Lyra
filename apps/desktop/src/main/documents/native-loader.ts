...
import type { DocsNativeBindings, DocsNativeLoadResult } from "./types";
...
const resolveNativeFileNames = (): readonly string[] => {
  if (process.platform === "win32") {
    return ["lyra_docs_napi.dll", "lyra_docs_napi.node"];
  }
  if (process.platform === "darwin") {
    return ["liblyra_docs_napi.dylib", "lyra_docs_napi.node"];
  }
  return ["liblyra_docs_napi.so", "lyra_docs_napi.node"];
};
...
export const resolveDocsNativeCandidates = (cwd: string): readonly string[] => {
  ...
  const explicit = process.env.LYRA_DOCS_NATIVE_LIB;
...
const validateBindings = (value: unknown): value is DocsNativeBindings => {
...
const loadFromPath = (candidatePath: string): DocsNativeBindings | null => {
...
export const loadDocsNativeBindings = (): DocsNativeLoadResult => {
  const cwd = process.cwd();
  const candidates = resolveDocsNativeCandidates(cwd);
  let lastError = "docs native addon not found";
