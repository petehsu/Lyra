...
import type { AiNativeBindings, AiNativeLoadResult } from "./types";
...
const requiredMethods: readonly (keyof AiNativeBindings)[] = [
  "registerAiEventCallback",
  "readAiProfilesJson",
  "upsertAiProfileJson",
  "deleteAiProfileJson",
  "setDefaultAiProfileJson",
  "validateAiProfileJson",
  "readAiSessionJson",
  "readAiSessionHistoryJson",
  "sendAiChatTurnJson",
  "cancelAiChatTurnJson",
  "shutdownAiRuntime"
];

const resolveNativeFileNames = (): readonly string[] => {
  if (process.platform === "win32") {
    return ["lyra_ai_napi.dll", "lyra_ai_napi.node"];
  }
  if (process.platform === "darwin") {
    return ["liblyra_ai_napi.dylib", "lyra_ai_napi.node"];
  }
  return ["liblyra_ai_napi.so", "lyra_ai_napi.node"];
};
...
export const resolveAiNativeCandidates = (cwd: string): readonly string[] => {
  ...
  const explicit = process.env.LYRA_AI_NATIVE_LIB;
...
const validateBindings = (value: unknown): value is AiNativeBindings => {
...
const loadFromPath = (candidatePath: string): AiNativeBindings | null => {
...
export const loadAiNativeBindings = (): AiNativeLoadResult => {
  const cwd = process.cwd();
  const candidates = resolveAiNativeCandidates(cwd);
  let lastError = "ai native addon not found";
