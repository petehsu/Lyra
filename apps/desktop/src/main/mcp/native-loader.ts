...
import type { McpNativeBindings, McpNativeLoadResult } from "./types";
...
const requiredMethods: readonly (keyof McpNativeBindings)[] = [
  "registerMcpEventCallback",
  "readMcpScopeDocumentJson",
  "writeMcpScopeDocumentJson",
  "readMcpSecretStoreJson",
  "writeMcpSecretStoreJson",
  "sanitizeMcpEnvironmentJson",
  "normalizeMcpEnvironmentInputJson",
  "deleteMcpSecretRefsJson",
  "mergeMcpEffectiveConfigJson",
  "validateMcpServerJson",
  "writeMcpManagedManifestJson",
  "materializeMcpRuntimeEnvironmentJson",
  "createMcpServerFromTemplateJson",
  "readMcpRuntimeStatusesJson",
  "readMcpRuntimeIntrospectionJson",
  "startMcpRuntimeJson",
  "stopMcpRuntimeJson",
  "restartMcpRuntimeJson",
  "shutdownMcpRuntime"
];

const resolveNativeFileNames = (): readonly string[] => {
  if (process.platform === "win32") {
    return ["lyra_mcp_napi.dll", "lyra_mcp_napi.node"];
  }

  if (process.platform === "darwin") {
    return ["liblyra_mcp_napi.dylib", "lyra_mcp_napi.node"];
  }

  return ["liblyra_mcp_napi.so", "lyra_mcp_napi.node"];
};
...
export const resolveMcpNativeCandidates = (cwd: string): readonly string[] => {
  ...
  const explicit = process.env.LYRA_MCP_NATIVE_LIB;
...
const validateBindings = (value: unknown): value is McpNativeBindings => {
...
const loadFromPath = (candidatePath: string): McpNativeBindings | null => {
...
export const loadMcpNativeBindings = (): McpNativeLoadResult => {
  const cwd = process.cwd();
  const candidates = resolveMcpNativeCandidates(cwd);

  let lastError = "mcp native addon not found";
