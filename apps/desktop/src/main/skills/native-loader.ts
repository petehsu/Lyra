...
const resolveNativeFileNames = (): readonly string[] => {
  if (process.platform === "win32") {
    return ["lyra_skills_napi.dll", "lyra_skills_napi.node"];
  }

  if (process.platform === "darwin") {
    return ["liblyra_skills_napi.dylib", "lyra_skills_napi.node"];
  }

  return ["liblyra_skills_napi.so", "lyra_skills_napi.node"];
};
...
export const resolveSkillsNativeCandidates = (cwd: string): readonly string[] => {
  ...
  const explicit = process.env.LYRA_SKILLS_NATIVE_LIB;
