...
export const createWorkbenchBrowserViewManager = ({
  ...
  type PendingAgentPageLoadWait = {
    readonly cancel: () => void;
  };

  const pendingAgentPageLoadWaits = new WeakMap<WebContents, PendingAgentPageLoadWait>();
  ...
  const cancelPendingAgentPageLoad = (webContents: WebContents): void => {
    pendingAgentPageLoadWaits.get(webContents)?.cancel();
  };
  ...
  const waitForAgentPageLoad = async (
    ...
    cancelPendingAgentPageLoad(webContents);
    await new Promise<void>((resolve) => {
      let settled = false;
      let timer: ReturnType<typeof setTimeout> | null = null;
      let waitRecord: PendingAgentPageLoadWait | null = null;
      const finish = (stopLoading: boolean): void => {
        ...
        if (waitRecord !== null && pendingAgentPageLoadWaits.get(webContents) === waitRecord) {
          pendingAgentPageLoadWaits.delete(webContents);
        }
        if (stopLoading) {
      ...
      pendingAgentPageLoadWaits.set(webContents, waitRecord);
  ...
  const snapshotViewport = (
    request: Record<string, unknown>
  ): { readonly width: number; readonly height: number; readonly deviceScaleFactor: number } | null => {
  ...
  const runRenderedSnapshotWait = async (
    webContents: WebContents,
    request: Record<string, unknown>,
    warnings: Array<{ readonly code: string; readonly message: string }>,
    deadlineMs: number
  ): Promise<void> => {
  ...
  const readRenderedSnapshotWithTemporaryRenderer = async (
    ...
    try {
      ...
      await waitForAgentPageLoad(webContents, url, remainingSnapshotMs(deadlineMs, "browser navigation"));
