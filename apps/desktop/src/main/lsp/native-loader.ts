...
const requiredMethods: readonly (keyof LspNativeBindings)[] = [
...
const resolveNativeFileNames = (): readonly string[] => {
  if (process.platform === "win32") {
    return ["lyra_lsp_napi.dll", "lyra_lsp_napi.node"];
  }

  if (process.platform === "darwin") {
    return ["liblyra_lsp_napi.dylib", "lyra_lsp_napi.node"];
  }

  return ["liblyra_lsp_napi.so", "lyra_lsp_napi.node"];
};
...
export const resolveNativeCandidates = (cwd: string): readonly string[] => {
  ...
  const explicit = process.env.LYRA_LSP_NATIVE_LIB;
...
const validateBindings = (value: unknown): value is LspNativeBindings => {
...
const loadFromPath = (candidatePath: string): LspNativeBindings | null => {
...
export const loadLspNativeBindings = (): LspNativeLoadResult => {
  const cwd = process.cwd();
  const candidates = resolveNativeCandidates(cwd);

  let lastError = "lsp native addon not found";
