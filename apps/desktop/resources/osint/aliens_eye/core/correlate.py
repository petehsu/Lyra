"""Cross-site correlation — adapted (no aiohttp_socks, local import paths)."""
...
def _common_names(profiles: list[Profile]) -> frozenset[str]:
    ...
    counts: Counter[str] = Counter(na for p in profiles if (na := _norm_name(p.name)))
