"""Fingerprint store — adapted for Lyra (local data path only)."""
...
class FingerprintStore:
    ...
    def load(self, logger=None) -> None:
        ...
        try:
            ...
            self.data = {"sites": {}}
            if logger:
                logger.debug("Fingerprint cache invalid: %s", exc)
    ...
    def _count_matches(self, entries: list[dict[str, str]], fingerprint: dict[str, str]) -> int:
        score = 0
