"""Feature extractor — adapted for Lyra (local data path for site_profiles)."""
...
def _load_site_profiles() -> dict[str, Any]:
    ...
    if _site_profiles_cache is None:
        try:
            text = (_DATA_DIR / "site_profiles.json").read_text("utf-8")
...
class FeatureExtractor:
    ...
    def extract(
        ...
        error_keyword_count = sum(1 for keyword in self.error_keywords if keyword in content_lower)
        positive_keyword_count = sum(1 for keyword in self.positive_keywords if keyword in content_lower)
        ...
        meta_error_keyword_count = sum(1 for keyword in self.error_keywords if keyword in meta_text)
        meta_positive_keyword_count = sum(
            1 for keyword in (self.positive_keywords + self.meta_keywords) if keyword in meta_text
        )
        ...
        features = {
            ...
            "title_has_username": 1.0 if username.lower() in title_text.lower() else 0.0,
            "meta_has_username": 1.0 if username.lower() in meta_text else 0.0,
    ...
    def _extract_profile(self, tree: HTMLParser, url: str, site_name: str, title_text: str) -> dict[str, str]:
