"""Username variation generation — unchanged from upstream."""
...
def generate_username_variations(username: str, level: str) -> list[str]:
    ...
    if level in {"intermediate", "advanced"}:
        variations.extend([
            f"{username}_", f"_{username}", f"{username}.", f".{username}",
            f"{username.replace(' ', '_')}", f"{username.replace(' ', '.')}",
            f"{username}1", f"{username}123", f"{username}007",
            f"{username}098", f"{username}x", f"__{username}", f"__{username}__",
        ])
    if level == "advanced":
