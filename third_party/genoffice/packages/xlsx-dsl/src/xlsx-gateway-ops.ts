        ...
        if (formulaCells.length) {
          ...
          if (input.computed === undefined) {
            reject(
              i,
              op.op,
              `the source has formulas (${formulaCells.slice(0, 5).join(', ')}); formula evaluation is not built in`,
            )
          }
...
async function loadImage(
  ...
  if (/^https?:\/\//i.test(source)) {
    // ponytail: no remote image fetch in the desktop build; a local png/jpeg/gif path still works.
    fail(`remote images are not built in: ${source}`)
  } else {
