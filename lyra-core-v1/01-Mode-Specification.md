...
## 1. 目标
定义 Agent / Oma 双模式的边界、共享能力、切换策略和一致性约束，避免模式语义漂移。
其中 Oma 全称为 `Open Multi-Agent`，宣传口号为 `Oh My Agent`。

## 2. 模式边界总览
| 维度 | Agent | Oma |
|---|---|---|
| 内核类型 | 共享能力内核 | 共享能力内核 + 独立编排层 |
| 主要目标 | 单体任务执行 | 多角色协作与组织化交付 |
| 默认执行主体 | 单 Agent | 多 Role 网络 |
| 委派能力 | 可选（轻量） | 核心能力（默认存在） |
| 并发能力 | 有限并发 | 结构化并发（强约束） |
| 会议能力 | 无 | 可选会议，角色投票触发 |
| 门禁强度 | 中 | 强 |
| 交付收敛 | 单体结果 | Integrator + Gate 收敛 |
| 与 Agent 关系 | 基准执行层 | 以 Agent 作为 Apex 执行层并向下编排 |
...
参考 OpenCode/Roo/Cline 的模式化经验，双模式在运行时需要绑定执行档位：
...
|---|---|---|
| 文件写入 | allow | allow |
| 终端命令 | ask-on-risk | ask-on-risk |
| 网络访问 | ask-on-risk | ask-on-risk |
| 生产环境变更 | deny-by-default | deny-by-default |
| 高风险工具调用 | explicit-approve | explicit-approve |
