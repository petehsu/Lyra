...
| Agent/model | prompts, messages, memory, files, attachments, page/tool results, device/screen/timezone | selected cloud, BYOK, custom, or local provider | Agent state, sessions, memory, artifacts and logs | provider retention is separate; describe by provider |
...
| Location | OS location permission if a location-capable feature requests it | OS location services; not injected into Agent context | no Lyra location-indicator store | public Nominatim calls disabled; Lyra first-party location authorization UI removed |
