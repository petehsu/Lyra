# Lyra Surface Migration Spec (shared)

You are migrating ONE workbench surface in `apps/desktop` to the established Lyra App-component design language. The foundation (Tailwind v4 + token bridge + `renderer/ui` component layer + UI style guard) is ALREADY in place and Settings/Software Store/Notification Center are the calibrated baselines. Follow them exactly. Read `UIUX-REWRITE-PLAN.md` and `apps/desktop/src/renderer/styles/README.md` for the design language; this file is the operational checklist.
