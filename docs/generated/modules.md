# Generated module index

Audience: Internal
Status: Generated
Last verified: 2026-07-28

> Generated file. Do not edit by hand.
>
> Sources: `Cargo.toml`, `apps/*/package.json`, `services/*/package.json`, `packages/*/package.json`, `web/*/package.json`, `apps/desktop/src/main`, `apps/desktop/src/modules/workbench`.
> Regenerate with `node docs/scripts/generate-inventories.mjs`.

This index lists build workspaces and first-level Desktop ownership modules.
It does not define a public package API.

## Rust workspace (69)

| Package | Location |
| --- | --- |
| `codegraph` | `crates/codegraph` |
| `codegraph-bash` | `crates/codegraph-bash` |
| `codegraph-c` | `crates/codegraph-c` |
| `codegraph-clojure` | `crates/codegraph-clojure` |
| `codegraph-cobol` | `crates/codegraph-cobol` |
| `codegraph-cpp` | `crates/codegraph-cpp` |
| `codegraph-csharp` | `crates/codegraph-csharp` |
| `codegraph-css` | `crates/codegraph-css` |
| `codegraph-dart` | `crates/codegraph-dart` |
| `codegraph-dockerfile` | `crates/codegraph-dockerfile` |
| `codegraph-elixir` | `crates/codegraph-elixir` |
| `codegraph-elm` | `crates/codegraph-elm` |
| `codegraph-erlang` | `crates/codegraph-erlang` |
| `codegraph-fortran` | `crates/codegraph-fortran` |
| `codegraph-go` | `crates/codegraph-go` |
| `codegraph-groovy` | `crates/codegraph-groovy` |
| `codegraph-harness` | `crates/codegraph-harness` |
| `codegraph-haskell` | `crates/codegraph-haskell` |
| `codegraph-hcl` | `crates/codegraph-hcl` |
| `codegraph-java` | `crates/codegraph-java` |
| `codegraph-julia` | `crates/codegraph-julia` |
| `codegraph-kotlin` | `crates/codegraph-kotlin` |
| `codegraph-lua` | `crates/codegraph-lua` |
| `codegraph-memory` | `crates/codegraph-memory` |
| `codegraph-objc` | `crates/codegraph-objc` |
| `codegraph-ocaml` | `crates/codegraph-ocaml` |
| `codegraph-parser-api` | `crates/codegraph-parser-api` |
| `codegraph-perl` | `crates/codegraph-perl` |
| `codegraph-php` | `crates/codegraph-php` |
| `codegraph-python` | `crates/codegraph-python` |
| `codegraph-r` | `crates/codegraph-r` |
| `codegraph-ruby` | `crates/codegraph-ruby` |
| `codegraph-rust` | `crates/codegraph-rust` |
| `codegraph-scala` | `crates/codegraph-scala` |
| `codegraph-server` | `crates/codegraph-server` |
| `codegraph-solidity` | `crates/codegraph-solidity` |
| `codegraph-swift` | `crates/codegraph-swift` |
| `codegraph-tcl` | `crates/codegraph-tcl` |
| `codegraph-toml` | `crates/codegraph-toml` |
| `codegraph-typescript` | `crates/codegraph-typescript` |
| `codegraph-verilog` | `crates/codegraph-verilog` |
| `codegraph-yaml` | `crates/codegraph-yaml` |
| `codegraph-zig` | `crates/codegraph-zig` |
| `lyra-accessibility-napi` | `crates/lyra-accessibility-napi` |
| `lyra-agent-api` | `crates/lyra-agent-api` |
| `lyra-agent-core` | `crates/lyra-agent-core` |
| `lyra-agent-kernel` | `crates/lyra-agent-kernel` |
| `lyra-agent-plugins` | `crates/lyra-agent-plugins` |
| `lyra-agent-reader` | `crates/lyra-agent-reader` |
| `lyra-agent-runtime` | `crates/lyra-agent-runtime` |
| `lyra-cli` | `crates/lyra-cli` |
| `lyra-code-intel-core` | `crates/lyra-code-intel-core` |
| `lyra-computer-use-core` | `crates/lyra-computer-use-core` |
| `lyra-docs-core` | `crates/lyra-docs-core` |
| `lyra-docs-napi` | `crates/lyra-docs-napi` |
| `lyra-download-core` | `crates/lyra-download-core` |
| `lyra-download-napi` | `crates/lyra-download-napi` |
| `lyra-files-core` | `crates/lyra-files-core` |
| `lyra-files-napi` | `crates/lyra-files-napi` |
| `lyra-hardware-core` | `crates/lyra-hardware-core` |
| `lyra-image-core` | `crates/lyra-image-core` |
| `lyra-image-napi` | `crates/lyra-image-napi` |
| `lyra-lsp-core` | `crates/lyra-lsp-core` |
| `lyra-performance-core` | `crates/lyra-performance-core` |
| `lyra-process-lifecycle-core` | `crates/lyra-process-lifecycle-core` |
| `lyra-runtime-protocol` | `crates/lyra-runtime-protocol` |
| `lyra-terminal-core` | `crates/lyra-terminal-core` |
| `lyra-tool-fs-core` | `crates/lyra-tool-fs-core` |
| `lyrad` | `crates/lyrad` |

## JavaScript workspaces (7)

| Package | Location | Private |
| --- | --- | --- |
| `@lyra/desktop` | `apps/desktop` | yes |
| `@lyra/markdown-render` | `packages/markdown-render` | yes |
| `@lyra/plugin-sdk` | `packages/plugin-sdk` | yes |
| `@lyra/browser-automation` | `services/browser-automation` | yes |
| `@lyra/control-plane` | `services/control-plane` | yes |
| `@lyra/docs-web` | `web/docs` | yes |
| `@lyra/site` | `web/site` | yes |

## Electron main service directories (31)

`accessibility`, `agent`, `auth`, `auto-update`, `documents`, `download-manager`, `events`, `files`, `identity`, `image-viewer`, `language-packs`, `linux-compat`, `location`, `login-manager`, `lsp`, `performance`, `persona`, `runtime`, `screenshot-preview`, `search`, `sensitive-values`, `shared-process`, `storage`, `system-notifications`, `terminal`, `tests`, `uiux-packs`, `workbench-browser`, `workbench-documents`, `workbench-observation`, `workbench-state`

## Workbench business modules (43)

`activity-dock`, `agent-git`, `agent-plan-board`, `agent-project-tree`, `agent-session-history`, `agent-session-view-model`, `ai-panel`, `brand`, `browser-history`, `browser-search`, `browser-tabs`, `config`, `context-menu`, `file-editor`, `file-manager`, `gateway`, `global-dialog`, `i18n`, `identity`, `image-viewer`, `interaction-policy`, `layout`, `location`, `login-manager`, `notifications`, `observation`, `preferences`, `settings-ai`, `shell`, `sidebar`, `software-capabilities`, `software-store`, `state-storage`, `tabs`, `terminal-dock`, `terminal-profiles`, `text-metrics`, `theme`, `ui-platform`, `ui-primitives`, `ui-style`, `workspace-apps`, `workspace-tabs`
