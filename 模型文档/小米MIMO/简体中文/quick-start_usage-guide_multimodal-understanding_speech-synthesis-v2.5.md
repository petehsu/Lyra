...
模型的指令遵循能力足以 cover 以下这些复杂控制（一条自然语言指令即可生效）：
  ...
  * **多粒度控制** ：从 _段落级_ （整体基调）→ _句子级_ （节奏）→ _词级_ （重音）→ _字粒度_ （某一个字的哽咽、拖音、气音），都可在指令中指定。
...
[语音识别（MiMo-V2.5-ASR）](</docs/zh-CN/quick-start/usage-guide/multimodal-understanding/Speech-Recognition>)[语音合成（MiMo-V2-TTS）](</docs/zh-CN/quick-start/usage-guide/multimodal-understanding/speech-synthesis>)