...
fn should_close_random_fds() -> bool {
    #[cfg(target_os = "linux")]
    {
        // In Electron/Node hosts, blindly closing every /dev/fd entry in a forked
        // child can trip Rust IO ownership checks before exec, causing shell startup
        // to abort with FD ownership violations. Keep Linux default-off for Lyra.
        false
    }
