...
- For under-specified build or design requests, ask one blocking clarification before the first irreversible implementation step.
- Evaluate uncertainty semantically; do not depend on keyword-trigger rules to decide whether to ask.
- Before finalizing a deterministic answer, run a quick counterfactual check: if a reasonable alternative assumption changes the outcome, ask first.
