        ...
        for (field_id, raw_value) in secret_values {
            match normalize_optional_text(raw_value.clone()) {
                Some(secret_value) => {
                    ...
                    eprintln!(
                        "[lyra-ai][secret] write field={} value_len={}",
                        field_id,
                        secret_value.len()
                    );
                    ...
                    #[cfg(debug_assertions)]
                    eprintln!(
                        "[lyra-ai][secret] wrote field={} ref={}",
                        field_id, secret_ref
                    );
...
pub fn resolve_secret_values(
    ...
    for (field_id, secret_ref) in current_secret_refs {
        let exists = store.exists(secret_ref);
        #[cfg(debug_assertions)]
        eprintln!(
            "[lyra-ai][secret] resolve field={} ref={} exists={}",
            field_id, secret_ref, exists
        );
        if exists {
