...
//! Integration tests for Dart parser
...
const SAMPLE_APP: &str = include_str!("fixtures/sample_app.dart");
...
fn test_parse_sample_app_classes() {
    ...
    let file_info = parser
        .parse_source(SAMPLE_APP, Path::new("sample_app.dart"), &mut graph)
        .unwrap();
    ...
    assert!(
        ...
        "Should contain UserService class, found: {:?}",
        class_names
    );
...
fn test_parse_sample_app_imports() {
    ...
    // Should find dart:async and dart:convert imports
    assert!(
...
fn test_parse_sample_app_calls() {
    ...
    // Check that graph has edges (call relationships should exist)
...
fn test_parse_sample_app_complexity() {
    ...
    // At least one function should have complexity > 1 (addRole, _validateRole have if-statements)
...
fn test_parse_sample_app_summary() {
    ...
    println!("\n=== Dart Parser Sample App Summary ===");
    println!("File: sample_app.dart");
