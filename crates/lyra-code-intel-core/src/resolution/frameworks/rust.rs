...
impl FrameworkResolver for RustResolver {
    ...
    fn extract(&self, file_path: &Path, content: &str, graph: &mut codegraph::CodeGraph) -> Vec<UnresolvedRef> {
        if file_path.extension().and_then(|e| e.to_str()) != Some("rs") { return vec![]; }
        ...
        for cap in attr_re.captures_iter(content) {
            ...
            if let Some(m) = regex::Regex::new(r"\n\s*(?:pub\s+)?(?:async\s+)?fn\s+(\w+)").unwrap().captures(after) {
        ...
        for mat in route_re.find_iter(content) {
            let route_path = regex::Regex::new(r#""([^"]+)""#).unwrap().captures(&content[mat.start()..mat.end()]).map(|c| c[1].to_string());
            let Some(route_path) = route_path else { continue; };
            let line = line_at(content, mat.start());
            // Look for method handlers in the rest of the call.
            let after = &content[mat.end()..];
            let mh_re = regex::Regex::new(r#"\b(get|post|put|patch|delete|head)\s*\(\s*([A-Za-z_][\w:]*)"#).unwrap();
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        if name.ends_with("_handler") || name.starts_with("handle_") {
            if let Some(id) = resolve_by_name_and_kind(name, &["function"], &["/handlers/", "/api/", "/routes/"], ctx) {
                return Some(ResolvedRef { target_node_id: id, confidence: 0.8 });
            }
        }
        if name.ends_with("Service") || name.ends_with("Repository") {
            if let Some(id) = resolve_by_name_and_kind(name, &["struct", "trait", "class", "interface"], &["/services/", "/repository/", "/domain/"], ctx) {
                return Some(ResolvedRef { target_node_id: id, confidence: 0.8 });
            }
        }
        if is_pascal_case(name) {
            if let Some(id) = resolve_by_name_and_kind(name, &["struct", "class"], &["/models/", "/entity/", "/domain/", "/types/"], ctx) {
