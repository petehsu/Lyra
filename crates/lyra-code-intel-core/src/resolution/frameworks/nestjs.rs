...
impl FrameworkResolver for NestjsResolver {
    ...
    fn detect(&self, ctx: &ResolutionContext) -> bool {
        if pkg_json_has_dep(ctx, |d| d.starts_with("@nestjs/")) {
            return true;
        }
        ctx.get_all_files()
            .iter()
            .filter(|f| {
                f.ends_with(".controller.ts")
                    || f.ends_with(".controller.js")
                    || f.ends_with(".module.ts")
                    || f.ends_with(".resolver.ts")
                    || f.ends_with(".gateway.ts")
            })
            .take(20)
            .any(|f| {
                ctx.read_file(f)
                    .map(|c| c.contains("@nestjs/") || c.contains("@Controller"))
                    .unwrap_or(false)
            })
    }
...
fn find_method_name(after: &str) -> Option<String> {
    ...
    let re = regex::Regex::new(
        r#"(?:@\w+(?:\([^)]*\))?\s*)*(?:(?:public|private|protected|async|static)\s+)*(\w+)\s*\("#,
    )
    .unwrap();
