...
impl FrameworkResolver for ReactNativeBridgeResolver {
    ...
    fn detect(&self, ctx: &ResolutionContext) -> bool {
        ...
        ctx.get_all_files().iter().take(200).any(|f| {
            if f.ends_with(".mm") || f.ends_with(".m") {
                ctx.read_file(f)
                    .map(|c| c.contains("RCT_EXPORT_MODULE"))
                    .unwrap_or(false)
            } else if f.ends_with(".ts") || f.ends_with(".tsx") {
                ctx.read_file(f)
                    .map(|c| c.contains("TurboModuleRegistry.get"))
                    .unwrap_or(false)
            } else {
                false
            }
    ...
    fn extract(
        ...
        if !(fp.ends_with(".m") || fp.ends_with(".mm")) {
            return vec![];
        }
        if !content.contains("RCT_EXPORT_MODULE") {
            return vec![];
        }
        ...
        let class_name = regex::Regex::new(r"@implementation\s+(\w+)")
            .unwrap()
            .captures(content)
            .map(|c| c[1].to_string());
        let module_name = regex::Regex::new(r"RCT_EXPORT_MODULE\s*\(\s*(\w+)?\s*\)")
            .unwrap()
            .captures(content)
            ...
            .or_else(|| {
                class_name
                    .as_ref()
                    .map(|cn| cn.strip_prefix("RCT").unwrap_or(cn).to_string())
            });

        // RCT_EXPORT_METHOD(selectorFirstKw:...)
        let export_re =
            regex::Regex::new(r"RCT_EXPORT_METHOD\s*\(\s*([A-Za-z_][A-Za-z0-9_]*)").unwrap();
        for cap in export_re.captures_iter(content) {
            let js_name = &cap[1];
            if RN_EMITTER_BUILTINS.contains(&js_name) {
                continue;
            }
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        if RN_EMITTER_BUILTINS.contains(&bare) {
            return None;
        }
        ...
        for m in methods.iter().filter(|n| n.kind == "method") {
            let is_native = m.path.ends_with(".m")
                || m.path.ends_with(".mm")
                || m.path.ends_with(".java")
                || m.path.ends_with(".kt");
            if is_native {
