...
impl FrameworkResolver for FabricViewResolver {
    fn name(&self) -> &'static str { "fabric-view" }
    ...
    fn detect(&self, ctx: &ResolutionContext) -> bool {
        ...
        ctx.get_all_files().iter().take(200).any(|f| {
            if f.ends_with(".ts") || f.ends_with(".tsx") {
                ctx.read_file(f).map(|c| c.contains("codegenNativeComponent")).unwrap_or(false)
            } else if f.ends_with(".m") || f.ends_with(".mm") {
                ctx.read_file(f).map(|c| c.contains("RCT_EXPORT_VIEW_PROPERTY")).unwrap_or(false)
            } else { false }
        })
    }
    ...
    fn extract(&self, file_path: &Path, content: &str, graph: &mut codegraph::CodeGraph) -> Vec<UnresolvedRef> {
        ...
        if matches!(lang, "typescript") {
            if !content.contains("codegenNativeComponent") { return vec![]; }
            let re = regex::Regex::new(r#"codegenNativeComponent\s*(?:<[^>]+>)?\s*\(\s*['"]([A-Za-z_][A-Za-z0-9_]*)['"]"#).unwrap();
            ...
            if let Some(body) = find_native_props_body(content) {
                for prop in extract_prop_names(&body) {
                    let line = content.find(&prop).map(|i| line_at(content, i)).unwrap_or(1);
        ...
        if lang == "objc" {
            if !content.contains("RCT_EXPORT_VIEW_PROPERTY") && !content.contains("RCT_CUSTOM_VIEW_PROPERTY") {
                return vec![];
            }
            if let Some(cap) = regex::Regex::new(r"@implementation\s+(\w+)").unwrap().captures(content) {
                ...
                if class_name.ends_with("Manager") || class_name.ends_with("ViewManager") {
                    ...
                    let prop_re = regex::Regex::new(r"RCT_(?:EXPORT|CUSTOM|REMAP)_VIEW_PROPERTY\s*\(\s*(\w+)").unwrap();
...
fn derive_component_name(class: &str) -> String {
    ...
    if let Some(s) = name.strip_suffix("ViewManager") { s.to_string() }
    else if let Some(s) = name.strip_suffix("Manager") { s.to_string() }
    else { name.to_string() }
}
