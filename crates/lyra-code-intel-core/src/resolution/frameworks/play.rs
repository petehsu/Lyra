...
impl FrameworkResolver for PlayResolver {
    ...
    fn detect(&self, ctx: &ResolutionContext) -> bool {
        if let Some(c) = ctx.read_file("build.sbt") {
            if c.contains("playframework")
                || c.contains("\"play\"")
                || c.contains("PlayScala")
                || c.contains("PlayJava")
            {
                return true;
            }
    ...
    fn extract(
        ...
        if base != "routes" && !base.ends_with(".routes") {
            return vec![];
        }

        let mut refs = Vec::new();
        let re = regex::Regex::new(r"^(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\s+(\S+)\s+(.+)$")
            .unwrap();
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        if let Some(cap) = regex::Regex::new(r"^(\w+)\.(\w+)$").unwrap().captures(name) {
                ...
                .filter(|n| n.kind == "class")
            {
                ...
                if let Some(n) = nodes
                    .iter()
                    .find(|n| (n.kind == "method" || n.kind == "function") && n.name == method_name)
                {
