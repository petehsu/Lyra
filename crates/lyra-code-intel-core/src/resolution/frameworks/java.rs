...
impl FrameworkResolver for SpringResolver {
    ...
    fn detect(&self, ctx: &ResolutionContext) -> bool {
        for f in &["pom.xml", "build.gradle", "build.gradle.kts"] {
            if let Some(c) = ctx.read_file(f) {
                if c.contains("spring-boot") || c.contains("springframework") { return true; }
            }
        }
        ctx.get_all_files().iter().filter(|f| f.ends_with(".java")).take(30).any(|f| {
            ctx.read_file(f).map(|c| c.contains("@SpringBootApplication") || c.contains("@RestController") || c.contains("@Service")).unwrap_or(false)
        })
    }
    ...
    fn extract(&self, file_path: &Path, content: &str, graph: &mut codegraph::CodeGraph) -> Vec<UnresolvedRef> {
        ...
        if !matches!(ext, "java" | "kt") { return vec![]; }
        ...
        let class_prefix = content.find("@RequestMapping").and_then(|_| {
        ...
        let verb_map = [("GetMapping", "GET"), ("PostMapping", "POST"), ("PutMapping", "PUT"), ("PatchMapping", "PATCH"), ("DeleteMapping", "DELETE")];
        for (deco, verb) in verb_map {
            let re = regex::Regex::new(&format!(r#"@{deco}\s*(?:\(\s*["']([^'"]*)['"]\s*\))?"#)).unwrap();
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        if name.ends_with("Service") {
            if let Some(id) = resolve_by_name_and_kind(name, &["class", "interface"], &["/service/", "/services/"], ctx) {
                return Some(ResolvedRef { target_node_id: id, confidence: 0.85 });
            }
        }
        if name.ends_with("Repository") {
            if let Some(id) = resolve_by_name_and_kind(name, &["class", "interface"], &["/repository/", "/repositories/"], ctx) {
                return Some(ResolvedRef { target_node_id: id, confidence: 0.85 });
            }
        }
        if name.ends_with("Controller") {
            if let Some(id) = resolve_by_name_and_kind(name, &["class"], &["/controller/", "/controllers/"], ctx) {
                return Some(ResolvedRef { target_node_id: id, confidence: 0.85 });
            }
        }
        if is_pascal_case(name) {
            if let Some(id) = resolve_by_name_and_kind(name, &["class"], &["/entity/", "/entities/", "/model/", "/domain/"], ctx) {
