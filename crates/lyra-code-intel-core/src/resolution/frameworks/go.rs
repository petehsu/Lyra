...
use super::helpers::{extract_tail_ident, line_at, make_ref, make_route_node, resolve_by_name_and_kind};
...
impl FrameworkResolver for GoResolver {
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        if name.ends_with("Handler") || name.starts_with("Handle") {
            if let Some(id) = resolve_by_name_and_kind(name, &["function"], &["/handler/", "/api/", "/routes/"], ctx) {
        ...
        if name.ends_with("Service") || name.ends_with("Repository") || name.ends_with("Store") {
            if let Some(id) = resolve_by_name_and_kind(name, &["struct", "interface", "class"], &["/service/", "/repository/", "/store/"], ctx) {
                return Some(ResolvedRef { target_node_id: id, confidence: 0.8 });
            }
        }
        if is_pascal_case(name) {
            if let Some(id) = resolve_by_name_and_kind(name, &["struct", "class"], &["/model/", "/models/", "/entity/", "/domain/"], ctx) {
