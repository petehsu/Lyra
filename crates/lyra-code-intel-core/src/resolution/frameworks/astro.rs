...
impl FrameworkResolver for AstroResolver {
    ...
    fn detect(&self, ctx: &ResolutionContext) -> bool {
        if pkg_json_has_dep(ctx, |d| d == "astro") {
            return true;
        }
    ...
    fn extract(
        ...
        let norm = fp.replace('\\', "/");
        if !norm.contains("/src/pages/") {
            return vec![];
        }
        if !(norm.ends_with(".astro")
            || norm.ends_with(".ts")
            || norm.ends_with(".js")
            || norm.ends_with(".mjs"))
        {
        ...
        if let Some(idx) = norm.find("/src/pages/") {
            ...
            let lang = if norm.ends_with(".astro") {
                "astro"
            } else {
                "typescript"
            };
...
fn astro_route(after_pages: &str) -> String {
    let without_ext = after_pages
        .strip_suffix(".astro")
        .or_else(|| after_pages.strip_suffix(".ts"))
        .or_else(|| after_pages.strip_suffix(".js"))
        .or_else(|| after_pages.strip_suffix(".mjs"))
