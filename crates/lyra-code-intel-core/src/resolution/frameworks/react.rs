...
use super::helpers::{
    line_at, make_ref, make_route_node, pkg_json_has_dep, resolve_by_name_and_kind,
};
...
impl FrameworkResolver for ReactResolver {
    ...
    fn detect(&self, ctx: &ResolutionContext) -> bool {
        if pkg_json_has_dep(ctx, |d| d == "react" || d == "next" || d == "react-native") {
            return true;
        }
        ctx.get_all_files()
            .iter()
    ...
    fn extract(
        ...
        let lang = if ext == "tsx" {
            ...
            super::helpers::detect_lang(file_path)
        };
        ...
        for mat in route_re.find_iter(content) {
            ...
            let Some(path_match) = path_cap.captures(window) else {
                continue;
            };
            let route_path = &path_match[1];
            let comp_re =
                regex::Regex::new(r#"(?:component|element)\s*=\s*\{?\s*<?\s*([A-Z][A-Za-z0-9_]*)"#)
                    .unwrap();
        ...
        if (fp_str.contains("/pages/") || fp_str.contains("/app/"))
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        if name.starts_with("use") && name.len() > 3 {
            if let Some(id) =
                resolve_by_name_and_kind(name, &["function"], &["/hooks/", "/src/hooks/"], ctx)
            {
                return Some(ResolvedRef {
                    target_node_id: id,
                    confidence: 0.85,
