...
use super::helpers::{join_path, line_at, make_ref, make_route_node, resolve_by_name_and_kind, extract_tail_ident};
...
impl FrameworkResolver for AspNetResolver {
    fn name(&self) -> &'static str { "aspnet" }

    fn detect(&self, ctx: &ResolutionContext) -> bool {
        // .csproj with ASP.NET references
        for f in ctx.get_all_files().iter().filter(|f| f.ends_with(".csproj")).take(10) {
            if let Some(c) = ctx.read_file(f) {
                if c.contains("Microsoft.AspNetCore") || c.contains("Microsoft.NET.Sdk.Web") { return true; }
            }
        }
        if ctx.file_exists("Startup.cs") { return true; }
        // Source-level detection
        ctx.get_all_files().iter()
            .filter(|f| f.ends_with("Controller.cs") || f.ends_with("Program.cs") || f.ends_with("Startup.cs"))
            .take(30)
            .any(|f| {
                ctx.read_file(f).map(|c| c.contains("ControllerBase") || c.contains("WebApplication") || c.contains("[HttpGet") || c.contains("[Route")).unwrap_or(false)
            })
    }

    fn extract(&self, file_path: &Path, content: &str, graph: &mut codegraph::CodeGraph) -> Vec<UnresolvedRef> {
        if file_path.extension().and_then(|e| e.to_str()) != Some("cs") { return vec![]; }
        let fp = file_path.to_string_lossy();
        let mut refs = Vec::new();

        // Class-level [Route("api/[controller]")] prefix
        let cls_re = regex::Regex::new(r#"\[Route\s*\(\s*"([^"]+)"#).unwrap();
        let class_prefix = content.find("[Route").and_then(|_| {
        ...
        let attrs = [("HttpGet", "GET"), ("HttpPost", "POST"), ("HttpPut", "PUT"), ("HttpPatch", "PATCH"), ("HttpDelete", "DELETE")];
        for (attr, verb) in attrs {
            let re = regex::Regex::new(&format!(r#"\[{attr}(?:\s*\(\s*"([^"]*)"[^)]*\))?\s*\]"#)).unwrap();
            for cap in re.captures_iter(content) {
                let sub = cap.get(1).map(|m| m.as_str().to_string()).unwrap_or_default();
                ...
                if let Some(m) = regex::Regex::new(r#"(?:public|private|protected|internal)\s+[\w<>,\s\[\]?.]+?\s+(\w+)\s*\("#).unwrap().captures(window) {
        ...
        let min_re = regex::Regex::new(r#"\.Map(Get|Post|Put|Patch|Delete)\s*\(\s*"([^"]+)"\s*,\s*([^,)]+)"#).unwrap();
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        if name.ends_with("Service") || (name.starts_with('I') && name.len() > 1) {
            if let Some(id) = resolve_by_name_and_kind(name, &["class", "interface"], &["/Services/", "/Application/"], ctx) {
                return Some(ResolvedRef { target_node_id: id, confidence: 0.85 });
            }
        }
        if name.ends_with("Repository") {
            if let Some(id) = resolve_by_name_and_kind(name, &["class", "interface"], &["/Repositories/", "/Data/"], ctx) {
                return Some(ResolvedRef { target_node_id: id, confidence: 0.85 });
            }
        }
        if is_pascal_case(name) {
            if let Some(id) = resolve_by_name_and_kind(name, &["class"], &["/Models/", "/Entities/", "/Domain/"], ctx) {
