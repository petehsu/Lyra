...
use super::helpers::{
    line_at, make_ref, make_route_node, make_synthetic_node, resolve_by_name_and_kind,
};
...
impl FrameworkResolver for SwiftUiResolver {
    ...
    fn detect(&self, ctx: &ResolutionContext) -> bool {
        ctx.get_all_files()
            ...
            .any(|f| {
                ctx.read_file(f)
                    .map(|c| c.contains("import SwiftUI"))
                    .unwrap_or(false)
            })
    }
    ...
    fn extract(
        ...
        if file_path.extension().and_then(|e| e.to_str()) != Some("swift") {
            return vec![];
        }
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        if name.ends_with("ViewModel") || name.ends_with("Store") || name.ends_with("Manager") {
            if let Some(id) = resolve_by_name_and_kind(
                name,
                &["class"],
                &["/ViewModels/", "/Stores/", "/Managers/", "/Services/"],
                ctx,
            ) {
                return Some(ResolvedRef {
                    target_node_id: id,
...
impl FrameworkResolver for UiKitResolver {
    ...
    fn detect(&self, ctx: &ResolutionContext) -> bool {
        ctx.get_all_files()
            ...
            .any(|f| {
                ctx.read_file(f)
                    .map(|c| {
                        c.contains("import UIKit")
                            || c.contains("UIViewController")
                            || c.contains("UIView")
                    })
                    .unwrap_or(false)
            })
    ...
    fn extract(
        ...
        let vc_re =
            regex::Regex::new(r"class\s+(\w+)\s*:\s*(?:\w+\s*,\s*)*UIViewController").unwrap();
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        if name.ends_with("ViewController") {
            if let Some(id) = resolve_by_name_and_kind(
                name,
                &["class"],
                &["/ViewControllers/", "/Controllers/", "/Screens/"],
                ctx,
            ) {
                return Some(ResolvedRef {
                    target_node_id: id,
        ...
        if name.ends_with("Cell") {
            if let Some(id) =
                resolve_by_name_and_kind(name, &["class"], &["/Cells/", "/Views/"], ctx)
            {
                return Some(ResolvedRef {
                    target_node_id: id,
                    confidence: 0.85,
...
impl FrameworkResolver for VaporResolver {
    ...
    fn extract(
        ...
        for cap in re.captures_iter(content) {
            ...
            for str_cap in regex::Regex::new(r#""([^"]*)""#)
                .unwrap()
                .captures_iter(segs)
            {
                path.push('/');
