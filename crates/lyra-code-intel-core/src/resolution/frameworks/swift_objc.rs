...
const GENERIC_NAMES: &[&str] = &[
    "init", "description", "hash", "isEqual", "copy", "class", "self", "count",
    "length", "value", "name", "data", "string", "add", "remove", "update",
    "load", "save", "reload", "cancel", "start", "stop", "close", "open",
    "show", "hide", "dealloc",
];

impl FrameworkResolver for SwiftObjcBridgeResolver {
    fn name(&self) -> &'static str { "swift-objc-bridge" }

    fn detect(&self, ctx: &ResolutionContext) -> bool {
        let mut has_swift = false;
        let mut has_objc = false;
        for f in ctx.get_all_files() {
            if f.ends_with(".swift") { has_swift = true; }
            if f.ends_with(".m") || f.ends_with(".mm") { has_objc = true; }
            if has_swift && has_objc { return true; }
        }
        false
    }

    fn extract(&self, _file_path: &Path, _content: &str, _graph: &mut codegraph::CodeGraph) -> Vec<UnresolvedRef> {
        vec![]
    }
    ...
    fn resolve(&self, reference: &UnresolvedRef, ctx: &ResolutionContext) -> Option<ResolvedRef> {
        ...
        match lang {
            "swift" => {
                ...
                if GENERIC_NAMES.contains(&bare) { return None; }
