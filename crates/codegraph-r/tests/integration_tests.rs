...
//! Integration tests for R parser
...
const SAMPLE_APP: &str = include_str!("fixtures/sample_app.R");
...
fn test_parse_sample_app_classes() {
    ...
    let file_info = parser
        .parse_source(SAMPLE_APP, Path::new("sample_app.R"), &mut graph)
        .unwrap();

    // R uses informal class constructors; parser may detect them as classes or functions
...
fn test_parse_sample_app_functions() {
    ...
    // Should find multiple functions (Entity, display, User, add_role, is_admin, etc.)
    assert!(
    ...
    assert!(
        func_names.iter().any(|n| n.contains("add_role")),
        "Should contain add_role function, found: {:?}",
        func_names
    );
    assert!(
        func_names.iter().any(|n| n.contains("find_user")),
        "Should contain find_user function, found: {:?}",
        func_names
    );
...
fn test_parse_sample_app_imports() {
    ...
    // R parser currently has a known issue with library() extraction.
    // When fixed, should find library(jsonlite), library(httr), source("utils.R").
    // For now, just verify parsing doesn't crash and report what was found.
    ...
    println!("Imports found: {} ({:?})", file_info.imports.len(), import_names);
}
...
fn test_parse_sample_app_complexity() {
    ...
    // At least one function should have complexity > 1 (add_role, find_user have if/for)
...
fn test_parse_sample_app_summary() {
    ...
    println!("\n=== R Parser Sample App Summary ===");
    println!("File: sample_app.R");
