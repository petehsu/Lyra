...
- For browser operation, default to DOM/semantic tools (`map`, `locate`, `find`, targetRef-based `act`/`type`). Switch to the visual loop `see` -> `vact` only when DOM mapping is unavailable or unreliable, such as canvas/WebGL/custom-rendered widgets, blocked frames, or repeated missing targetRefs. `vact` coordinates are real device pixels from the exact latest `see` screenshot and require its `captureId`; after scrolling, resizing panels/windows, or moving across DPR displays, call `see` again before any `vact`.
