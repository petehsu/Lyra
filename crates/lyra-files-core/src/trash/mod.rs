...
fn mac_trash_root() -> Result<PathBuf> {
    let home = dirs::home_dir().ok_or_else(|| crate::fail("home directory is unavailable"))?;
    Ok(home.join(".Trash"))
}
...
fn move_to_trash_macos(paths: &[String], storage_root: &Path) -> Result<()> {
    ...
    fs::create_dir_all(&trash_root)
        .map_err(|error| crate::io_fail("failed to create macOS trash directory", error))?;
...
fn restore_from_trash_macos(item_ids: &[String], storage_root: &Path) -> Result<()> {
    ...
    for item_id in item_ids {
        let record = index
            ...
            .cloned()
            .ok_or_else(|| crate::fail(format!("trash item not restorable: {}", item_id)))?;
        ...
        if let Some(parent) = original_path.parent() {
            fs::create_dir_all(parent).map_err(|error| {
                crate::io_fail(format!("failed to create {}", parent.display()), error)
            })?;
        }
            ...
            .map_err(|error| crate::io_fail(format!("failed to read {}", trash_root.display()), error))?
        {
            ...
            if path.is_dir() {
                fs::remove_dir_all(&path).map_err(|error| {
                    crate::io_fail(format!("failed to remove {}", path.display()), error)
                })?;
            } else {
...
fn trash_entry_from_native(item: &TrashItem) -> Result<FileManagerTrashEntry> {
    let metadata = os_limited::metadata(item)
        .map_err(|error| crate::fail(format!("failed to read trash metadata: {}", error)))?;
...
pub fn move_to_trash(paths: &[String], storage_root: &str) -> Result<()> {
    ...
    #[cfg(not(target_os = "macos"))]
    {
        ...
        trash::delete_all(paths)
            .map_err(|error| crate::fail(format!("failed to move items to trash: {}", error)))?;
        return Ok(());
    }
...
pub fn restore_from_trash(item_ids: &[String], storage_root: &str) -> Result<()> {
    ...
    ))]
    {
        ...
        os_limited::restore_all(items)
            .map_err(|error| crate::fail(format!("failed to restore trash items: {}", error)))?;
        return Ok(());
    }
...
pub fn empty_trash(storage_root: &str) -> Result<()> {
    ...
    ))]
    {
        ...
        os_limited::purge_all(&items)
            .map_err(|error| crate::fail(format!("failed to empty trash: {}", error)))?;
        return Ok(());
    }
