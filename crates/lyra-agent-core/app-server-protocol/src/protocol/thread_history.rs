...
use lyra_protocol::protocol::TurnStartedEvent;
use lyra_protocol::protocol::UserMessageEvent;
use lyra_protocol::protocol::ViewImageToolCallEvent;
use lyra_protocol::protocol::WebSearchBeginEvent;
use lyra_protocol::protocol::WebSearchEndEvent;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `crates/lyra-agent-core/core/rollout/policy.rs`.
    pub fn handle_event(&mut self, event: &EventMsg) {
        match event {
            ...
            EventMsg::PlanApprovalResolved(payload) => self.handle_plan_approval_resolved(payload),
    ...
    fn handle_response_item(&mut self, item: &lyra_protocol::models::ResponseItem) {
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &lyra_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &lyra_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &lyra_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &lyra_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &lyra_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &lyra_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &lyra_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &lyra_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &lyra_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &lyra_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &lyra_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(&mut self, payload: &lyra_protocol::protocol::ReviewRequest) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &lyra_protocol::protocol::ExitedReviewModeEvent,
    ) {
        let review = payload
            .review_output
            .as_ref()
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        turn.error = Some(V2TurnError {
            ...
            lyra_error_info: payload.lyra_error_info.clone().map(Into::into),
    ...
    fn finish_current_turn(&mut self) {
        if let Some(turn) = self.current_turn.take() {
            if turn.items.is_empty() && !turn.opened_explicitly {
                return;
            }
...
fn convert_dynamic_tool_content_items(
    items: &[lyra_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            lyra_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            lyra_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
...
pub use crate::protocol::thread_ai_panel_view_model::build_thread_ai_panel_view_model;
...
mod tests {
    ...
    #[test]
    fn builds_ai_panel_view_model_from_thread_turns() {
        let thread = Thread {
            id: "thread-1".into(),
            forked_from_id: None,
            preview: "Hello".into(),
        ...
        assert_eq!(view_model.messages[0].role, ThreadAiPanelMessageRole::User);
        assert!(view_model.messages[0].content.contains("Hello"));
        assert_eq!(view_model.messages[0].content_parts.len(), 3);
        assert_eq!(
            view_model.messages[1].display_content.as_deref(),
            Some("Hi there")
        );
        assert_eq!(
            view_model.messages[2].display_content.as_deref(),
            Some("thinking")
        );

        assert_eq!(view_model.tool_calls.len(), 1);
        assert_eq!(view_model.tool_calls[0].id, "exec-1");
        assert_eq!(view_model.tool_calls[0].tool_name, "terminal.exec");
        assert_eq!(
            view_model.tool_calls[0].status,
            ThreadAiPanelToolCallStatus::Completed
        );
        assert_eq!(view_model.tool_calls[0].started_at_ms, 10_003);
        assert_eq!(view_model.tool_calls[0].finished_at_ms, Some(10_015));
        ...
        assert_eq!(
            view_model.turn_meta[0]
                .first_assistant_message_id
                .as_deref(),
            Some("assistant-1")
        );
        assert_eq!(
            view_model.turn_meta[0].last_assistant_message_id.as_deref(),
            Some("reasoning-1")
        );
        assert_eq!(view_model.turn_meta[0].assistant_order, Some(1));
        assert!(view_model.turn_meta[0].has_assistant_display);
    }
    ...
    fn plan_approval_resolution_marks_waiting_plan_completed() {
        ...
        let items = vec![
            ...
            RolloutItem::EventMsg(EventMsg::PlanApprovalResolved(PlanApprovalResolvedEvent {
                thread_id: thread_id.to_string(),
                plan_turn_id: "turn-plan".into(),
                request_id: "plan:turn-plan".into(),
                decision: PlanApprovalResolutionDecision::ApproveAndImplement,
            })),
        ];
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(lyra_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(lyra_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: lyra_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(lyra_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                lyra_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
            ...
            EventMsg::CollabAgentInteractionEnd(
                lyra_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
                    receiver_agent_nickname: None,
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                lyra_error_info: Some(LyraErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                message: "stream failure".into(),
                lyra_error_info: Some(
                    crate::protocol::v2::LyraErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(lyra_protocol::models::ResponseItem::Message {
                id: Some("msg-1".into()),
                role: "user".into(),
                content: vec![lyra_protocol::models::ContentItem::InputText {
