...
use crate::RequestId;
use lyra_protocol::approvals::AutoReviewAssessmentAction as CoreAutoReviewAssessmentAction;
use lyra_protocol::approvals::AutoReviewAssessmentDecisionSource as CoreAutoReviewAssessmentDecisionSource;
use lyra_protocol::approvals::AutoReviewCommandSource as CoreAutoReviewCommandSource;
use lyra_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use lyra_protocol::approvals::ExecPolicyAmendment as CoreExecPolicyAmendment;
use lyra_protocol::approvals::NetworkApprovalContext as CoreNetworkApprovalContext;
use lyra_protocol::approvals::NetworkApprovalProtocol as CoreNetworkApprovalProtocol;
use lyra_protocol::approvals::NetworkPolicyAmendment as CoreNetworkPolicyAmendment;
use lyra_protocol::approvals::NetworkPolicyRuleAction as CoreNetworkPolicyRuleAction;
use lyra_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use lyra_protocol::config_types::CollaborationMode;
use lyra_protocol::config_types::CollaborationModeMask as CoreCollaborationModeMask;
use lyra_protocol::config_types::ModeKind;
use lyra_protocol::config_types::ReasoningSummary;
use lyra_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use lyra_protocol::config_types::WebSearchToolConfig;
use lyra_protocol::items::AgentMessageContent as CoreAgentMessageContent;
use lyra_protocol::items::TurnItem as CoreTurnItem;
use lyra_protocol::mcp::CallToolResult as CoreMcpCallToolResult;
use lyra_protocol::mcp::Resource as McpResource;
pub use lyra_protocol::mcp::ResourceContent as McpResourceContent;
use lyra_protocol::mcp::ResourceTemplate as McpResourceTemplate;
use lyra_protocol::mcp::Tool as McpTool;
use lyra_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use lyra_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
use lyra_protocol::models::FileSystemPermissions as CoreFileSystemPermissions;
use lyra_protocol::models::MessagePhase;
use lyra_protocol::models::NetworkPermissions as CoreNetworkPermissions;
use lyra_protocol::models::PermissionProfile as CorePermissionProfile;
use lyra_protocol::openai_models::InputModality;
use lyra_protocol::openai_models::ModelAvailabilityNux as CoreModelAvailabilityNux;
...
use lyra_protocol::protocol::AskForApproval as CoreAskForApproval;
use lyra_protocol::protocol::AutoReviewRiskLevel as CoreAutoReviewRiskLevel;
use lyra_protocol::protocol::AutoReviewUserAuthorization as CoreAutoReviewUserAuthorization;
use lyra_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use lyra_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
use lyra_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use lyra_protocol::protocol::HookEventName as CoreHookEventName;
use lyra_protocol::protocol::HookExecutionMode as CoreHookExecutionMode;
use lyra_protocol::protocol::HookHandlerType as CoreHookHandlerType;
use lyra_protocol::protocol::HookOutputEntry as CoreHookOutputEntry;
use lyra_protocol::protocol::HookOutputEntryKind as CoreHookOutputEntryKind;
use lyra_protocol::protocol::HookRunStatus as CoreHookRunStatus;
use lyra_protocol::protocol::HookRunSummary as CoreHookRunSummary;
use lyra_protocol::protocol::HookScope as CoreHookScope;
use lyra_protocol::protocol::HookSource as CoreHookSource;
use lyra_protocol::protocol::LyraErrorInfo as CoreLyraErrorInfo;
use lyra_protocol::protocol::ModelRerouteReason as CoreModelRerouteReason;
use lyra_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use lyra_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
use lyra_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
use lyra_protocol::protocol::ReadOnlyAccess as CoreReadOnlyAccess;
use lyra_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
use lyra_protocol::protocol::ReviewDecision as CoreReviewDecision;
use lyra_protocol::protocol::SessionSource as CoreSessionSource;
use lyra_protocol::protocol::SkillDependencies as CoreSkillDependencies;
use lyra_protocol::protocol::SkillInterface as CoreSkillInterface;
use lyra_protocol::protocol::SkillMetadata as CoreSkillMetadata;
use lyra_protocol::protocol::SkillScope as CoreSkillScope;
use lyra_protocol::protocol::SkillToolDependency as CoreSkillToolDependency;
use lyra_protocol::protocol::SubAgentSource as CoreSubAgentSource;
use lyra_protocol::protocol::TokenUsage as CoreTokenUsage;
use lyra_protocol::protocol::TokenUsageInfo as CoreTokenUsageInfo;
use lyra_protocol::request_permissions::PermissionGrantScope as CorePermissionGrantScope;
use lyra_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use lyra_protocol::user_input::ByteRange as CoreByteRange;
use lyra_protocol::user_input::TextElement as CoreTextElement;
use lyra_protocol::user_input::UserInput as CoreUserInput;
...
pub enum LyraErrorInfo {
    ...
    /// cannot accept same-turn steering, for example `/review`.
...
impl From<CoreLyraErrorInfo> for LyraErrorInfo {
    fn from(value: CoreLyraErrorInfo) -> Self {
        match value {
            CoreLyraErrorInfo::ContextWindowExceeded => LyraErrorInfo::ContextWindowExceeded,
            CoreLyraErrorInfo::UsageLimitExceeded => LyraErrorInfo::UsageLimitExceeded,
            CoreLyraErrorInfo::ServerOverloaded => LyraErrorInfo::ServerOverloaded,
            CoreLyraErrorInfo::HttpConnectionFailed { http_status_code } => {
                LyraErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreLyraErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                LyraErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreLyraErrorInfo::InternalServerError => LyraErrorInfo::InternalServerError,
            CoreLyraErrorInfo::Unauthorized => LyraErrorInfo::Unauthorized,
            CoreLyraErrorInfo::BadRequest => LyraErrorInfo::BadRequest,
            CoreLyraErrorInfo::ThreadRollbackFailed => LyraErrorInfo::ThreadRollbackFailed,
            CoreLyraErrorInfo::SandboxError => LyraErrorInfo::SandboxError,
            CoreLyraErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                LyraErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreLyraErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                LyraErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreLyraErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                LyraErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreLyraErrorInfo::Other => LyraErrorInfo::Other,
        }
    }
}
...
v2_enum_from_core!(
    pub enum ReviewDelivery from lyra_protocol::protocol::ReviewDelivery {
        Inline, Detached
    }
);

v2_enum_from_core!(
    pub enum McpAuthStatus from lyra_protocol::protocol::McpAuthStatus {
        Unsupported,
        NotLoggedIn,
        BearerToken,
        OAuth
...
pub enum ConfigLayerSource {
    ...
    /// User config layer from $LYRA_HOME/config.toml. This layer is special
    ...
    /// Path to a .lyra/ folder within a project. There could be multiple of
    /// these between `cwd` and the project/repo root.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    Project {
        dot_lyra_folder: AbsolutePathBuf,
    },
...
pub struct NetworkRequirements {
    ...
    /// Canonical network permission map for `network`.
    ...
    /// Canonical unix socket permission map for `network`.
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraAiProviderFieldOption {
    pub value: String,
    pub label: String,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraAiProviderFieldSchema {
    pub id: String,
    pub label: String,
    pub kind: String,
    pub scope: String,
    ...
    pub options: Vec<LyraAiProviderFieldOption>,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraAiProviderModelEntry {
    pub id: String,
    pub name: String,
    pub description: Option<String>,
    pub context_window: Option<u64>,
    ...
    pub runtime_metadata: Option<LyraAiModelRuntimeMetadata>,
    pub source: String,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraAiModelDiscoveryState {
    pub status: String,
    pub last_checked_at: Option<i64>,
    pub error_message: Option<String>,
    #[serde(default)]
    pub models: Vec<LyraAiProviderModelEntry>,
}

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraAiProviderProfile {
    pub id: String,
    pub name: String,
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraConfigProfilesListResponse {
    #[serde(default)]
    pub profiles: Vec<LyraAiProviderProfile>,
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraConfigProfileUpsertParams {
    ...
    pub auth_config: BTreeMap<String, String>,
    #[serde(default)]
    pub secret_values: BTreeMap<String, JsonValue>,
    #[serde(default)]
    ...
    pub custom_models: Vec<LyraAiProviderModelEntry>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub discovery_state: Option<LyraAiModelDiscoveryState>,
}

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraConfigProfileUpsertResponse {
    pub profile: LyraAiProviderProfile,
}

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraConfigProfileDeleteParams {
    pub id: String,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraConfigProfileDeleteResponse {}

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraConfigProfileSetDefaultParams {
    pub id: String,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraConfigProfileSetDefaultResponse {
    pub profile: Option<LyraAiProviderProfile>,
}

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraAiProviderCatalogItem {
    pub id: String,
    pub label: String,
    pub description: String,
    pub protocol_id: String,
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraAiProviderPreset {
    pub id: String,
    pub provider_id: String,
    pub protocol_id: String,
    pub label: String,
    ...
    #[serde(default)]
    pub connection_fields: Vec<LyraAiProviderFieldSchema>,
    #[serde(default)]
    pub auth_fields: Vec<LyraAiProviderFieldSchema>,
    #[serde(default)]
    pub default_connection_config: BTreeMap<String, String>,
    #[serde(default)]
    pub default_auth_config: BTreeMap<String, String>,
    #[serde(default)]
    pub recommended_models: Vec<LyraAiProviderModelEntry>,
}

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraConfigProvidersCatalogReadResponse {
    #[serde(default)]
    pub providers: Vec<LyraAiProviderCatalogItem>,
    #[serde(default)]
    pub presets: Vec<LyraAiProviderPreset>,
}

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraHostToolsSyncParams {
    pub tool_set_id: String,
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraHostToolsSyncResponse {
    pub accepted_count: usize,
    pub dropped_as_lyra_owned_count: usize,
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraHostToolsRemoveParams {
    pub tool_set_id: String,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraHostToolsRemoveResponse {}

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraPersonaContextParams {
    pub persona_name: String,
    pub company_name: String,
    pub company_description: String,
    pub coworker_label: String,
    ...
    pub physical_location_display: String,
    pub ip_location_display: String,
    #[ts(optional = nullable)]
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct LyraPersonaContextSetResponse {}
...
impl SandboxPolicy {
    pub fn to_core(&self) -> lyra_protocol::protocol::SandboxPolicy {
        match self {
            SandboxPolicy::DangerFullAccess => {
                lyra_protocol::protocol::SandboxPolicy::DangerFullAccess
            }
            SandboxPolicy::ReadOnly {
                access,
                network_access,
            } => lyra_protocol::protocol::SandboxPolicy::ReadOnly {
                access: access.to_core(),
                network_access: *network_access,
            },
            SandboxPolicy::ExternalSandbox { network_access } => {
                lyra_protocol::protocol::SandboxPolicy::ExternalSandbox {
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                exclude_slash_tmp,
            } => lyra_protocol::protocol::SandboxPolicy::WorkspaceWrite {
...
impl From<lyra_protocol::protocol::SandboxPolicy> for SandboxPolicy {
    fn from(value: lyra_protocol::protocol::SandboxPolicy) -> Self {
        match value {
            lyra_protocol::protocol::SandboxPolicy::DangerFullAccess => {
                SandboxPolicy::DangerFullAccess
            }
            ...
            lyra_protocol::protocol::SandboxPolicy::ExternalSandbox { network_access } => {
...
/// collaboration mode preset metadata for clients.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
/// app metadata returned by app-list APIs.
...
pub struct ThreadStartParams {
    ...
    /// This is for internal use only (e.g. Lyra Cloud).
    #[serde(default)]
    pub raw_events: bool,
...
pub struct ThreadMetadataUpdateParams {
    ...
    #[ts(optional = nullable)]
    pub bound_project_root: Option<Option<String>>,
...
impl ThreadMemoryMode {
    ...
    pub fn to_core(self) -> lyra_protocol::protocol::ThreadMemoryMode {
        match self {
            Self::Enabled => lyra_protocol::protocol::ThreadMemoryMode::Enabled,
            Self::Disabled => lyra_protocol::protocol::ThreadMemoryMode::Disabled,
        }
    }
}
...
pub struct Thread {
    ...
    pub bound_project_root: Option<AbsolutePathBuf>,
    /// Version of the CLI that created the thread.
    pub cli_version: String,
    /// Origin of the thread (CLI, VSCode, lyra exec, lyra app-server, etc.).
...
pub struct TurnError {
    pub message: String,
    pub lyra_error_info: Option<LyraErrorInfo>,
    #[serde(default)]
...
/// transport used by thread realtime.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(tag = "type", rename_all = "camelCase")]
...
/// response for starting thread realtime.
#[derive(Serialize, Deserialize, Debug, Default, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct ThreadRealtimeStartResponse {}

/// append audio input to thread realtime.
#[derive(Serialize, Deserialize, Debug, Default, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
/// response for appending realtime audio input.
#[derive(Serialize, Deserialize, Debug, Default, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct ThreadRealtimeAppendAudioResponse {}

/// append text input to thread realtime.
#[derive(Serialize, Deserialize, Debug, Default, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
/// response for stopping thread realtime.
#[derive(Serialize, Deserialize, Debug, Default, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct ThreadRealtimeStopResponse {}

/// list voices supported by thread realtime.
#[derive(Serialize, Deserialize, Debug, Default, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct ThreadRealtimeListVoicesParams {}

/// response for listing supported realtime voices.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
/// flat transcript delta emitted whenever realtime
...
/// final transcript text emitted when realtime completes
/// a transcript part.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
/// emitted with the remote SDP for a WebRTC realtime session.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
/// emitted when thread realtime encounters an error.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
impl ThreadItem {
    pub fn id(&self) -> &str {
        match self {
            ...
            | ThreadItem::ExitedReviewMode { id, .. } => id,
        }
    }
}
...
impl From<CoreAutoReviewAssessmentDecisionSource> for AutoReviewDecisionSource {
    fn from(value: CoreAutoReviewAssessmentDecisionSource) -> Self {
        match value {
            CoreAutoReviewAssessmentDecisionSource::Agent => Self::Agent,
        }
    }
}
...
impl From<CoreAutoReviewRiskLevel> for AutoReviewRiskLevel {
    fn from(value: CoreAutoReviewRiskLevel) -> Self {
        match value {
            CoreAutoReviewRiskLevel::Low => Self::Low,
            CoreAutoReviewRiskLevel::Medium => Self::Medium,
            CoreAutoReviewRiskLevel::High => Self::High,
            CoreAutoReviewRiskLevel::Critical => Self::Critical,
        }
    }
}
...
impl From<CoreAutoReviewUserAuthorization> for AutoReviewUserAuthorization {
    fn from(value: CoreAutoReviewUserAuthorization) -> Self {
        match value {
            CoreAutoReviewUserAuthorization::Unknown => Self::Unknown,
            CoreAutoReviewUserAuthorization::Low => Self::Low,
            CoreAutoReviewUserAuthorization::Medium => Self::Medium,
            CoreAutoReviewUserAuthorization::High => Self::High,
        }
    }
}
...
impl From<CoreAutoReviewCommandSource> for AutoReviewCommandSource {
    fn from(value: CoreAutoReviewCommandSource) -> Self {
        match value {
            CoreAutoReviewCommandSource::Shell => Self::Shell,
            CoreAutoReviewCommandSource::UnifiedExec => Self::UnifiedExec,
        }
    }
}

impl From<AutoReviewCommandSource> for CoreAutoReviewCommandSource {
    fn from(value: AutoReviewCommandSource) -> Self {
        match value {
            AutoReviewCommandSource::Shell => Self::Shell,
            AutoReviewCommandSource::UnifiedExec => Self::UnifiedExec,
        }
    }
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct AutoReviewCommandReviewAction {
    pub source: AutoReviewCommandSource,
    pub command: String,
    pub cwd: AbsolutePathBuf,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct AutoReviewExecveReviewAction {
    pub source: AutoReviewCommandSource,
    pub program: String,
    pub argv: Vec<String>,
    pub cwd: AbsolutePathBuf,
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct AutoReviewApplyPatchReviewAction {
    pub cwd: AbsolutePathBuf,
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct AutoReviewNetworkAccessReviewAction {
    pub target: String,
    pub host: String,
    pub protocol: NetworkApprovalProtocol,
    pub port: u16,
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct AutoReviewMcpToolCallReviewAction {
    pub server: String,
...
#[ts(export_to = "v2/")]
pub enum AutoApprovalReviewAction {
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    Command {
...
impl From<CoreAutoReviewAssessmentAction> for AutoApprovalReviewAction {
    fn from(value: CoreAutoReviewAssessmentAction) -> Self {
        match value {
            CoreAutoReviewAssessmentAction::Command {
                source,
                command,
                cwd,
            } => Self::Command {
            ...
            CoreAutoReviewAssessmentAction::Execve {
                source,
                program,
                argv,
                cwd,
            ...
            CoreAutoReviewAssessmentAction::ApplyPatch { cwd, files } => {
                Self::ApplyPatch { cwd, files }
            }
            CoreAutoReviewAssessmentAction::NetworkAccess {
                target,
                host,
                protocol,
                port,
            ...
            CoreAutoReviewAssessmentAction::McpToolCall {
                server,
                tool_name,
                connector_id,
                connector_name,
...
impl From<AutoApprovalReviewAction> for CoreAutoReviewAssessmentAction {
    fn from(value: AutoApprovalReviewAction) -> Self {
        match value {
            ...
            AutoApprovalReviewAction::Execve {
                source,
                program,
                argv,
                cwd,
            ...
            AutoApprovalReviewAction::McpToolCall {
                server,
                tool_name,
                connector_id,
                connector_name,
...
impl From<lyra_protocol::models::WebSearchAction> for WebSearchAction {
    fn from(value: lyra_protocol::models::WebSearchAction) -> Self {
        match value {
            lyra_protocol::models::WebSearchAction::Search { query, queries } => {
                WebSearchAction::Search { query, queries }
            }
            lyra_protocol::models::WebSearchAction::OpenPage { url } => {
                WebSearchAction::OpenPage { url }
            }
            lyra_protocol::models::WebSearchAction::FindInPage { url, pattern } => {
                WebSearchAction::FindInPage { url, pattern }
            }
            lyra_protocol::models::WebSearchAction::Other => WebSearchAction::Other,
        }
    }
}
...
impl From<lyra_protocol::items::HookPromptFragment> for HookPromptFragment {
    fn from(value: lyra_protocol::items::HookPromptFragment) -> Self {
        Self {
            text: value.text,
...
pub struct ItemAutoApprovalReviewStartedNotification {
    ...
    pub review: AutoApprovalReview,
    pub action: AutoApprovalReviewAction,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
/// proposed plan streaming deltas for plan items. Clients should
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct MemoryTrimmedNotification {
    pub thread_id: String,
    pub turn_id: String,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct MemorySharedUpdatedNotification {
    pub thread_id: String,
    pub turn_id: String,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct MemoryFrozenUpdatedNotification {
    pub thread_id: String,
    pub turn_id: String,
}
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct MemoryPromptCacheUpdatedNotification {
    pub thread_id: String,
    pub turn_id: String,
}
...
impl McpServerElicitationAction {
    pub fn to_core(self) -> lyra_protocol::approvals::ElicitationAction {
        match self {
            Self::Accept => lyra_protocol::approvals::ElicitationAction::Accept,
            Self::Decline => lyra_protocol::approvals::ElicitationAction::Decline,
            Self::Cancel => lyra_protocol::approvals::ElicitationAction::Cancel,
        }
    }
}
...
pub struct McpServerElicitationRequestParams {
    pub thread_id: String,
    /// Active Lyra turn when this elicitation was observed, if app-server could correlate one.
    ///
...
impl From<DynamicToolCallOutputContentItem>
    for lyra_protocol::dynamic_tools::DynamicToolCallOutputContentItem
{
...
mod tests {
    ...
    use lyra_protocol::items::ReasoningItem;
    use lyra_protocol::items::TurnItem;
    use lyra_protocol::items::UserMessageItem;
    use lyra_protocol::items::WebSearchItem;
    use lyra_protocol::models::WebSearchAction as CoreWebSearchAction;
    use lyra_protocol::protocol::NetworkAccess as CoreNetworkAccess;
    use lyra_protocol::protocol::ReadOnlyAccess as CoreReadOnlyAccess;
    use lyra_protocol::user_input::UserInput as CoreUserInput;
    ...
    fn auto_review_approval_review_action_round_trips_command_shape() {
        ...
        let action: AutoApprovalReviewAction =
            serde_json::from_value(value.clone()).expect("auto_review review action");
        ...
        assert_eq!(
            serde_json::to_value(&action).expect("serialize auto_review review action"),
            value
        );
    }
    ...
    fn network_requirements_deserializes_legacy_fields() {
        ...
        assert_eq!(
            ...
            NetworkRequirements {
                ...
                allowed_domains: Some(vec!["api.example.test".to_string()]),
        ...
        let user_item = TurnItem::UserMessage(UserMessageItem {
            ...
            content: vec![
                ...
                CoreUserInput::Skill {
                    ...
                    path: PathBuf::from("/repo/.lyra/skills/skill-creator/SKILL.md"),
                },
    ...
    fn plugin_read_params_serialization_uses_install_source_fields() {
        ...
        assert_eq!(
            ...
            PluginReadParams {
                ...
                remote_marketplace_name: Some("remote-market".to_string()),
                plugin_name: "gmail".to_string(),
            },
        );
    }
    ...
    fn plugin_uninstall_params_serialization_omits_force_remote_sync() {
        assert_eq!(
            serde_json::to_value(PluginUninstallParams {
                plugin_id: "gmail@remote-market".to_string(),
            })
            .unwrap(),
            json!({
                "pluginId": "gmail@remote-market",
            }),
        );
    ...
    #[test]
    fn lyra_error_info_serializes_http_status_code_in_camel_case() {
        let value = LyraErrorInfo::ResponseTooManyFailedAttempts {
    ...
    #[test]
    fn lyra_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
        let value = LyraErrorInfo::ActiveTurnNotSteerable {
