...
use lyra_sandboxing::SandboxExecRequest;
...
use crate::fs_helper::LYRA_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.lyra_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![LYRA_FS_HELPER_ARG1.to_string()],
            cwd: cwd.clone(),
...
mod tests {
    ...
    fn helper_permissions_strip_network_grants() {
        ...
        let readable = AbsolutePathBuf::from_absolute_path(
            lyra_self_exe.parent().expect("current exe parent"),
        )
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/lyra".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let lyra_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths = ExecServerRuntimePaths::new(lyra_self_exe.clone(), Some(lyra_self_exe))
            .expect("runtime paths");
        let runner = FileSystemSandboxRunner::new(runtime_paths);
