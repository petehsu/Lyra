...
pub const LYRA_EXEC_SERVER_URL_ENV_VAR: &str = "LYRA_EXEC_SERVER_URL";
...
impl EnvironmentManager {
    /// Builds a manager from the raw `LYRA_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self::new_with_runtime_paths(exec_server_url, /*local_runtime_paths*/ None)
    }

    /// Builds a manager from the raw `LYRA_EXEC_SERVER_URL` value and local
    ...
    pub fn from_env_with_runtime_paths(
        ...
        Self::new_with_runtime_paths(
            std::env::var(LYRA_EXEC_SERVER_URL_ENV_VAR).ok(),
            local_runtime_paths,
        )
    }
...
impl Environment {
    /// Builds an environment from the raw `LYRA_EXEC_SERVER_URL` value.
    ...
    /// Builds an environment from the raw `LYRA_EXEC_SERVER_URL` value and
        ...
        let remote_exec_server_client = if let Some(exec_server_url) = &exec_server_url {
            Some(
                ExecServerClient::connect_websocket(RemoteExecServerConnectArgs {
                    ...
                    client_name: "lyra-environment".to_string(),
