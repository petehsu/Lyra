...
pub fn build_tool_registry_plan(
    ...
    if config.code_mode_enabled {
        let namespace_descriptions = params
            ...
            .map(|(namespace, detail)| {
                (
                    namespace.clone(),
                    lyra_code_mode::ToolNamespaceDescription {
        ...
        && (params.deferred_mcp_tools.is_some() || !deferred_dynamic_tools.is_empty())
    {
        ...
        if !deferred_dynamic_tools.is_empty() {
            search_source_infos.push(ToolSearchSourceInfo {
                ...
                description: Some("Tools provided by the current Lyra thread.".to_string()),
            });
        }
    ...
    if config.has_environment && config.supported_tools.iter().any(|tool| tool == "list_dir") {
        plan.push_spec(
...
fn compare_code_mode_tools(
    left: &lyra_code_mode::ToolDefinition,
    right: &lyra_code_mode::ToolDefinition,
...
fn code_mode_namespace_name<'a>(
    tool: &lyra_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, lyra_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
