...
//!
//! # Relationship to `lyra-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `lyra-app-server-client`,
...
use lyra_app_server_protocol::Result;
...
pub use lyra_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
pub struct InProcessStartArgs {
    ...
    /// SQLite tracing layer used by app-server runtime diagnostics.
...
/// through `lyra-app-server-client`, which adds worker-task buffering,
...
fn start_uninitialized(args: InProcessStartArgs) -> InProcessClientHandle {
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let auth_manager =
            AuthManager::shared_from_config(args.config.as_ref(), args.enable_lyra_api_key_env);
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "lyra-in-process-test".to_string(),
                    title: None,
