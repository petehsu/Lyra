...
use lyra_core::INTERACTIVE_SESSION_SOURCES;
use lyra_core::LYRA_DESKTOP_SESSION_SOURCE;
    ...
    if requires_post_filter {
        ...
        let interactive_sources = source_kinds
            ...
            .filter_map(|kind| match kind {
                ...
                ThreadSourceKind::Lyra => Some(CoreSessionSource::Custom(
                    LYRA_DESKTOP_SESSION_SOURCE.to_string(),
                )),
                ThreadSourceKind::Exec
                | ThreadSourceKind::AppServer
                | ThreadSourceKind::SubAgent
...
pub(crate) fn source_kind_matches(source: &CoreSessionSource, filter: &[ThreadSourceKind]) -> bool {
    filter.iter().any(|kind| match kind {
        ...
        ThreadSourceKind::Lyra => matches!(
            source,
            CoreSessionSource::Custom(custom_source)
                if custom_source == LYRA_DESKTOP_SESSION_SOURCE
        ),
...
mod tests {
    ...
    fn compute_source_filters_defaults_to_interactive_sources() {
        ...
        assert!(allowed_sources.contains(&CoreSessionSource::Custom(
            LYRA_DESKTOP_SESSION_SOURCE.to_string()
        )));
    ...
    #[test]
    fn compute_source_filters_can_select_lyra_desktop_source() {
        let source_kinds = vec![ThreadSourceKind::Lyra];
        let (allowed_sources, filter) = compute_source_filters(Some(source_kinds.clone()));

        assert_eq!(
            allowed_sources,
            vec![CoreSessionSource::Custom(
                LYRA_DESKTOP_SESSION_SOURCE.to_string()
            )]
        );
    ...
    fn source_kind_matches_distinguishes_subagent_variants() {
        ...
        assert!(source_kind_matches(
            &CoreSessionSource::Custom(LYRA_DESKTOP_SESSION_SOURCE.to_string()),
            &[ThreadSourceKind::Lyra]
        ));
        assert!(!source_kind_matches(
            &CoreSessionSource::Custom("other".to_string()),
            &[ThreadSourceKind::Lyra]
        ));
    }
}
