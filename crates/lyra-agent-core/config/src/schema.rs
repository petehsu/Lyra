...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == lyra_features::Feature::Artifact {
            continue;
        }
        if feature.id == lyra_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<lyra_features::FeatureToml<
                    lyra_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
