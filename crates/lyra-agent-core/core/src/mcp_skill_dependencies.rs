...
use lyra_config::ConfigEditsBuilder;
use lyra_config::McpServerConfig;
use lyra_config::McpServerTransportConfig;
use lyra_config::load_global_mcp_servers;
...
use lyra_rmcp_client::perform_oauth_login;
...
use lyra_mcp::McpOAuthLoginSupport;
use lyra_mcp::mcp_permission_prompt_is_auto_approved;
use lyra_mcp::oauth_login_support;
use lyra_mcp::resolve_oauth_scopes;
use lyra_mcp::should_retry_without_scopes;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(lyra_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&lyra_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&lyra_home)
