...
use crate::lyra_delegate::run_lyra_thread_interactive;
...
use super::AUTO_REVIEW_TIMEOUT;
use super::AutoReviewApprovalRequest;
use super::prompt::AutoReviewPromptMode;
use super::prompt::AutoReviewTranscriptCursor;
use super::prompt::auto_review_policy_prompt;
use super::prompt::auto_review_policy_prompt_with_config;
use super::prompt::build_auto_review_prompt_items;

const AUTO_REVIEW_INTERRUPT_DRAIN_TIMEOUT: Duration = Duration::from_secs(5);
const AUTO_REVIEW_FOLLOWUP_REVIEW_REMINDER: &str = concat!(
...
#[derive(Default)]
pub(crate) struct AutoReviewSessionManager {
    state: Arc<Mutex<AutoReviewSessionState>>,
}

#[derive(Default)]
struct AutoReviewSessionState {
    trunk: Option<Arc<AutoReviewSession>>,
    ephemeral_reviews: Vec<Arc<AutoReviewSession>>,
}

struct AutoReviewSession {
    session_handle: LyraSessionHandle,
    cancel_token: CancellationToken,
    reuse_key: AutoReviewSessionReuseKey,
    review_lock: Mutex<()>,
    state: Mutex<AutoReviewState>,
}

struct AutoReviewState {
    prior_review_count: usize,
    last_reviewed_transcript_cursor: Option<AutoReviewTranscriptCursor>,
    last_committed_fork_snapshot: Option<AutoReviewForkSnapshot>,
}

struct EphemeralReviewCleanup {
    state: Arc<Mutex<AutoReviewSessionState>>,
    review_session: Option<Arc<AutoReviewSession>>,
}

#[derive(Clone)]
struct AutoReviewForkSnapshot {
...
struct AutoReviewSessionReuseKey {
    ...
    use_unified_exec_tool: bool,
}

impl AutoReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            lyra_linux_sandbox_exe: spawn_config.lyra_linux_sandbox_exe.clone(),
            ...
            use_unified_exec_tool: spawn_config.use_unified_exec_tool,
        }
    }
}

impl AutoReviewSession {
    async fn shutdown(&self) {
        self.cancel_token.cancel();
        let _ = self.session_handle.shutdown_and_wait().await;
    }
    ...
    async fn fork_snapshot(&self) -> Option<AutoReviewForkSnapshot> {
        self.state.lock().await.last_committed_fork_snapshot.clone()
    }

    async fn refresh_last_committed_fork_snapshot(&self) {
        match load_rollout_items_for_fork(&self.session_handle.session).await {
            Ok(Some(items)) if !items.is_empty() => {
                ...
                state.last_committed_fork_snapshot = Some(AutoReviewForkSnapshot {
            ...
            Ok(None) => {}
            Err(err) => {
                warn!("failed to refresh auto_review trunk rollout snapshot: {err}");
            }
        }
    }
}
...
impl EphemeralReviewCleanup {
    fn new(
        state: Arc<Mutex<AutoReviewSessionState>>,
        review_session: Arc<AutoReviewSession>,
    ) -> Self {
        Self {
            state,
...
impl AutoReviewSessionManager {
    ...
    pub(crate) async fn run_review(
        &self,
        params: AutoReviewSessionParams,
    ) -> AutoReviewSessionOutcome {
        let deadline = tokio::time::Instant::now() + AUTO_REVIEW_TIMEOUT;
        let next_reuse_key = AutoReviewSessionReuseKey::from_spawn_config(&params.spawn_config);
            ...
            Ok(mut state) => {
                ...
                if state.trunk.is_none() {
                    ...
                    let review_session = match run_before_review_deadline_with_cancel(
                        ...
                        params.external_cancel.as_ref(),
                        &spawn_cancel_token,
                        Box::pin(spawn_auto_review_session(
                            &params,
                    ...
                    .await
                    {
                        ...
                        Ok(Err(err)) => {
                            return AutoReviewSessionOutcome::Completed(Err(err));
                        }
        ...
        let Some(trunk) = trunk_candidate else {
            return AutoReviewSessionOutcome::Completed(Err(anyhow!(
                "auto_review review session was not available after spawn"
            )));
        };
        ...
        if keep_review_session && matches!(outcome, AutoReviewSessionOutcome::Completed(_)) {
    ...
    async fn register_active_ephemeral(&self, review_session: Arc<AutoReviewSession>) {
        self.state
            .lock()
            .await
            .ephemeral_reviews
    ...
    async fn run_ephemeral_review(
        ...
        fork_snapshot: Option<AutoReviewForkSnapshot>,
    ) -> AutoReviewSessionOutcome {
        let spawn_cancel_token = CancellationToken::new();
        ...
        {
            Ok(Ok(review_session)) => Arc::new(review_session),
            Ok(Err(err)) => return AutoReviewSessionOutcome::Completed(Err(err)),
            Err(outcome) => return outcome,
        };
...
async fn spawn_auto_review_session(
    params: &AutoReviewSessionParams,
    spawn_config: Config,
    reuse_key: AutoReviewSessionReuseKey,
    cancel_token: CancellationToken,
    fork_snapshot: Option<AutoReviewForkSnapshot>,
) -> anyhow::Result<AutoReviewSession> {
    ...
    let session_handle = Box::pin(run_lyra_thread_interactive(
        spawn_config,
        ...
        cancel_token.clone(),
        SubAgentSource::Other(AUTO_REVIEW_AGENT_NAME.to_string()),
        initial_history,
    ))
    .await?;

    Ok(AutoReviewSession {
        session_handle,
        cancel_token,
        reuse_key,
...
async fn run_review_on_session(
    review_session: &AutoReviewSession,
    params: &AutoReviewSessionParams,
    deadline: tokio::time::Instant,
) -> (AutoReviewSessionOutcome, bool) {
    ...
    if send_followup_reminder {
        append_auto_review_followup_reminder(review_session).await;
    }
    ...
    let submit_result = run_before_review_deadline(
        ...
        Box::pin(async {
            ...
            let prompt_items = build_auto_review_prompt_items(
            ...
            Ok::<AutoReviewTranscriptCursor, anyhow::Error>(prompt_items.transcript_cursor)
        }),
    )
    .await;
    ...
    let transcript_cursor = match submit_result {
        ...
        Err(err) => {
            return (AutoReviewSessionOutcome::Completed(Err(err)), false);
        }
    };

    let outcome =
        wait_for_auto_review(review_session, deadline, params.external_cancel.as_ref()).await;
    if matches!(outcome.0, AutoReviewSessionOutcome::Completed(_)) {
...
async fn append_auto_review_followup_reminder(review_session: &AutoReviewSession) {
    let turn_context = review_session
        .session_handle
        .session
        .new_default_turn()
        .await;
    let reminder: ResponseItem =
        DeveloperInstructions::new(AUTO_REVIEW_FOLLOWUP_REVIEW_REMINDER).into();
    review_session
        .session_handle
        .session
...
async fn wait_for_auto_review(
    ...
    loop {
        tokio::select! {
            _ = &mut timeout => {
                let keep_review_session = interrupt_and_drain_turn(&review_session.session_handle).await.is_ok();
                return (AutoReviewSessionOutcome::TimedOut, keep_review_session);
            }
            _ = async {
                ...
                let keep_review_session = interrupt_and_drain_turn(&review_session.session_handle).await.is_ok();
                return (AutoReviewSessionOutcome::Aborted, keep_review_session);
            }
            event = review_session.session_handle.next_event() => {
                match event {
                    Ok(event) => match event.msg {
                        EventMsg::TurnComplete(turn_complete) => {
                            ...
                            {
                                return (
                                    AutoReviewSessionOutcome::Completed(Err(anyhow!(error_message))),
                                    true,
                                );
                            }
                            return (
                                AutoReviewSessionOutcome::Completed(Ok(turn_complete.last_agent_message)),
                                true,
                            );
                        }
                        ...
                        EventMsg::TurnAborted(_) => {
                            return (AutoReviewSessionOutcome::Aborted, true);
                        }
                        _ => {}
                    },
                    Err(err) => {
                        return (
                            AutoReviewSessionOutcome::Completed(Err(err.into())),
                            false,
                        );
                    }
                }
...
pub(crate) fn build_auto_review_session_config(
    parent_config: &Config,
    live_network_config: Option<lyra_network_proxy::NetworkProxyConfig>,
    active_model: &str,
    reasoning_effort: Option<lyra_protocol::openai_models::ReasoningEffort>,
) -> anyhow::Result<Config> {
    let mut auto_review_config = parent_config.clone();
    auto_review_config.model = Some(active_model.to_string());
    auto_review_config.model_reasoning_effort = reasoning_effort;
    auto_review_config.include_skill_instructions = false;
    auto_review_config.developer_instructions = Some(
        parent_config
            .auto_review_policy_config
            .as_deref()
            .map(auto_review_policy_prompt_with_config)
            .unwrap_or_else(auto_review_policy_prompt),
    );
    auto_review_config.permissions.approval_policy = Constrained::allow_only(AskForApproval::Never);
    auto_review_config.permissions.sandbox_policy =
        Constrained::allow_only(SandboxPolicy::new_read_only_policy());
    if let Some(live_network_config) = live_network_config
        && auto_review_config.permissions.network.is_some()
    {
        let network_constraints = auto_review_config
            .config_layer_stack
            .requirements()
            .network
            .as_ref()
            .map(|network| network.value.clone());
        auto_review_config.permissions.network =
    ...
    for feature in [
        ...
        auto_review_config
            ...
            .map_err(|err| {
                anyhow::anyhow!(
                    "auto_review review session could not disable `features.{}`: {err}",
                    feature.key()
                )
            })?;
        if auto_review_config.features.enabled(feature) {
            anyhow::bail!(
                "auto_review review session requires `features.{}` to be disabled",
                feature.key()
            );
        }
    }
    Ok(auto_review_config)
}
...
) -> Result<T, AutoReviewSessionOutcome> {
    tokio::select! {
        _ = tokio::time::sleep_until(deadline) => Err(AutoReviewSessionOutcome::TimedOut),
        ...
        } => Err(AutoReviewSessionOutcome::Aborted),
    }
}
...
async fn interrupt_and_drain_turn(session_handle: &LyraSessionHandle) -> anyhow::Result<()> {
    let _ = session_handle.submit(Op::Interrupt).await;

    tokio::time::timeout(AUTO_REVIEW_INTERRUPT_DRAIN_TIMEOUT, async {
        loop {
    ...
    .await
    .map_err(|_| anyhow!("timed out draining auto_review review session after interrupt"))??;
...
mod tests {
    ...
    #[tokio::test]
    async fn auto_review_session_config_change_invalidates_cached_session() {
        let parent_config = crate::config::test_config().await;
        let cached_spawn_config = build_auto_review_session_config(
            &parent_config,
            /*live_network_config*/ None,
            "active-model",
            /*reasoning_effort*/ None,
        )
        .expect("cached auto_review config");
        let cached_reuse_key = AutoReviewSessionReuseKey::from_spawn_config(&cached_spawn_config);

        let mut changed_parent_config = parent_config;
        changed_parent_config.model_provider.base_url =
            Some("https://auto_review.example.invalid/v1".to_string());
        let next_spawn_config = build_auto_review_session_config(
            &changed_parent_config,
            /*live_network_config*/ None,
            "active-model",
            /*reasoning_effort*/ None,
        )
        .expect("next auto_review config");
        let next_reuse_key = AutoReviewSessionReuseKey::from_spawn_config(&next_spawn_config);

        assert_ne!(cached_reuse_key, next_reuse_key);
        assert_eq!(
            cached_reuse_key,
            AutoReviewSessionReuseKey::from_spawn_config(&cached_spawn_config)
        );
    }

    #[tokio::test]
    async fn auto_review_session_config_disables_hooks() {
        let mut parent_config = crate::config::test_config().await;
        parent_config
            .features
            .enable(Feature::LyraHooks)
        ...
        .expect("auto_review config");

        assert!(!auto_review_config.features.enabled(Feature::LyraHooks));
    }

    #[tokio::test]
    async fn auto_review_session_config_disables_skill_instructions() {
        let mut parent_config = crate::config::test_config().await;
        ...
        assert!(!auto_review_config.include_skill_instructions);
    }
    ...
    async fn run_before_review_deadline_times_out_before_future_completes() {
        ...
        assert!(matches!(outcome, Err(AutoReviewSessionOutcome::TimedOut)));
    }
    ...
    async fn run_before_review_deadline_aborts_when_cancelled() {
        ...
        assert!(matches!(outcome, Err(AutoReviewSessionOutcome::Aborted)));
    }
