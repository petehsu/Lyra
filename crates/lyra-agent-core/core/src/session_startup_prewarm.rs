...
use lyra_otel::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use lyra_otel::STARTUP_PREWARM_DURATION_METRIC;
...
pub(crate) struct SessionStartupPrewarmHandle {
    task: JoinHandle<LyraResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<LyraResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
async fn schedule_startup_prewarm_inner(
    ...
    client_session
        .prewarm_websocket(
            ...
            startup_turn_context.config.model_verbosity,
