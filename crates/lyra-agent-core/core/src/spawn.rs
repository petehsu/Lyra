...
pub const LYRA_SANDBOX_ENV_VAR: &str = "LYRA_SANDBOX";
...
/// `LYRA_SANDBOX_NETWORK_DISABLED_ENV_VAR` environment variable.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(LYRA_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }

    // If this Lyra process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                lyra_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            Ok(())
        });
    }
