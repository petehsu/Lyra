...
use lyra_core_plugins::marketplace::MarketplacePluginInstallPolicy;
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "lyra-test@example.com"]);
    run_git(repo, &["config", "user.name", "Lyra Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(config_toml: &str, lyra_home: &Path) -> PluginLoadOutcome {
    write_file(&lyra_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(lyra_home, lyra_home).await;
    PluginsManager::new(lyra_home.to_path_buf())
        .plugins_for_config(&config)
        .await
}

async fn load_config(lyra_home: &Path, cwd: &Path) -> crate::config::Config {
...
async fn load_plugins_loads_default_skills_and_mcp_servers() {
    let lyra_home = TempDir::new().unwrap();
...
enabled = true
"#;
    let outcome = load_plugins_from_config(config_toml, lyra_home.path()).await;
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let lyra_home = TempDir::new().unwrap();
    let plugin_a_root = lyra_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = lyra_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".lyra-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".lyra-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(lyra_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![lyra_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![lyra_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    write_file(
        &lyra_home.path().join(CONFIG_TOML_FILE),
        &plugin_config_toml(
            /*enabled*/ true, /*plugins_feature_enabled*/ false,
        ),
    );

    let config = load_config(lyra_home.path(), lyra_home.path()).await;
    let outcome = PluginsManager::new(lyra_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".lyra-plugin/plugin.json").is_file());
}
...
"#,
    ...
    write_file(
        &repo_root.join(".lyra/config.toml"),
        r#"[[skills.config]]
...
async fn read_plugin_for_config_installed_git_source_reads_from_cache_without_cloning() {
  ...
  "plugins": [
    ...
    write_file(
        &cached_plugin_root.join(".lyra-plugin/plugin.json"),
        r#"{
  "name": "toolkit",
...
async fn load_plugins_ignores_project_config_files() {
    let lyra_home = TempDir::new().unwrap();
    let project_root = lyra_home.path().join("project");
    let plugin_root = lyra_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".lyra/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_lyra_folder: AbsolutePathBuf::try_from(project_root.join(".lyra")).unwrap(),
            },
    ...
    let outcome = load_plugins_from_layer_stack(
        &stack,
        &PluginStore::new(lyra_home.path().to_path_buf()),
        Some(Product::Lyra),
    )
    .await;
