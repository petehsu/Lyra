...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .lyra_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = lyra_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        lyra_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
fn schema_error_points_to_feature_value() {
    ...
    let _guard = lyra_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = lyra_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when LYRA_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let lyra_home = tmp.path().join("home");
    ...
    let err = load_config_layers_state(
        ...
        CloudRequirementsLoader::new(async {
            Err(CloudRequirementsLoadError::new(
                lyra_config::CloudRequirementsLoadErrorCode::RequestFailed,
                /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".lyra")).await?;
    tokio::fs::create_dir_all(project_root.join(".lyra")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".lyra").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".lyra").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            super::ConfigLayerSource::Project { dot_lyra_folder } => Some(dot_lyra_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".lyra").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".lyra").as_path()
    );
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_lyra_and_override_in_order() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    tokio::fs::create_dir_all(project_root.join(".lyra")).await?;
...
"#;
    tokio::fs::write(project_root.join(".lyra").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".lyra").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".lyra").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(nested.join(".lyra").join("child.txt"), "child instructions").await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let lyra_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&lyra_home).await?;
    tokio::fs::write(lyra_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_lyra_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_lyra_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".lyra"))?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            raw_toml: None,
            version: version_for_toml(&TomlValue::Table(toml::map::Map::new())),
...
#[tokio::test]
async fn lyra_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let lyra_home = home_dir.join(".lyra");
    tokio::fs::create_dir_all(&lyra_home).await?;
    tokio::fs::write(lyra_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn lyra_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_lyra = project_root.join(".lyra");
    let nested_dot_lyra = nested.join(".lyra");

    tokio::fs::create_dir_all(&nested_dot_lyra).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(nested_dot_lyra.join(CONFIG_TOML_FILE), "foo = \"child\"\n").await?;

    tokio::fs::create_dir_all(&project_dot_lyra).await?;
    make_config_for_test(
        &project_dot_lyra,
        &project_root,
        TrustLevel::Trusted,
    ...
    .await?;
    let user_config_path = project_dot_lyra.join(CONFIG_TOML_FILE);
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_lyra_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_lyra)?,
            },
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let lyra_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&lyra_home_untrusted).await?;
    make_config_for_test(
        &lyra_home_untrusted,
        &project_root,
    ...
    .await?;
    let untrusted_config_path = lyra_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    let lyra_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&lyra_home_unknown).await?;
    tokio::fs::write(lyra_home_unknown.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
-> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let dot_lyra = project_root.join(".lyra");
    let lyra_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&nested).await?;
    tokio::fs::create_dir_all(&dot_lyra).await?;
    tokio::fs::create_dir_all(&lyra_home).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(
        dot_lyra.join(CONFIG_TOML_FILE),
        r#"
[mcp_servers.sentry]
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".lyra")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".lyra").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let lyra_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&lyra_home).await?;
        let config_path = lyra_home.join(CONFIG_TOML_FILE);
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let lyra_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&lyra_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &lyra_home,
...
mod requirements_exec_policy_tests {
    ...
    use lyra_config::RequirementsExecPolicyDecisionToml;
    use lyra_config::RequirementsExecPolicyParseError;
    use lyra_config::RequirementsExecPolicyPatternTokenToml;
    use lyra_config::RequirementsExecPolicyPrefixRuleToml;
    use lyra_config::RequirementsExecPolicyToml;
    use lyra_execpolicy::Decision;
    use lyra_execpolicy::Evaluation;
    use lyra_execpolicy::RuleMatch;
    use lyra_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_lyra_folder_with_requirements(
        dot_lyra_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_lyra_folder =
            AbsolutePathBuf::from_absolute_path(dot_lyra_folder).expect("absolute dot_lyra_folder");
        let layer = ConfigLayerEntry::new(
            ConfigLayerSource::Project { dot_lyra_folder },
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_lyra_folder_with_requirements(temp_dir.path(), requirements);
