...
use lyra_app_server_protocol::ConfigLayerSource;
use lyra_config::CONFIG_TOML_FILE;
use lyra_config::ConfigRequirementsWithSources;
...
pub use lyra_config::AppRequirementToml;
pub use lyra_config::AppsRequirementsToml;
pub use lyra_config::CloudRequirementsLoadError;
pub use lyra_config::CloudRequirementsLoadErrorCode;
pub use lyra_config::CloudRequirementsLoader;
pub use lyra_config::ConfigError;
pub use lyra_config::ConfigLayerEntry;
pub use lyra_config::ConfigLayerStack;
pub use lyra_config::ConfigLayerStackOrdering;
pub use lyra_config::ConfigLoadError;
pub use lyra_config::ConfigRequirements;
pub use lyra_config::ConfigRequirementsToml;
pub use lyra_config::ConstrainedWithSource;
pub use lyra_config::FeatureRequirementsToml;
pub use lyra_config::FilesystemConstraints;
pub use lyra_config::FilesystemDenyReadPattern;
pub use lyra_config::LoaderOverrides;
pub use lyra_config::McpServerIdentity;
pub use lyra_config::McpServerRequirement;
pub use lyra_config::NetworkConstraints;
pub use lyra_config::NetworkDomainPermissionToml;
pub use lyra_config::NetworkDomainPermissionsToml;
pub use lyra_config::NetworkRequirementsToml;
pub use lyra_config::NetworkUnixSocketPermissionToml;
pub use lyra_config::NetworkUnixSocketPermissionsToml;
pub use lyra_config::RequirementSource;
pub use lyra_config::ResidencyRequirement;
pub use lyra_config::SandboxModeRequirement;
pub use lyra_config::Sourced;
pub use lyra_config::TextPosition;
pub use lyra_config::TextRange;
pub use lyra_config::WebSearchModeRequirement;
pub(crate) use lyra_config::build_cli_overrides_layer;
pub(crate) use lyra_config::config_error_from_toml;
pub use lyra_config::default_project_root_markers;
pub use lyra_config::format_config_error;
pub use lyra_config::format_config_error_with_source;
pub(crate) use lyra_config::io_error_from_config_error;
pub use lyra_config::merge_toml_values;
pub use lyra_config::project_root_markers_from_config;
#[cfg(test)]
pub(crate) use lyra_config::version_for_toml;

/// On Unix systems, load default settings from this file path, if present.
/// Note that /etc/lyra/ is treated as a "config folder," so subfolders such
/// as skills/ and rules/ will also be honored.
pub const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/lyra/config.toml";

#[cfg(windows)]
const DEFAULT_PROGRAM_DATA_DIR_WINDOWS: &str = r"C:\ProgramData";

pub(crate) async fn first_layer_config_error(layers: &ConfigLayerStack) -> Option<ConfigError> {
    lyra_config::first_layer_config_error::<ConfigToml>(layers, CONFIG_TOML_FILE).await
}

pub(crate) async fn first_layer_config_error_from_entries(
    layers: &[ConfigLayerEntry],
) -> Option<ConfigError> {
    lyra_config::first_layer_config_error_from_entries::<ConfigToml>(layers, CONFIG_TOML_FILE).await
}
...
/// - system    `/etc/lyra/requirements.toml` (Unix) or
///   `%ProgramData%\Lyra\requirements.toml` (Windows)
///
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/lyra/config.toml` (Unix) or
///   `%ProgramData%\Lyra\config.toml` (Windows)
/// - user      `${LYRA_HOME}/config.toml`
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.lyra/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.lyra/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See project security documentation for details.
///
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, lyra_home, overrides).await?;
    ...
    // Add a layer for $LYRA_HOME/config.toml if it exists. Note if the file
    // exists, but is malformed, then this error should be propagated to the
    // user.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, lyra_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        // value for base_dir, so lyra_home is as good a value as any.
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, lyra_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/lyra/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_lyra_system_dir() -> PathBuf {
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_lyra_folder: dot_lyra_folder.clone(),
    };
...
async fn load_project_layers(
    ...
    let lyra_home_abs = AbsolutePathBuf::from_absolute_path(lyra_home)?;
    let lyra_home_normalized =
        normalize_path(lyra_home_abs.as_path()).unwrap_or_else(|_| lyra_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_lyra_abs = dir.join(".lyra");
        if !fs
            .get_metadata(&dot_lyra_abs, /*sandbox*/ None)
            .await
        ...
        let dot_lyra_normalized =
            normalize_path(dot_lyra_abs.as_path()).unwrap_or_else(|_| dot_lyra_abs.to_path_buf());
        if dot_lyra_abs == lyra_home_abs || dot_lyra_normalized == lyra_home_normalized {
            continue;
        }
        let config_file = dot_lyra_abs.join(CONFIG_TOML_FILE);
        match fs.read_file_text(&config_file, /*sandbox*/ None).await {
            Ok(contents) => {
                ...
                let config = resolve_relative_paths_in_config_toml(config, dot_lyra_abs.as_path())?;
                let entry = project_layer_entry(&dot_lyra_abs, config, disabled_reason.clone());
                layers.push(entry);
            }
            Err(err) => {
...
/// from a file like `/etc/lyra/managed_config.toml` that has the same
...
/// only the specified value. `approvals_reviewer = "auto_review"` also
/// allows `user` so people can opt out of the auto reviewer.
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for Lyra to function correctly.
...
"#;
    ...
    #[test]
    fn legacy_managed_config_backfill_allows_user_when_auto_review_is_required() {
        let legacy = LegacyManagedConfigToml {
            approval_policy: None,
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("Lyra").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("Lyra").join("config.toml"))
        );
    }
}
