...
use lyra_app_server_protocol::McpElicitationObjectType;
use lyra_app_server_protocol::McpElicitationSchema;
...
use lyra_config::types::AppToolApproval;
use lyra_features::Feature;
use lyra_mcp::SandboxState;
use lyra_mcp::mcp_permission_prompt_is_auto_approved;
...
use lyra_protocol::request_user_input::RequestUserInputAnswer;
use lyra_protocol::request_user_input::RequestUserInputArgs;
use lyra_protocol::request_user_input::RequestUserInputQuestion;
use lyra_protocol::request_user_input::RequestUserInputQuestionOption;
...
const MCP_CALL_COUNT_METRIC: &str = "lyra.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "lyra.mcp.call.duration_ms";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let approval_mode = custom_mcp_tool_approval_mode(turn_context.as_ref(), &server, &tool_name);
    let request_meta =
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                lyra_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn custom_mcp_tool_approval_mode(
    ...
    turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, lyra_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    if let Some(turn_metadata) = turn_context.turn_metadata_state.current_meta_value() {
        request_meta.insert(
            crate::X_LYRA_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }
...
const MCP_TOOL_APPROVAL_KIND_KEY: &str = "lyra_approval_kind";
...
async fn maybe_request_mcp_tool_approval(
    ...
    question.question = mcp_tool_approval_question_text(question.question);
...
fn mcp_tool_approval_question_text(question: String) -> String {
    question
}
...
async fn persist_custom_mcp_tool_approval(
        ...
        project_mcp_tool_approval_config_folder(config, server)
    {
        ...
        let servers = load_global_mcp_servers(&config.lyra_home).await?;
        if !servers.contains_key(server) {
            anyhow::bail!("MCP server `{server}` is not configured in config.toml");
        }
        config.lyra_home.clone()
    };
