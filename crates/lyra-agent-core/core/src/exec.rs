...
use lyra_protocol::permissions::FileSystemSandboxKind;
...
use lyra_protocol::protocol::ExecCommandOutputDeltaEvent;
use lyra_protocol::protocol::ExecOutputStream;
use lyra_protocol::protocol::SandboxPolicy;
use lyra_sandboxing::SandboxCommand;
use lyra_sandboxing::SandboxManager;
use lyra_sandboxing::SandboxTransformRequest;
use lyra_sandboxing::SandboxType;
use lyra_sandboxing::SandboxablePreference;
use lyra_utils_absolute_path::AbsolutePathBuf;
use lyra_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use lyra_utils_pty::process_group::kill_child_process_group;
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: lyra_protocol::config_types::WindowsSandboxLevel,
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            lyra_linux_sandbox_exe: lyra_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
        .map(|request| ExecRequest::from_sandbox_exec_request(request, options))
        .map_err(LyraErr::from)?;
    ...
    .map_err(LyraErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        lyra_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    ...
    if let Some(metrics) = lyra_otel::global() {
        let _ = metrics.counter(
            "lyra.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_lyra_home;
    use lyra_windows_sandbox::run_windows_sandbox_capture_elevated;
    use lyra_windows_sandbox::run_windows_sandbox_capture_with_extra_deny_write_paths;
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        LyraErr::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let lyra_home = find_lyra_home().map_err(|err| {
        LyraErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve lyra_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_elevated(
                lyra_windows_sandbox::ElevatedSandboxCaptureRequest {
                    policy_json_or_preset: policy_str.as_str(),
                    sandbox_policy_cwd: &sandbox_cwd,
                    lyra_home: lyra_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(LyraErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, LyraErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(LyraErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(LyraErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(LyraErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// LYRA_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
...
pub(crate) fn resolve_windows_elevated_filesystem_overrides(
    ...
    let legacy_readable_root_set: BTreeSet<PathBuf> = sandbox_policy
        ...
        .into_iter()
        .map(lyra_utils_absolute_path::AbsolutePathBuf::into_path_buf)
        .map(&normalize_path)
        .collect();
    let legacy_root_paths: BTreeSet<PathBuf> = legacy_writable_roots
        .iter()
...
fn has_reopened_writable_descendant(
    writable_roots: &[lyra_protocol::protocol::WritableRoot],
) -> bool {
