...
use lyra_analytics::TurnTokenUsageFact;
use lyra_login::AuthManager;
use lyra_models_manager::manager::ModelsManager;
use lyra_otel::SessionTelemetry;
use lyra_otel::TURN_E2E_DURATION_METRIC;
use lyra_otel::TURN_NETWORK_PROXY_METRIC;
use lyra_otel::TURN_TOKEN_USAGE_METRIC;
use lyra_otel::TURN_TOOL_CALL_METRIC;
...
///
/// Implementations encapsulate a specific Lyra workflow (regular chat,
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Lyra will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
