...
use crate::lyra_delegate::run_lyra_thread_one_shot;
...
impl SessionTask for ReviewTask {
    ...
    async fn run(
        ...
        session
            ...
            .services
            .session_telemetry
            .counter("lyra.task.review", /*inc*/ 1, &[]);

        // Start sub-Lyra conversation and get the receiver for events.
...
async fn process_review_events(
    ...
    while let Ok(event) = receiver.recv().await {
        match event.clone().msg {
            ...
            // Suppress assistant message completions in review mode; review
            // output is surfaced through the structured ReviewOutput item.
