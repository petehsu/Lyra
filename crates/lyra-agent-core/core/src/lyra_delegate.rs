...
/// Start an interactive delegated Lyra thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_lyra_thread_interactive(
    config: Config,
...
) -> Result<LyraSessionHandle, LyraErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);

    let LyraSessionSpawnOk { session_handle, .. } =
        Box::pin(LyraSessionHandle::spawn(LyraSessionSpawnArgs {
            config,
            auth_manager,
            models_manager,
            ...
            parent_rollout_thread_trace: parent_session.services.rollout_thread_trace.clone(),
            parent_trace: None,
            analytics_events_client: Some(parent_session.services.analytics_events_client.clone()),
        }))
        .await?;
    if parent_session.enabled(lyra_features::Feature::GeneralAnalytics) {
        let thread_config = session_handle.thread_config_snapshot().await;
        let client_metadata = parent_session.app_server_client_metadata().await;
        emit_subagent_session_started(
            &parent_session.services.analytics_events_client,
            client_metadata,
            session_handle.session.conversation_id,
            Some(parent_session.conversation_id),
            thread_config,
            subagent_source,
        );
    }
    let session_handle = Arc::new(session_handle);
    ...
    let lyra_for_ops = Arc::clone(&session_handle);
    tokio::spawn(async move {
        forward_ops(lyra_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(LyraSessionHandle {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: session_handle.agent_status.clone(),
        session: Arc::clone(&session_handle.session),
        session_loop_termination: session_handle.session_loop_termination.clone(),
    })
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_lyra_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: Arc<ModelsManager>,
    input: Vec<UserInput>,
    ...
    let io = Box::pin(run_lyra_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    session_handle: Arc<LyraSessionHandle>,
    tx_sub: Sender<Event>,
    parent_session: Arc<Session>,
    parent_ctx: Arc<TurnContext>,
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&session_handle).await;
                break;
            }
            event = session_handle.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&session_handle, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(session_handle: &LyraSessionHandle) {
    let _ = session_handle.submit(Op::Interrupt).await;
    let _ = session_handle.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = session_handle.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = session_handle.submit_with_id(submission).await;
    }
}
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                lyra_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                lyra_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                lyra_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            lyra_protocol::request_user_input::RequestUserInputAnswer {
...
) -> lyra_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = lyra_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, lyra_protocol::protocol::ReviewDecision::Abort)
                .await;
            lyra_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
