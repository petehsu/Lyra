...
use lyra_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    async fn new() -> Self {
        ...
        let manager = ThreadManager::with_models_provider_and_home_for_tests(
            LyraAuth::from_api_key("dummy"),
            config.model_provider.clone(),
            config.lyra_home.to_path_buf(),
            std::sync::Arc::new(lyra_exec_server::EnvironmentManager::new(
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<LyraThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<LyraThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<LyraThread>, message: &str) {
    thread
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, LyraErr::ThreadNotFound(id) if id == thread_id);
}
...
async fn send_inter_agent_communication_without_turn_queues_message_without_triggering_turn() {
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            if thread.session_handle.session.has_pending_input().await {
                break;
            }
...
async fn spawn_agent_can_fork_parent_thread_history_with_sanitized_items() {
    ...
    let history = child_thread.session_handle.session.clone_history().await;
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let LyraErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected LyraErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let LyraErr::AgentLimitReached { max_threads } = err else {
        panic!("expected LyraErr::AgentLimitReached");
    };
...
async fn resume_agent_from_rollout_reads_archived_rollout_path() {
    ...
    let store = LocalThreadStore::new(lyra_rollout::RolloutConfig::from_view(&harness.config));
    store
