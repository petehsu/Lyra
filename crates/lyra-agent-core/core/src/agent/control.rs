...
fn keep_forked_rollout_item(item: &RolloutItem) -> bool {
    match item {
        ...
        RolloutItem::EventMsg(_) | RolloutItem::SessionMeta(_) | RolloutItem::TurnContext(_) => {
            true
        }
    }
}
...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> LyraThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    async fn spawn_forked_thread(
    ...
    ) -> LyraResult<crate::thread_manager::NewThread> {
        if options.fork_parent_spawn_call_id.is_none() {
            return Err(LyraErr::Fatal(
        ...
        if let Some(parent_thread) = parent_thread.as_ref() {
            ...
            parent_thread
                ...
                .await;
            parent_thread.session_handle.session.flush_rollout().await?;
        }
        ...
        let rollout_path = parent_thread
            ...
            .or(find_thread_path_by_id_str(
                config.lyra_home.as_path(),
                &parent_thread_id.to_string(),
            )
            .await?)
            .ok_or_else(|| {
                LyraErr::Fatal(format!(
    ...
    async fn resume_single_agent_from_rollout(
        ...
        let inherited_exec_policy = self
            ...
            .await;
        let rollout_path =
            match find_thread_path_by_id_str(config.lyra_home.as_path(), &thread_id.to_string())
                .await?
            {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> LyraResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: LyraResult<String>,
    ) -> LyraResult<String> {
        if matches!(result, Err(LyraErr::InternalAgentDied)) {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> LyraResult<String> {
        let state = self.upgrade()?;
        let result = if let Ok(thread) = state.get_thread(agent_id).await {
            thread
                ...
                .ensure_rollout_materialized()
                .await;
            thread.session_handle.session.flush_rollout().await?;
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> LyraResult<String> {
        let state = self.upgrade()?;
    ...
    async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> LyraResult<String> {
        let descendant_ids = self.live_thread_spawn_descendants(agent_id).await?;
        let result = self.shutdown_live_agent(agent_id).await;
        for descendant_id in descendant_ids {
            match self.shutdown_live_agent(descendant_id).await {
                Ok(_) | Err(LyraErr::ThreadNotFound(_)) | Err(LyraErr::InternalAgentDied) => {}
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(LyraErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> LyraResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> LyraResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> LyraResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| LyraErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn inherited_exec_policy_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        let parent_config = parent_thread.session_handle.session.get_config().await;
        if !crate::exec_policy::child_uses_parent_exec_policy(&parent_config, child_config) {
            return None;
        }

        Some(Arc::clone(
            &parent_thread.session_handle.session.services.exec_policy,
        ))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> LyraResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> LyraResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
