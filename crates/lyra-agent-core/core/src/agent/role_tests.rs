...
async fn apply_explorer_role_sets_model_and_adds_session_flags_layer() {
    ...
    assert_eq!(config.model.as_deref(), Some("gpt-5.1-lyra-mini"));
...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.lyra_linux_sandbox_exe = Some(PathBuf::from("/tmp/lyra-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/lyra-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(config.model_reasoning_effort, Some(ReasoningEffort::High));
    assert_eq!(
        config.lyra_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/lyra-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/lyra-execve-wrapper"))
    );
}
...
"#,
    ...
    let outcome = skills_manager
        .skills_for_config(&skills_input, Some(Arc::clone(&lyra_exec_server::LOCAL_FS)))
        .await;
    let skill = outcome
        .skills
        .iter()
