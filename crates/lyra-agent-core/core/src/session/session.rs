...
pub(crate) struct SessionConfiguration {
    ...
    /// Directory containing all Lyra runtime state for this session.
    pub(super) lyra_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    pub(crate) fn lyra_home(&self) -> &AbsolutePathBuf {
        &self.lyra_home
    }
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        let window_generation = match &initial_history {
            InitialHistory::Resumed(resumed_history) => {
                u64::try_from(resumed_history.history.iter().count()).unwrap_or(u64::MAX)
            }
        ...
        let trace_agent_path = session_configuration
            .session_source
            .get_agent_path()
            .unwrap_or_else(lyra_protocol::AgentPath::root);
        let trace_task_name =
        ...
        let auth_mode = auth.map(LyraAuth::auth_mode).map(TelemetryAuthMode::from);
        let account_id = None;
        ...
        let auth_env_telemetry = collect_auth_env_telemetry(
            &session_configuration.provider,
            auth_manager.lyra_api_key_env_enabled(),
        );
        ...
        let thread_name =
            thread_title_from_state_db(state_db_ctx.as_ref(), &config.lyra_home, conversation_id)
                .instrument(info_span!(
        ...
        let installation_id = resolve_installation_id(&config.lyra_home).await?;
        ...
        let services = SessionServices {
            ...
            _thread_store: LocalThreadStore::new(RolloutConfig::from_view(config.as_ref())),
        ...
        let session_start_source = match &initial_history {
            InitialHistory::Resumed(_) => lyra_hooks::SessionStartSource::Resume,
            InitialHistory::New | InitialHistory::Forked(_) => {
                lyra_hooks::SessionStartSource::Startup
            }
            InitialHistory::Cleared => lyra_hooks::SessionStartSource::Clear,
        };
