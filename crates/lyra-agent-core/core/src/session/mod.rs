...
use lyra_analytics::SubAgentThreadStartedInput;
use lyra_app_server_protocol::McpServerElicitationRequest;
...
use lyra_network_proxy::NetworkProxyAuditMetadata;
use lyra_network_proxy::normalize_host;
use lyra_otel::current_span_trace_id;
...
use lyra_protocol::approvals::ExecPolicyAmendment;
use lyra_protocol::approvals::NetworkPolicyAmendment;
use lyra_protocol::approvals::NetworkPolicyRuleAction;
...
use lyra_protocol::models::format_allow_prefixes;
...
use lyra_protocol::protocol::RawResponseItemEvent;
...
use lyra_protocol::protocol::TurnContextNetworkItem;
use lyra_protocol::protocol::W3cTraceContext;
use lyra_protocol::request_permissions::PermissionGrantScope;
use lyra_protocol::request_permissions::RequestPermissionProfile;
use lyra_protocol::request_permissions::RequestPermissionsArgs;
use lyra_protocol::request_permissions::RequestPermissionsEvent;
...
use lyra_git_utils::get_git_repo_root;
...
use lyra_protocol::protocol::ModelRerouteEvent;
use lyra_protocol::protocol::ModelRerouteReason;
use lyra_protocol::protocol::NetworkApprovalContext;
use lyra_protocol::protocol::NonSteerableTurnKind;
use lyra_protocol::protocol::Op;
use lyra_protocol::protocol::RateLimitSnapshot;
use lyra_protocol::protocol::RequestUserInputEvent;
...
use lyra_protocol::protocol::SessionNetworkProxyRuntime;
use lyra_protocol::protocol::SkillDependencies as ProtocolSkillDependencies;
use lyra_protocol::protocol::SkillErrorInfo;
use lyra_protocol::protocol::SkillInterface as ProtocolSkillInterface;
use lyra_protocol::protocol::SkillMetadata as ProtocolSkillMetadata;
use lyra_protocol::protocol::SkillToolDependency as ProtocolSkillToolDependency;
use lyra_protocol::protocol::StreamErrorEvent;
...
use lyra_tools::ToolsConfigParams;
use lyra_utils_absolute_path::AbsolutePathBuf;
use lyra_utils_readiness::Readiness;
use lyra_utils_readiness::ReadinessFlag;

/// The high-level interface to a live Lyra session.
/// It operates as a queue pair where you send submissions and receive events.
pub struct LyraSessionHandle {
...
pub struct LyraSessionSpawnOk {
    pub session_handle: LyraSessionHandle,
    pub thread_id: ThreadId,
}

pub(crate) struct LyraSessionSpawnArgs {
...
const CYBER_SAFETY_URL: &str = ""; // TODO(lyra): replace with Lyra documentation URL

impl LyraSessionHandle {
    /// Spawn a new [`LyraSessionHandle`] and initialize the session.
    pub(crate) async fn spawn(args: LyraSessionSpawnArgs) -> LyraResult<LyraSessionSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if lyra_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(LyraSessionSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: LyraSessionSpawnArgs) -> LyraResult<LyraSessionSpawnOk> {
        let LyraSessionSpawnArgs {
            mut config,
            auth_manager,
            models_manager,
            environment_manager,
        ...
        let exec_policy = if crate::auto_review::is_auto_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| LyraErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = match session_source {
            SessionSource::SubAgent(_) => lyra_models_manager::manager::RefreshStrategy::Offline,
            _ => lyra_models_manager::manager::RefreshStrategy::OnlineIfUncached,
        };
        if config.model.is_none()
            || !matches!(
                refresh_strategy,
                lyra_models_manager::manager::RefreshStrategy::Offline
            )
        {
        ...
        let persisted_tools = if dynamic_tools.is_empty() {
            ...
            match thread_id {
                Some(thread_id) => {
                    ...
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "lyra_spawn")
                        .await
                }
                None => None,
            }
        ...
        let session_configuration = SessionConfiguration {
            ...
            lyra_home: config.lyra_home.clone(),
            thread_name: None,
        ...
        // Generate a unique ID for the lifetime of this LyraSessionHandle session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.lyra_home)
        })?;
        ...
        let session_handle = LyraSessionHandle {
            tx_sub,
            rx_event,
    ...
    /// Use sparingly: prefer `submit()` so the session handle is responsible
    /// for generating unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> LyraResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| LyraErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: lyra_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
...
async fn thread_title_from_state_db(
    ...
    find_thread_name_by_id(lyra_home, &conversation_id)
        .await
        .ok()
        .flatten()
}
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &lyra_execpolicy::Policy,
        sandbox_policy: &SandboxPolicy,
        network_policy_decider: Option<Arc<dyn lyra_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn lyra_network_proxy::BlockedRequestObserver>>,
    ...
    pub(crate) async fn lyra_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.lyra_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Lyra session performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn update_settings(
        ...
        let (previous_cwd, sandbox_policy_changed, next_cwd, lyra_home, session_source) = {
            let mut state = self.state.lock().await;
            ...
            let lyra_home = updated.lyra_home.clone();
    ...
    pub(crate) async fn developer_instructions_snapshot(&self) -> Option<String> {
        let state = self.state.lock().await;
    ...
    pub(crate) async fn reload_user_config_layer(&self) {
        let config_toml_path = {
            let state = self.state.lock().await;
            state.session_configuration.lyra_home.join(CONFIG_TOML_FILE)
        };
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &lyra_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| lyra_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&lyra_home, amendment)
            .await?;
    ...
    pub(crate) async fn record_model_warning(&self, message: impl Into<String>, ctx: &TurnContext) {
        self.services
            .session_telemetry
            .counter("lyra.model_warning", /*inc*/ 1, &[]);
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "This request was routed to a fallback model because the requested model is restricted for the current risk posture. Review the provider safety guidance: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn build_initial_context(
        ...
        if turn_context.features.enabled(Feature::LyraGitCommit)
    ...
    /// Non-regular tasks intentionally do not update the baseline.
    ...
    pub(crate) async fn notify_stream_error(
        ...
        let additional_details = lyra_error.to_string();
        let lyra_error_info = LyraErrorInfo::ResponseStreamDisconnected {
            http_status_code: lyra_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            lyra_error_info: Some(lyra_error_info),
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<lyra_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
