...
use crate::content_items::collect_user_messages;
...
use lyra_hooks::HookEvent;
use lyra_hooks::HookEventAfterAgent;
use lyra_hooks::HookPayload;
use lyra_hooks::HookResult;
...
use lyra_protocol::protocol::AgentMessageContentDeltaEvent;
use lyra_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use lyra_protocol::protocol::PlanDeltaEvent;
use lyra_protocol::protocol::ReasoningContentDeltaEvent;
use lyra_protocol::protocol::ReasoningRawContentDeltaEvent;
...
use lyra_utils_stream_parser::AssistantTextChunk;
use lyra_utils_stream_parser::AssistantTextStreamParser;
use lyra_utils_stream_parser::ProposedPlanSegment;
use lyra_utils_stream_parser::extract_proposed_plan_text;
...
pub(crate) async fn run_turn(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = lyra_connectors::merge::merge_plugin_connectors_with_accessible(
            loaded_plugins
                .effective_apps()
                .into_iter()
    ...
    if !input.is_empty() {
        ...
        // standalone tasks (shell/review/undo) cannot suppress future model injections.
    ...
    // Although from the perspective of lyra.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    // 2. When model/tool continuation needs to resume before any steer.
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    .to_string();
                    let stop_request = lyra_hooks::StopRequest {
                    ...
                    for run in sess.hooks().preview_stop(&stop_request) {
                        sess.send_event(
                            &turn_context,
                            EventMsg::HookStarted(lyra_protocol::protocol::HookStartedEvent {
            ...
            Err(LyraErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(LyraErr::InvalidImageRequest()) => {
                {
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = lyra_connectors::metadata::connector_mention_slug(connector);
...
fn connector_inserted_in_messages(
    ...
    let mention_slug = lyra_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    ...
    cancellation_token: CancellationToken,
) -> LyraResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(LyraErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(LyraErr::ContextWindowExceeded);
            }
            Err(LyraErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(LyraErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                LyraErr::Stream(_, requested_delay) => {
...
fn agent_message_text(item: &lyra_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: lyra_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(lyra_protocol::items::AgentMessageItem {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, LyraResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> LyraResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, LyraResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: LyraResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(lyra_async_utils::CancelErr::Cancelled) => break Err(LyraErr::TurnAborted),
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![lyra_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(LyraErr::TurnAborted);
    }
