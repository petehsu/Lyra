use super::*;
use anyhow::Result;
use lyra_app_server_protocol::AppConfig;
use lyra_app_server_protocol::AppToolApproval;
use lyra_app_server_protocol::AppsConfig;
...
async fn write_value_preserves_comments_and_order() -> Result<()> {
    ...
    let original = r#"# Lyra user configuration
model = "gpt-5"
...
"#;
    ...
    let expected = r#"# Lyra user configuration
model = "gpt-5"
approval_policy = "on-request"
...
async fn reserved_builtin_provider_override_rejected() {
    ...
    let error = service
        .write_value(ConfigValueWriteParams {
            ...
            value: serde_json::json!("Provider Override"),
            merge_strategy: MergeStrategy::Replace,
            expected_version: None,
        })
        .await
