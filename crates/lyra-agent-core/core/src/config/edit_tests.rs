...
use lyra_config::types::McpServerToolConfig;
...
"#,
    ...
    let raw = std::fs::read_to_string(lyra_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = lyra_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let lyra_home = tmp.path();
    let link_a = lyra_home.join("a.toml");
    let link_b = lyra_home.join("b.toml");
    let config_path = lyra_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(lyra_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(lyra_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn blocking_set_hide_gpt_5_1_lyra_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let lyra_home = tmp.path();
    std::fs::write(
        lyra_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        ...
        /*profile*/ None,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-lyra-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-lyra-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !lyra_home.join(CONFIG_TOML_FILE).exists(),
...
async fn async_builder_set_model_persists() {
    let tmp = tempdir().expect("tmpdir");
    let lyra_home = tmp.path().to_path_buf();
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(lyra_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(lyra_home.join(CONFIG_TOML_FILE)).expect("read config");
