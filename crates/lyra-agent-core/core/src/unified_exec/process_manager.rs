...
) -> lyra_exec_server::ExecEnvPolicy {
    lyra_exec_server::ExecEnvPolicy {
...
fn exec_server_env_for_request(
    ...
    Option<lyra_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        environment: &lyra_exec_server::Environment,
        ...
        let spawn_result = if tty {
            lyra_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
                &request.arg0,
                lyra_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            lyra_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            LYRA_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Lyra(LyraErr::Sandbox(SandboxErr::Denied { output, .. })) => {
                    let output = *output;
