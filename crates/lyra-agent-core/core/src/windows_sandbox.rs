...
pub fn sandbox_setup_is_complete(lyra_home: &Path) -> bool {
    lyra_windows_sandbox::sandbox_setup_is_complete(lyra_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_lyra_home: &Path) -> bool {
    false
}

#[cfg(target_os = "windows")]
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = lyra_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = lyra_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if lyra_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            lyra_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
        "lyra.windows_sandbox.elevated_setup_canceled"
    } else {
        "lyra.windows_sandbox.elevated_setup_failure"
    }
}
...
pub fn run_elevated_setup(
    ...
    lyra_windows_sandbox::run_elevated_setup(
        lyra_windows_sandbox::SandboxSetupRequest {
            policy,
            policy_cwd,
            command_cwd,
            env_map,
        ...
        lyra_windows_sandbox::SetupRootOverrides::default(),
    )
}
...
pub fn run_legacy_setup_preflight(
    ...
    lyra_windows_sandbox::run_windows_sandbox_legacy_preflight(
        policy,
        policy_cwd,
        lyra_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    lyra_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
pub async fn run_windows_sandbox_setup(request: WindowsSandboxSetupRequest) -> anyhow::Result<()> {
    ...
    let client_name_tag = sanitize_metric_tag_value(client_name.as_str());
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let lyra_home = request.lyra_home;
    let active_profile = request.active_profile;
    let setup_lyra_home = lyra_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_lyra_home.as_path()) {
                    run_elevated_setup(
                        &policy,
                        policy_cwd.as_path(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_lyra_home.as_path(),
                    )?;
                }
            }
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let Some(metrics) = lyra_otel::global() else {
        return;
    };
    let mode_tag = windows_sandbox_setup_mode_tag(mode);
    let _ = metrics.record_duration(
        "lyra.windows_sandbox.setup_duration_ms",
        duration,
        &[
            ("result", "success"),
    ...
    let _ = metrics.counter(
        "lyra.windows_sandbox.setup_success",
        /*inc*/ 1,
        &[("client_name", client_name_tag), ("mode", mode_tag)],
    );
}
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    let _ = metrics.counter(
        "lyra.windows_sandbox.setup_failure",
        /*inc*/ 1,
        &[("client_name", client_name_tag), ("mode", mode_tag)],
    );
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        #[cfg(target_os = "windows")]
        {
            let mut failure_tags: Vec<(&str, &str)> = vec![("client_name", client_name_tag)];
        ...
        let _ = metrics.counter(
            "lyra.windows_sandbox.legacy_setup_preflight_failed",
            /*inc*/ 1,
            &[("client_name", client_name_tag)],
        );
    }
