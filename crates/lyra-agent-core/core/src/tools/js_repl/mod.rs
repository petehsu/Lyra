...
pub(crate) const JS_REPL_PRAGMA_PREFIX: &str = "// lyra-js-repl:";
...
impl JsReplManager {
    ...
    async fn start_kernel(
        ...
        let node_module_dirs_key = "LYRA_JS_REPL_NODE_MODULE_DIRS";
...
fn validate_emitted_image_url(image_url: &str) -> Result<(), String> {
        ...
        .is_some_and(|scheme| scheme.eq_ignore_ascii_case("data:"))
    {
        ...
        Err("lyra.emitImage only accepts data URLs".to_string())
    }
}
...
pub(crate) fn resolve_node(config_path: Option<&Path>) -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("LYRA_JS_REPL_NODE_PATH") {
