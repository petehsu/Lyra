...
use lyra_protocol::protocol::CollabAgentStatusEntry;
...
pub(crate) fn collab_spawn_error(err: LyraErr) -> FunctionCallError {
    match err {
        LyraErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        LyraErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: LyraErr) -> FunctionCallError {
    match err {
        LyraErr::ThreadNotFound(id) => {
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.lyra_linux_sandbox_exe = turn.lyra_linux_sandbox_exe.clone();
...
fn find_spawn_agent_model_name(
    available_models: &[lyra_protocol::openai_models::ModelPreset],
    requested_model: &str,
