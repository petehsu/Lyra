    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    lyra_protocol::models::ContentItem::InputText { text }
                    | lyra_protocol::models::ContentItem::OutputText { text } => {
                        FunctionCallOutputContentItem::InputText { text }
                    }
                    lyra_protocol::models::ContentItem::InputImage { image_url, detail } => {
