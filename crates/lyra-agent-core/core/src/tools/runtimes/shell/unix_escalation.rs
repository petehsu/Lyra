...
use lyra_execpolicy::MatchOptions;
...
use lyra_shell_command::bash::parse_shell_lc_plain_commands;
use lyra_shell_command::bash::parse_shell_lc_single_command_prefix;
use lyra_shell_escalation::EscalateServer;
use lyra_shell_escalation::EscalationDecision;
use lyra_shell_escalation::EscalationExecution;
use lyra_shell_escalation::EscalationPermissions;
use lyra_shell_escalation::EscalationPolicy;
use lyra_shell_escalation::EscalationSession;
use lyra_shell_escalation::ExecParams;
use lyra_shell_escalation::ExecResult;
use lyra_shell_escalation::Permissions as EscalatedPermissions;
use lyra_shell_escalation::PreparedExec;
use lyra_shell_escalation::ShellCommandExecutor;
use lyra_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(|err| ToolError::Lyra(err.into()))?;
...
struct CoreShellCommandExecutor {
    ...
    network: Option<lyra_network_proxy::NetworkProxy>,
...
impl ShellCommandExecutor for CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["LYRA_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Lyra(LyraErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::Lyra(LyraErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),
