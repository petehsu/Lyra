...
*/
use crate::exec_env::LYRA_THREAD_ID_ENV_VAR;
...
/// runtime-only vars like `LYRA_THREAD_ID` without pretending they came from
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    if let Some(thread_id) = env.get(LYRA_THREAD_ID_ENV_VAR) {
        override_env.insert(LYRA_THREAD_ID_ENV_VAR.to_string(), thread_id.clone());
    }
...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    let captures = keys
        ...
        .map(|(idx, key)| {
            format!(
                "__LYRA_SNAPSHOT_OVERRIDE_SET_{idx}=\"${{{key}+x}}\"\n__LYRA_SNAPSHOT_OVERRIDE_{idx}=\"${{{key}-}}\""
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
    let restores = keys
        ...
        .map(|(idx, key)| {
            format!(
                "if [ -n \"${{__LYRA_SNAPSHOT_OVERRIDE_SET_{idx}}}\" ]; then export {key}=\"${{__LYRA_SNAPSHOT_OVERRIDE_{idx}}}\"; else unset {key}; fi"
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
