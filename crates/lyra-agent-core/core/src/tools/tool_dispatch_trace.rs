...
use lyra_rollout_trace::ExecutionStatus;
use lyra_rollout_trace::ToolDispatchInvocation;
use lyra_rollout_trace::ToolDispatchPayload;
use lyra_rollout_trace::ToolDispatchRequester;
use lyra_rollout_trace::ToolDispatchResult;
use lyra_rollout_trace::ToolDispatchTraceContext;
...
fn tool_dispatch_invocation(invocation: &ToolInvocation) -> Option<ToolDispatchInvocation> {
    let requester = match invocation.source {
        ...
        ToolCallSource::JsRepl | ToolCallSource::CodeMode => return None,
    };

    Some(ToolDispatchInvocation {
        thread_id: invocation.session.conversation_id.to_string(),
        lyra_turn_id: invocation.turn.sub_id.clone(),
...
fn tool_dispatch_result(
    ...
    match invocation.source {
        ...
        ToolCallSource::JsRepl | ToolCallSource::CodeMode => None,
    }
}
