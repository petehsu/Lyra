...
pub fn find_lyra_home() -> std::io::Result<AbsolutePathBuf> {
    let lyra_home_env = std::env::var("LYRA_HOME")
        .ok()
        ...
        Some(val) => {
            ...
            if !metadata.is_dir() {
                Err(std::io::Error::new(
                    ...
                    format!(
                        "configured Lyra runtime home points to {val:?}, but that path is not a directory"
                    ),
                ))
            } else {
                let canonical = path.canonicalize().map_err(|err| {
                    std::io::Error::new(
                        err.kind(),
                        format!(
                            "failed to canonicalize configured Lyra runtime home {val:?}: {err}"
                        ),
                    )
                })?;
