...
use lyra_rollout::RolloutRecorder;
use lyra_rollout::find_archived_thread_path_by_id_str;
use lyra_rollout::find_thread_name_by_id;
use lyra_rollout::find_thread_path_by_id_str;
use lyra_rollout::read_session_meta_line;
use lyra_rollout::read_thread_item_from_rollout;
...
pub(super) async fn read_thread(
        ...
        && (params.include_archived || metadata.archived_at.is_none())
    {
        ...
        if params.include_history {
            ...
            let items = load_history_items(thread_id, &path).await?;
...
async fn resolve_rollout_path(
    store: &LocalThreadStore,
    thread_id: lyra_protocol::ThreadId,
    include_archived: bool,
) -> ThreadStoreResult<Option<std::path::PathBuf>> {
    if include_archived {
        match find_thread_path_by_id_str(store.config.lyra_home.as_path(), &thread_id.to_string())
            .await
        ...
        find_thread_path_by_id_str(store.config.lyra_home.as_path(), &thread_id.to_string())
            .await
            .map_err(|err| ThreadStoreError::InvalidRequest {
                message: format!("failed to locate thread id {thread_id}: {err}"),
            })
...
async fn read_thread_from_rollout_path(
    ...
    let Some(item) = read_thread_item_from_rollout(path.clone()).await else {
        return stored_thread_from_session_meta(store, thread_id, path).await;
    };
    ...
    if let Ok(Some(title)) =
        find_thread_name_by_id(store.config.lyra_home.as_path(), &thread_id).await
    {
...
async fn load_history_items(
    thread_id: lyra_protocol::ThreadId,
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<lyra_protocol::protocol::RolloutItem>> {
    let (items, _, _) = RolloutRecorder::load_rollout_items(path)
        .await
        .map_err(|err| rollout_read_error(thread_id, path, err, "load thread history"))?;
    Ok(items)
}
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let name = match distinct_title(&metadata) {
        ...
        None => find_thread_name_by_id(store.config.lyra_home.as_path(), &metadata.id)
            .await
            .ok()
            .flatten(),
    };
    ...
    StoredThread {
        ...
        cwd: metadata.cwd,
        bound_project_root: metadata.bound_project_root,
...
async fn stored_thread_from_session_meta(
    ...
    let meta_line = read_session_meta_line(path.as_path())
        .await
        .map_err(|err| rollout_read_error(thread_id, path.as_path(), err, "read thread"))?;
    let archived = path.starts_with(
        store
            .config
            .lyra_home
...
fn rollout_read_error(
    thread_id: lyra_protocol::ThreadId,
    path: &std::path::Path,
    err: std::io::Error,
    operation: &str,
) -> ThreadStoreError {
    if is_rollout_not_materialized_error(&err) {
        return ThreadStoreError::NotMaterialized {
            thread_id,
            message: format!(
                "rollout {} is not ready while trying to {operation}: {err}",
                path.display()
            ),
        };
    }
    ...
    ThreadStoreError::Internal {
        message: format!("failed to {operation} {}: {err}", path.display()),
    }
}

fn is_rollout_not_materialized_error(err: &std::io::Error) -> bool {
    matches!(
        err.kind(),
...
mod tests {
    ...
    async fn read_thread_applies_sqlite_thread_name() {
        ...
        let rollout_path =
            write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        let runtime = lyra_state::StateRuntime::init(
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        lyra_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
    ...
    #[tokio::test]
    async fn read_thread_reports_missing_sqlite_rollout_as_not_materialized() {
        let home = TempDir::new().expect("temp dir");
        let config = test_config(home.path());
        let store = LocalThreadStore::new(config.clone());
        ...
        let err = store
            ...
            .await
            .expect_err("missing sqlite rollout should be materialization state");

        let ThreadStoreError::NotMaterialized {
            thread_id: err_thread_id,
            message,
        } = err
        else {
            panic!("expected not materialized error");
        };
    ...
    #[tokio::test]
    async fn read_thread_reports_empty_rollout_as_not_materialized() {
        let home = TempDir::new().expect("temp dir");
        let store = LocalThreadStore::new(test_config(home.path()));
        ...
        std::fs::create_dir_all(&day_dir).expect("sessions dir");
        let rollout_path = day_dir.join(format!("rollout-2025-01-03T12-00-00-{uuid}.jsonl"));
        std::fs::File::create(&rollout_path).expect("empty session file");
        ...
        let err = store
            ...
            .await
            .expect_err("empty rollout should be materialization state");
