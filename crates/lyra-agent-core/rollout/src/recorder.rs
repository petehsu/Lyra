//! Persist Lyra session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use crate::default_client::client_identity;
use crate::state_db;
use crate::state_db::StateDbHandle;
use lyra_git_utils::collect_git_info;
use lyra_protocol::protocol::EventMsg;
use lyra_protocol::protocol::GitInfo as ProtocolGitInfo;
use lyra_protocol::protocol::InitialHistory;
use lyra_protocol::protocol::ResumedHistory;
use lyra_protocol::protocol::RolloutItem;
use lyra_protocol::protocol::RolloutLine;
use lyra_protocol::protocol::SessionMeta;
...
use lyra_utils_path as path_utils;
...
///
/// ```ignore
/// $ jq -C . ~/.lyra/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.lyra/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Lyra runtime home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        let lyra_home = config.lyra_home();
    ...
    pub async fn load_rollout_items(
        ...
        if text.trim().is_empty() {
            return Err(IoError::new(ErrorKind::UnexpectedEof, "empty session file"));
        }
...
async fn list_threads_from_files_desc(
    ...
    if let Some(search_term) = search_term {
        ...
        loop {
            ...
            filter_thread_items_by_search_term(lyra_home, &mut page.items, Some(search_term))
                .await?;
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = lyra_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(lyra_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(lyra_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.lyra/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.lyra_home().to_path_buf();
...
async fn sync_thread_state_after_write(
    ...
    if new_thread_memory_mode.is_some()
        || items
            .iter()
            .any(lyra_state::rollout_item_affects_thread_metadata)
    {
...
impl From<lyra_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: lyra_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
            .map(|item| ThreadItem {
