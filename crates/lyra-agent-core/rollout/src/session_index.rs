...
pub async fn append_thread_name(
    ...
    append_session_index_entry(lyra_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(lyra_home);
...
pub async fn find_thread_meta_by_name_str(
    ...
    while let Some(thread_id) = rx.recv().await {
        ...
        if let Some(path) =
            super::list::find_thread_path_by_id_str(lyra_home, &thread_id.to_string()).await?
...
fn session_index_path(lyra_home: &Path) -> PathBuf {
    lyra_home.join(SESSION_INDEX_FILE)
}
