...
use lyra_protocol::protocol::SessionStateUpdate;
use lyra_state::BackfillStatus;
...
fn test_config(lyra_home: PathBuf) -> RolloutConfig {
    RolloutConfig {
        sqlite_home: lyra_home.clone(),
        cwd: lyra_home.clone(),
        lyra_home,
...
fn builder_from_items_falls_back_to_filename() {
    ...
    let path = dir
        .path()
        .join(format!("rollout-2026-01-27T12-34-56-{uuid}.jsonl"));
    let items = vec![RolloutItem::SessionState(SessionStateUpdate::default())];
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let lyra_home = dir.path().to_path_buf();
    ...
    let runtime = lyra_state::StateRuntime::init(lyra_home.clone(), "test-provider".to_string())
        .await
        .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(lyra_home.as_path(), first_path.as_path());
    ...
    let config = test_config(lyra_home.clone());
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let lyra_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = lyra_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = lyra_home.join("sessions");
