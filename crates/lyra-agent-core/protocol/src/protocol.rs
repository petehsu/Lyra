//! Defines the protocol for a Lyra session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Lyra:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::lyra_thread::LyraThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    OverrideTurnContext {
        ...
        /// Updated model verbosity (honored only for verbosity-capable models).
        #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// This is a local-only operation handled by lyra-core; it does not
    /// involve the model.
    ...
    /// Request to shut down the Lyra instance.
    Shutdown,
...
/// running the command proposed by Lyra.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.lyra`, `.git`, notably `.git/hooks`) under
...
fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .lyra read-only to the agent by
    ...
    let top_level_lyra = writable_root.join(".lyra");
    if protect_missing_dot_lyra || top_level_lyra.as_path().is_dir() {
        subpaths.push(top_level_lyra);
    }
...
/// Lyra errors that we expose to clients.
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.lyra_error_info
            .as_ref()
            .is_none_or(LyraErrorInfo::affects_turn_status)
    }
}
...
// Includes prompts, tool calls, and runtime bookkeeping.
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Lyra),
...
impl TruncationPolicy {
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(lyra_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => lyra_utils_string::approx_bytes_for_tokens(*tokens),
        }
    }
}
...
mod tests {
    ...
    fn session_source_from_startup_arg_normalizes_custom_values() {
        assert_eq!(
            SessionSource::from_startup_arg("workspace").unwrap(),
            SessionSource::Custom("workspace".to_string())
        );
        assert_eq!(
            SessionSource::from_startup_arg(" Workspace ").unwrap(),
            SessionSource::Custom("workspace".to_string())
    ...
    fn session_source_thread_source_name_classifies_user_and_subagent_sources() {
        for (source, expected) in [
            ...
            (SessionSource::Custom("workspace".to_string()), None),
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_lyra() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        assert_eq!(
            SessionSource::Custom("lyra".to_string()).restriction_product(),
            Some(Product::Lyra)
        );
        assert_eq!(
            SessionSource::Custom("workspace".to_string()).restriction_product(),
            None
        );
    }
    ...
    fn session_source_matches_product_restriction() {
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Lyra]));
        assert!(SessionSource::Custom("workspace".to_string()).matches_product_restriction(&[]));
    }
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            lyra_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".lyra")).expect("create .lyra");
        let canonical_cwd = lyra_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_lyra = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".lyra"))
            .expect("canonical .lyra");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_lyra.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_lyra = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".lyra"))
            .expect("canonical .lyra");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![expected_dot_lyra.to_path_buf(), expected_docs.to_path_buf()],
                ),
    ...
    fn serialize_event() -> Result<()> {
        ...
        let event = Event {
            ...
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                ...
                forked_from_id: None,
                thread_name: None,
                model: "lyra-mini-latest".to_string(),
