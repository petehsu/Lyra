...
use lyra_utils_absolute_path::canonicalize_preserving_symlinks;
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Lyra release degrades to
    /// warn-and-ignore instead of failing to load. Older releases rejected
    /// unknown values here, which broke forward compatibility for newer config.
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            ...
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_lyra*/ true,
                ) {
            ...
            for writable_root in writable_roots {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    writable_root,
                    /*protect_missing_dot_lyra*/ false,
                ) {
                    append_default_read_only_path_if_no_explicit_rule(
                        &mut file_system_policy.entries,
                        protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let raw_writable_roots: Vec<&AbsolutePathBuf> = writable_entries
                ...
                .collect();
            let protect_missing_dot_lyra = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_lyra)
                    .into_iter()
            ...
            // Example: if `<root>/.lyra -> <root>/decoy`, bwrap must still see
            // `<root>/.lyra`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.lyra` so downstream sandboxes can still detect and mask
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "LYRA_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn writable_roots_proactively_protect_missing_dot_lyra() {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_dot_lyra)
        );
    }

    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_lyra_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_lyra = expected_root.join(".lyra");
        ...
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_lyra),
            "explicit .lyra rule should win over the default protected carveout"
        );
        assert!(
            policy.can_write_path_with_cwd(
                explicit_dot_lyra.join("config.toml").as_path(),
                cwd.path()
            )
        );
    }
    ...
    fn projected_workspace_write_projection_accepts_relative_cwd() {
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".lyra/config.toml"), relative_cwd,)
        );
    }
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let lyra_dir = real_root.join(".lyra");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&lyra_dir).expect("create .lyra");
        ...
        let expected_lyra = link_root.join(".lyra");
    ...
    fn current_working_directory_special_path_preserves_symlinked_cwd() {
        ...
        let expected_lyra = expected_root.join(".lyra");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-lyra");
        let dot_lyra = root.join(".lyra");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_lyra).expect("create .lyra symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let lyra_dir = real_tmpdir.join(".lyra");
