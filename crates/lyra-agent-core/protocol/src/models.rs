...
use lyra_utils_image::PromptImageMode;
use lyra_utils_image::load_for_prompt_bytes;
...
use lyra_utils_image::ImageProcessingError;
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Lyra could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Lyra cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
fn convert_mcp_content_to_items(
    ...
    const LYRA_IMAGE_DETAIL_META_KEY: &str = "lyra/imageDetail";
            ...
            Ok(McpContent::Image {
                ...
                FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: meta
                        ...
                        .and_then(|meta| meta.get(LYRA_IMAGE_DETAIL_META_KEY))
...
// (Moved event mapping logic into lyra-core to avoid coupling protocol to UI-facing events.)
...
mod tests {
    ...
    fn function_call_deserializes_optional_namespace() {
        let item: ResponseItem = serde_json::from_value(serde_json::json!({
            "type": "function_call",
            "name": "mcp__workspace__gmail_get_recent_emails",
            "namespace": "mcp__workspace__gmail",
            "arguments": "{\"top_k\":5}",
            "call_id": "call-1",
        }))
        ...
        assert_eq!(
            ...
            ResponseItem::FunctionCall {
                id: None,
                name: "mcp__workspace__gmail_get_recent_emails".to_string(),
                namespace: Some("mcp__workspace__gmail".to_string()),
    ...
    fn includes_request_rule_instructions_for_on_request() {
        ...
        exec_policy
            .add_prefix_rule(
                ...
                lyra_execpolicy::Decision::Allow,
            )
            .expect("add rule");
        let instructions = DeveloperInstructions::from_permissions_with_network(
            SandboxMode::WorkspaceWrite,
    ...
    fn tool_search_output_roundtrips() -> Result<()> {
        let input = ResponseInputItem::ToolSearchOutput {
            ...
            tools: vec![serde_json::json!({
                "type": "function",
                "name": "mcp__workspace__calendar_create_event",
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Lyra cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
