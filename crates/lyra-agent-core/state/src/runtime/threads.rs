...
fn thread_spawn_parent_thread_id_from_source_str(source: &str) -> Option<ThreadId> {
    ...
    match parsed_source.ok() {
        Some(SessionSource::SubAgent(lyra_protocol::protocol::SubAgentSource::ThreadSpawn {
            parent_thread_id,
            ..
...
mod tests {
    ...
    use lyra_protocol::protocol::GitInfo;
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&lyra_home, thread_id, lyra_home.clone());
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(lyra_git_utils::GitSha::new("rollout-sha")),
                branch: Some("rollout-branch".to_string()),
                repository_url: Some("git@example.com:lyra/agent.git".to_string()),
            }),
        })];
        ...
        assert_eq!(
            ...
            Some("git@example.com:lyra/agent.git")
        );
    }
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:lyra/agent.git")),
            )
            .await
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&lyra_home, thread_id, lyra_home.clone());
        ...
        let mut fallback = test_thread_metadata(&lyra_home, thread_id, lyra_home.clone());
    ...
    async fn update_thread_git_info_can_clear_fields() {
        ...
        metadata.git_origin_url = Some("git@example.com:lyra/agent.git".to_string());
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&lyra_home, first_id, lyra_home.clone());
        first.updated_at = updated_at;
        let mut second = test_thread_metadata(&lyra_home, second_id, lyra_home.clone());
        ...
        let mut older = test_thread_metadata(&lyra_home, older_id, lyra_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            lyra_protocol::protocol::TokenCountEvent {
                info: Some(lyra_protocol::protocol::TokenUsageInfo {
                    total_token_usage: lyra_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: lyra_protocol::protocol::TokenUsage::default(),
    ...
    async fn thread_spawn_edges_track_directional_status() {
        let lyra_home = unique_temp_dir();
        let runtime = StateRuntime::init(lyra_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
