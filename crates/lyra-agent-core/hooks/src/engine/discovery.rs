...
use lyra_config::ConfigLayerSource;
...
pub(crate) fn discover_handlers(config_layer_stack: Option<&ConfigLayerStack>) -> DiscoveryResult {
    ...
    for layer in config_layer_stack.get_layers(
        ...
        for (event_name, groups) in [
            (
                lyra_protocol::protocol::HookEventName::PreToolUse,
                pre_tool_use,
            ),
            (
                lyra_protocol::protocol::HookEventName::PermissionRequest,
                permission_request,
            ),
            (
                lyra_protocol::protocol::HookEventName::PostToolUse,
                post_tool_use,
            ),
            (
                lyra_protocol::protocol::HookEventName::SessionStart,
                session_start,
            ),
            (
                lyra_protocol::protocol::HookEventName::UserPromptSubmit,
                user_prompt_submit,
            ),
            (lyra_protocol::protocol::HookEventName::Stop, stop),
        ] {
...
fn append_matcher_groups(
    ...
    source: HookSource,
    event_name: lyra_protocol::protocol::HookEventName,
...
mod tests {
    ...
    fn hook_source_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.lyra/config.toml").abs();
        let dot_lyra_folder = test_path_buf("/tmp/worktree/.lyra").abs();
        ...
        assert_eq!(
            super::hook_source_for_config_layer_source(&ConfigLayerSource::Mdm {
                domain: "com.openai.lyra".to_string(),
