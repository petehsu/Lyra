...
mod exec_approval;
mod lyra_tool_config;
mod lyra_tool_runner;
...
pub use crate::lyra_tool_config::LyraToolCallParam;
pub use crate::lyra_tool_config::LyraToolCallReplyParam;
...
const OTEL_SERVICE_NAME: &str = "lyra_mcp_server";
...
mod tests {
    ...
    async fn mcp_server_builds_otel_provider_with_logs_traces_and_metrics() -> anyhow::Result<()> {
        ...
        let provider = lyra_core::otel_init::build_provider(
            &config,
            "0.0.0-test",
            Some(OTEL_SERVICE_NAME),
            DEFAULT_ANALYTICS_ENABLED,
