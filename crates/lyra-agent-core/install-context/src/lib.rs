...
const RESOURCES_DIRNAME: &str = "lyra-resources";
...
pub enum InstallContext {
    Standalone {
        ...
        /// `~/.lyra/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        release_dir: PathBuf,
    ...
    /// A Lyra binary launched through the npm-managed `lyra.js` shim.
    Npm,
    /// A Lyra binary launched through the bun-managed `lyra.js` shim.
    Bun,
    /// A Lyra binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Lyra binaries, custom
    /// internal launchers, and tests that execute Lyra from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let lyra_home = lyra_utils_home_dir::find_lyra_home().ok();
        Self::from_exe_with_lyra_home(
            is_macos,
            current_exe,
            managed_by_npm,
            managed_by_bun,
    ...
    fn from_exe_with_lyra_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        managed_by_npm: bool,
        managed_by_bun: bool,
        lyra_home: Option<&Path>,
    ) -> Self {
        if managed_by_npm {
            return Self::Npm;
        }
        ...
        if let Some(exe_path) = current_exe
            && let Some(standalone_context) = standalone_install_context(exe_path, lyra_home)
        {
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("LYRA_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("LYRA_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
...
fn standalone_install_context(exe_path: &Path, lyra_home: Option<&Path>) -> Option<InstallContext> {
    let canonical_exe = std::fs::canonicalize(exe_path).ok()?;
    let canonical_lyra_home = std::fs::canonicalize(lyra_home?).ok()?;
    let release_dir = canonical_exe.parent()?.to_path_buf();
    let releases_root = canonical_lyra_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        let lyra_home = tempfile::tempdir()?;
        let release_dir = lyra_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
        let resources_dir = release_dir.join(RESOURCES_DIRNAME);
        fs::create_dir_all(&resources_dir)?;
        let exe_path = release_dir.join(if cfg!(windows) { "lyra.exe" } else { "lyra" });
        fs::write(&exe_path, "")?;
        ...
        let context = InstallContext::from_exe_with_lyra_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*lyra_home*/ Some(lyra_home.path()),
        );
        assert_eq!(
            context,
    ...
    fn npm_and_bun_take_precedence() {
        let npm_context = InstallContext::from_exe_with_lyra_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/lyra")),
            /*managed_by_npm*/ true,
            /*managed_by_bun*/ false,
            /*lyra_home*/ None,
        );
        ...
        let bun_context = InstallContext::from_exe_with_lyra_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/lyra")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ true,
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_lyra_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/lyra")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*lyra_home*/ None,
        );
