...
use crate::protocol_event::lyra_turn_trace_event;
...
pub const LYRA_ROLLOUT_TRACE_ROOT_ENV: &str = "LYRA_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `LYRA_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Lyra
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(LYRA_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Lyra turn lifecycle events from protocol lifecycle events.
    pub fn record_lyra_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            lyra_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Lyra did while executing that boundary.
    pub fn record_tool_call_event(&self, lyra_turn_id: impl Into<LyraTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(lyra_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_lyra_turn_id: impl Into<LyraTurnId>,
    ...
    pub fn record_lyra_turn_started(&self, lyra_turn_id: impl Into<LyraTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let lyra_turn_id = lyra_turn_id.into();
        context.append_with_context_best_effort(
            lyra_turn_id.clone(),
            RawTraceEventPayload::LyraTurnStarted {
                lyra_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one Lyra turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        lyra_turn_id: LyraTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            lyra_turn_id: Some(lyra_turn_id),
        };
