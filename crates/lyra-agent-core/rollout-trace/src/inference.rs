...
///
/// A Lyra turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Lyra turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        lyra_turn_id: LyraTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                lyra_turn_id: attempt.context.lyra_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps lyra-core focused on
...
/// Lyra received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        lyra_turn_id: Some(context.lyra_turn_id.clone()),
    };
...
mod tests {
    ...
    use lyra_protocol::models::ReasoningItemReasoningSummary;
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        writer.append(RawTraceEventPayload::LyraTurnStarted {
            lyra_turn_id: "turn-1".to_string(),
            thread_id: "thread-root".to_string(),
        })?;
        ...
        assert_eq!(inference.lyra_turn_id, "turn-1");
