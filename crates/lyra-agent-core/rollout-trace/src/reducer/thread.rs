...
use crate::model::LyraTurn;
use crate::model::LyraTurnId;
...
impl TraceReducer {
    ...
    /// Starts a Lyra turn inside an existing thread.
    pub(super) fn start_lyra_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.lyra_turns.contains_key(&lyra_turn_id) {
            bail!("duplicate lyra turn start for {lyra_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.lyra_turns.insert(
            lyra_turn_id.clone(),
            LyraTurn {
                lyra_turn_id,
                thread_id,
    ...
    /// Marks a Lyra turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_lyra_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.lyra_turns.get(&lyra_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "lyra turn end for {lyra_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.lyra_turns.get_mut(&lyra_turn_id) else {
            bail!("lyra turn end referenced unknown turn {lyra_turn_id}");
        };
