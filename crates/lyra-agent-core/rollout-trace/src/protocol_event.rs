//! Mapping from Lyra protocol events into raw rollout-trace events.
//!
...
//! `lyra-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `lyra-rollout-trace`.
//!
...
use lyra_protocol::protocol::ExecCommandStatus;
...
use lyra_protocol::protocol::PatchApplyStatus;
...
pub(crate) struct LyraTurnTraceEvent {
    pub context_turn_id: LyraTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn lyra_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<LyraTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let lyra_turn_id = event.turn_id.clone();
            Some(LyraTurnTraceEvent {
                context_turn_id: lyra_turn_id.clone(),
                payload: RawTraceEventPayload::LyraTurnStarted {
                    lyra_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(LyraTurnTraceEvent {
                context_turn_id: lyra_turn_id.clone(),
                payload: RawTraceEventPayload::LyraTurnEnded {
                    lyra_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a lyra_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a lyra_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a lyra_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a lyra_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a lyra_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a lyra_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a lyra_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a lyra_protocol::protocol::CollabCloseEndEvent),
}
...
pub(crate) fn wrapped_protocol_event_type(event: &EventMsg) -> Option<&'static str> {
    match event {
        ...
        | EventMsg::PlanApprovalResolved(_) => None,
    }
}
