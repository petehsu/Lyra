...
use lyra_config::MarketplaceConfigUpdate;
use lyra_config::record_user_marketplace;
use lyra_config::types::MarketplaceConfig;
use lyra_config::types::MarketplaceSourceType;
use lyra_plugin::validate_plugin_segment;
...
pub fn upgrade_configured_git_marketplaces(
    ...
    for marketplace in marketplaces {
        match upgrade_configured_git_marketplace(lyra_home, &install_root, &marketplace) {
...
fn marketplace_install_root(lyra_home: &Path) -> PathBuf {
    lyra_home.join(INSTALLED_MARKETPLACES_DIR)
}
...
fn upgrade_configured_git_marketplace(
    ...
    activate_marketplace_root(&destination, staged_dir, || {
        ensure_configured_git_marketplace_unchanged(lyra_home, marketplace)?;
        record_user_marketplace(lyra_home, &marketplace.name, &update).map_err(|err| {
            format!(
...
fn ensure_configured_git_marketplace_unchanged(
    ...
    let current = read_configured_git_marketplace(lyra_home, &expected.name)?;
    match current {
