...
use lyra_core_skills::SkillMetadata;
use lyra_core_skills::config_rules::SkillConfigRules;
use lyra_core_skills::config_rules::resolve_disabled_skill_paths;
use lyra_core_skills::config_rules::skill_config_rules_from_stack;
use lyra_core_skills::loader::SkillRoot;
use lyra_core_skills::loader::load_skills_from_roots;
use lyra_exec_server::LOCAL_FS;
use lyra_plugin::AppConnectorId;
use lyra_plugin::LoadedPlugin;
use lyra_plugin::PluginCapabilitySummary;
use lyra_plugin::PluginId;
use lyra_plugin::PluginIdError;
use lyra_plugin::PluginLoadOutcome;
use lyra_plugin::PluginTelemetryMetadata;
use lyra_protocol::protocol::Product;
use lyra_protocol::protocol::SkillScope;
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let configured_plugin_ids = configured_plugin_ids(
        configured_plugins_from_lyra_home(
            lyra_home,
            "failed to read user config while refreshing plugin cache",
            "failed to parse user config while refreshing plugin cache",
        ),
        "ignoring invalid plugin key during plugin cache refresh setup",
    );
    if configured_plugin_ids.is_empty() {
        return Ok(false);
    }
    let configured_plugin_keys = configured_plugin_ids
        .iter()
        .map(PluginId::as_key)
        .collect::<HashSet<_>>();

    let store = PluginStore::try_new(lyra_home.to_path_buf()).map_err(|err| err.to_string())?;
    ...
    for marketplace in marketplace_outcome.marketplaces {
        for plugin in marketplace.plugins {
            ...
            if !configured_plugin_keys.contains(&plugin_key) {
                continue;
            }
            if plugin_sources.contains_key(&plugin_key) {
                warn!(
                    ...
                    "ignoring duplicate plugin entry during cache refresh"
                );
                continue;
            }
    ...
    for plugin_id in configured_plugin_ids {
        let plugin_key = plugin_id.as_key();
        let Some(source) = plugin_sources.get(&plugin_key).cloned() else {
            warn!(
                ...
                "configured plugin no longer exists in discovered marketplaces during cache refresh"
            );
            continue;
        };
        let materialized =
            materialize_marketplace_plugin_source(lyra_home, &source).map_err(|err| {
...
fn configured_plugins_from_lyra_home(
    lyra_home: &Path,
...
pub async fn installed_plugin_telemetry_metadata(
    ...
    let store = match PluginStore::try_new(lyra_home.to_path_buf()) {
        Ok(store) => store,
        Err(err) => {
...
fn normalize_plugin_mcp_server_value(
        ...
        && oauth.contains_key("callbackPort")
    {
        warn!(
            plugin = %plugin_root.display(),
            "plugin MCP server OAuth callbackPort is ignored; Lyra uses global MCP OAuth callback settings"
        );
    }
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = lyra_home.join("plugins/.marketplace-plugin-source-staging");
...
mod tests {
    ...
    fn materialize_git_subdir_uses_sparse_checkout() {
        let lyra_home = tempfile::tempdir().expect("create Lyra home");
