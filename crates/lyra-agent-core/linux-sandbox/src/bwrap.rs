...
//! - sensitive subpaths such as `.git` and `.lyra` remain read-only even when
...
use lyra_protocol::protocol::FileSystemSandboxPolicy;
use lyra_protocol::protocol::WritableRoot;
    ...
    for (search_root, globs) in patterns_by_search_root {
        for path in ripgrep_files(search_root.as_path(), &globs, max_depth)? {
            ...
            if expanded_paths.len() > MAX_UNREADABLE_GLOB_MATCHES {
                return Err(LyraErr::Fatal(format!(
...
mod tests {
    use super::*;
    use lyra_protocol::protocol::FileSystemAccessMode;
    use lyra_protocol::protocol::FileSystemPath;
    use lyra_protocol::protocol::FileSystemSandboxEntry;
    use lyra_protocol::protocol::FileSystemSandboxPolicy;
    use lyra_protocol::protocol::FileSystemSpecialPath;
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_lyra = temp_dir.path().join("real-lyra");
        let logical_lyra = logical_home.join(".lyra");
        let real_memories = real_lyra.join("memories");
        let logical_memories = logical_lyra.join("memories");
        ...
        std::os::unix::fs::symlink(&real_lyra, &logical_lyra).expect("create symlinked Lyra home");
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/".to_string(),
                "/".to_string(),
                // Mask the default protected .lyra subpath under that writable
