...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=LYRA_BWRAP_SOURCE_DIR");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_ALLOW_CROSS");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_PATH");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_SYSROOT_DIR");
    println!("cargo:rerun-if-env-changed=LYRA_SKIP_VENDORED_BWRAP");

    // Rebuild if the vendored bwrap sources change.
    let manifest_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap_or_default());
    let vendor_dir = default_bwrap_source_dir(&manifest_dir);
    println!(
    ...
    if target_os != "linux" || env::var_os("LYRA_SKIP_VENDORED_BWRAP").is_some() {
        return;
    }
...
fn try_build_vendored_bwrap() -> Result<(), String> {
    ...
    std::fs::write(
        &config_h,
        r#"#pragma once
#define PACKAGE_STRING "bubblewrap built at lyra build-time"
"#,
    )
...
///
/// Priority:
/// 1. `LYRA_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
/// 2. The vendored bubblewrap tree under `third-party/native/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("LYRA_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "LYRA_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set LYRA_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under third-party/native.",
        vendor_dir.display()
    ))
}

fn default_bwrap_source_dir(manifest_dir: &Path) -> PathBuf {
    manifest_dir.join("../../../third-party/native/bubblewrap")
}
