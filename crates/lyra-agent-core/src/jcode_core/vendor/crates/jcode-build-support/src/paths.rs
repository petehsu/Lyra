...
pub fn selfdev_build_command_for_target(
    ...
    if is_lyra_repo(repo_dir) {
        return lyra_selfdev_build_command_for_target(repo_dir, target);
    }
...
fn lyra_selfdev_build_command_for_target(
    repo_dir: &Path,
    target: SelfDevBuildTarget,
) -> SelfDevBuildCommand {
    let target = match target {
        SelfDevBuildTarget::Auto => infer_lyra_selfdev_build_target(repo_dir),
        explicit => explicit,
    };
    let commands = match target {
        SelfDevBuildTarget::Tui => vec![
            "cargo test -p lyra-agent-core lyra_runtime -- --nocapture".to_string(),
            "cargo test -p lyrad".to_string(),
        ],
        SelfDevBuildTarget::Desktop => {
            vec!["npm --prefix apps/desktop run typecheck".to_string()]
        }
        SelfDevBuildTarget::All | SelfDevBuildTarget::Auto => vec![
            "cargo test -p lyra-agent-core lyra_runtime -- --nocapture".to_string(),
            "cargo test -p lyrad".to_string(),
            "npm --prefix apps/desktop run typecheck".to_string(),
        ],
    };
...
fn infer_lyra_selfdev_build_target(repo_dir: &Path) -> SelfDevBuildTarget {
    let output = Command::new("git")
        .args(["status", "--porcelain=v1", "--untracked-files=all"])
        .current_dir(repo_dir)
        .output();
    ...
    for line in text.lines() {
        ...
        {
            desktop = true;
        } else if path.starts_with("crates/") || path == "Cargo.toml" || path == "Cargo.lock" {
            rust = true;
        } else if !path.is_empty() {
            desktop = true;
            rust = true;
    ...
    match (desktop, rust) {
        ...
        (false, true) => SelfDevBuildTarget::Tui,
        (true, true) => SelfDevBuildTarget::All,
        (false, false) => SelfDevBuildTarget::All,
    }
}
...
fn is_lyra_repo(dir: &Path) -> bool {
    let cargo_toml = dir.join("Cargo.toml");
    cargo_toml.is_file()
        && dir.join(".git").exists()
        && dir.join("crates").join("lyra-agent-core").is_dir()
        && dir
            .join("apps")
            .join("desktop")
            .join("package.json")
