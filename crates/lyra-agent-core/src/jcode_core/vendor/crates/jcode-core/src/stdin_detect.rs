...
pub fn is_waiting_for_stdin(pid: u32) -> StdinState {
    ...
    #[cfg(target_os = "macos")]
    return macos::check_process_tree(pid);
...
mod macos {
    ...
    pub fn check_process_tree(pid: u32) -> StdinState {
        ...
        let num_pids = ret as usize / 4;
        let short_info_size = mem::size_of::<proc_bsdshortinfo>() as i32;

        for i in 0..num_pids {
            let child_pid = pids[i];
            if child_pid <= 0 || child_pid as u32 == pid {
                continue;
            }
            ...
            if info_ret == short_info_size && info.pbsi_ppid == pid {
                let child_result = check_process_tree(child_pid as u32);
                if child_result == StdinState::Reading {
                    return StdinState::Reading;
                }
            }
