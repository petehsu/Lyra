...
const LYRA_AGENT_HOME_ENV: &str = "LYRA_AGENT_HOME";
const JCODE_HOME_ENV: &str = "JCODE_HOME";
const LYRA_AGENT_RUNTIME_DIR_ENV: &str = "LYRA_AGENT_RUNTIME_DIR";
const JCODE_RUNTIME_DIR_ENV: &str = "JCODE_RUNTIME_DIR";
...
///
/// Can be overridden with `$LYRA_AGENT_RUNTIME_DIR`, with `$JCODE_RUNTIME_DIR`
/// retained as a compatibility fallback.
pub fn runtime_dir() -> PathBuf {
    if let Some(dir) = env_path(LYRA_AGENT_RUNTIME_DIR_ENV, JCODE_RUNTIME_DIR_ENV) {
        return dir;
    }
...
pub fn jcode_dir() -> Result<PathBuf> {
    if let Some(path) = env_path(LYRA_AGENT_HOME_ENV, JCODE_HOME_ENV) {
        return Ok(path);
    }
...
/// Resolve the app-owned Agent config directory.
///
/// Default location is the platform config dir + `jcode` for upstream
/// compatibility. When `LYRA_AGENT_HOME` is set, sandbox this under
/// `$LYRA_AGENT_HOME/config/lyra-agent`; the compatibility `JCODE_HOME` fallback
/// keeps the legacy `$JCODE_HOME/config/jcode` layout.
pub fn app_config_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var(LYRA_AGENT_HOME_ENV)
        && !path.trim().is_empty()
    {
        return Ok(PathBuf::from(path).join("config").join("lyra-agent"));
    }
    if let Ok(path) = std::env::var(JCODE_HOME_ENV)
        && !path.trim().is_empty()
    {
...
/// `$LYRA_AGENT_HOME/external/` when set, with `$JCODE_HOME` as a fallback.
///
