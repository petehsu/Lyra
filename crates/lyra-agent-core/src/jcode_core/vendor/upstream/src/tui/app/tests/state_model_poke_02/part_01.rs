...
#[test]
fn test_registered_command_suggestions_include_canonical_commands_and_hide_aliases() {
    let app = create_test_app();
    ...
    assert!(commands.contains(&"/splitview"));
    assert!(commands.contains(&"/account"));
    assert!(!commands.contains(&"/?"));
    assert!(!commands.contains(&"/commands"));
    assert!(!commands.contains(&"/models"));
    assert!(!commands.contains(&"/sessions"));
    assert!(!commands.contains(&"/split-view"));
    assert!(!commands.contains(&"/accounts"));
    assert!(!commands.contains(&"/changelog"));
    assert!(!commands.contains(&"/reload"));
    assert!(!commands.contains(&"/restart"));
    assert!(!commands.contains(&"/rebuild"));
    assert!(!commands.contains(&"/update"));
    assert!(!commands.contains(&"/quit"));
    assert!(!commands.contains(&"/debug-visual"));
    assert!(!commands.contains(&"/screenshot-mode"));
    assert!(!commands.contains(&"/screenshot"));
    assert!(!commands.contains(&"/record"));
    assert!(!commands.contains(&"/client-reload"));
    assert!(!commands.contains(&"/server-reload"));
    assert!(!commands.contains(&"/z"));
    assert!(!commands.contains(&"/zz"));
    assert!(!commands.contains(&"/zzz"));
    assert!(!commands.contains(&"/zstatus"));
}
...
#[test]
fn test_top_level_command_suggestions_include_config() {
    let app = create_test_app();
...
#[test]
fn test_top_level_command_suggestions_include_git_but_not_transcript() {
    let app = create_test_app();
    ...
    assert!(suggestions.iter().all(|(cmd, _)| cmd != "/transcript"));
}

#[test]
fn test_transcript_command_suggestions_are_removed() {
    let app = create_test_app();

    let suggestions = app.get_suggestions_for("/transcript p");

    assert!(suggestions.iter().all(|(cmd, _)| cmd != "/transcript path"));
}
...
fn test_nested_command_suggestions_filter_partial_suffixes() {
    ...
    assert!(
        suggestions.iter().all(|(cmd, _)| !cmd.starts_with("/memory")),
        "memory is an internal default capability, not a slash command"
    );
