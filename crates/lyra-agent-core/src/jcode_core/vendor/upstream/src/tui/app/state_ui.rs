...
fn format_cache_stats(app: &App) -> String {
    ...
    lines.push("Raw session/cache diagnostic state for this client. Cache usage is provider-reported when available.".to_string());
    lines.push(String::new());
    ...
    lines.push("**Provider cache usage totals**".to_string());
    lines.push(format!(
