...
fn test_handle_server_event_transcript_send_prefixes_user_message() {
    ...
    app.handle_server_event(
        crate::protocol::ServerEvent::Transcript {
            text: "transcript hello".to_string(),
    ...
    assert_eq!(last.content, "[transcription] transcript hello");
    assert!(app.messages.is_empty());
    assert!(matches!(
        app.session.messages.last().and_then(|message| message.content.last()),
        Some(crate::message::ContentBlock::Text { text, .. }) if text == "[transcription] transcript hello"
    ));
    assert!(
        app.pending_turn,
