...
pub trait TuiState {
    ...
    /// Usage overlay for provider limit summaries
...
/// - explicit provider cache stats showing either a cache rewrite without a read, or an explicit
...
pub(crate) fn detect_kv_cache_problem(
    ...
    // Read-only cache providers (OpenAI/Gemini and known OpenRouter upstreams) do not expose
