...
impl App {
    ...
    pub(super) fn handle_turn_error(&mut self, error: impl Into<String>) {
        ...
        if is_context_limit_error(&error) {
            ...
            let hint = match recovery {
                Some(msg) => format!(" {}", msg),
                None => " Context limit exceeded and automatic recovery did not succeed. Use retry/recover from the GUI error state.".to_string(),
            };
            ...
            self.push_display_message(DisplayMessage::error(format!(
                "Error: {} Use retry/recover from the GUI error state.",
                error
            )));
            super::commands::stop_auto_poke_for_non_retryable_error(self, &error);
            self.stop_overnight_auto_poke_for_non_retryable_error(&error);
    ...
    fn format_usage_display_card(
        ...
        if refreshing {
            ...
            lines.push("Use `/login claude` or `/login openai`, then refresh usage.".to_string());
            return lines.join("\n");
        } else {
