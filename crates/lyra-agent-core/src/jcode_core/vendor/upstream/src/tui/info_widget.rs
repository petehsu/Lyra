...
/// Session-level KV cache usage for providers that report cache usage.
#[derive(Debug, Default, Clone, PartialEq, Eq)]
pub struct CacheHitInfo {
    /// Input tokens from completed API requests that included explicit cache usage.
    ...
    /// Input tokens from the latest completed request with cache usage.
    pub last_reported_input_tokens: Option<u64>,
    /// Cached input tokens read on the latest completed request with cache usage.
...
pub struct InfoWidgetData {
    ...
    /// Session-level cache read ratio, when the active provider reports cache usage.
