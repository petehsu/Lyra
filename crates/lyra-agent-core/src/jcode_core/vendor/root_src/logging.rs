//! Logging infrastructure for Lyra Agent
//!
//! Logs to the Lyra Agent module data directory with automatic rotation
//!
...
impl Logger {
    fn new() -> Option<Self> {
        ...
        let path = log_dir.join(format!("lyra-agent-{}.log", date));
    ...
    fn write(&mut self, level: &str, message: &str) {
        ...
        if let Err(err) = self.file.write_all(line.as_bytes()) {
            eprintln!("Lyra Agent logger write failed: {err}");
            return;
        }
        if let Err(err) = self.file.flush() {
            eprintln!("Lyra Agent logger flush failed: {err}");
        }
    }
}
...
pub fn log_path() -> Option<PathBuf> {
    let log_dir = log_dir()?;
    let date = Local::now().format("%Y-%m-%d");
    Some(log_dir.join(format!("lyra-agent-{}.log", date)))
}
        ...
        && let Ok(entries) = fs::read_dir(&log_dir)
    {
        ...
        for entry in entries.flatten() {
                ...
                && let Ok(modified) = metadata.modified()
            {
                ...
                {
                    eprintln!("Lyra Agent logger cleanup failed: {err}");
                }
            }
        }
    }
