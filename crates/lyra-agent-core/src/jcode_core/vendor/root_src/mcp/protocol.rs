...
impl McpConfig {
    ...
    /// Only runs if the Lyra Agent MCP config doesn't exist yet.
    #[expect(
    ...
    /// Load from default locations (merges Lyra Agent global + local,
    /// compatibility local config overrides)
    #[expect(
        clippy::collapsible_if,
    ...
    pub fn load() -> Self {
        ...
        // Load Lyra Agent's own global config.
        ...
        // Load project-local Lyra Agent config.
        let local_lyra = std::path::Path::new(".lyra-agent/mcp.json");
        if local_lyra.exists() {
            if let Ok(config) = Self::load_from_file(local_lyra) {
                merged.servers.extend(config.servers);
            }
        }

        // Load legacy project-local jcode config for compatibility.
