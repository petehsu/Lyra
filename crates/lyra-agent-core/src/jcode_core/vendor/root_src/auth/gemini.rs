...
pub fn load_tokens() -> Result<GeminiTokens> {
    ...
    anyhow::bail!("No Gemini OAuth tokens found. Sign in again from Lyra Agent settings.")
}
...
pub async fn login(no_browser: bool) -> Result<GeminiTokens> {
        ...
        && let Ok(listener) = super::oauth::bind_callback_listener(0)
    {
        ...
        if browser_opened {
            ...
            eprintln!(
                "If the page says sign-in succeeded but Lyra Agent does not continue within a few seconds, press Ctrl+C and retry with the manual code flow."
            );
