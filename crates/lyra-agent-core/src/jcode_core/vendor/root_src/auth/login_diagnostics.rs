...
pub fn auth_failure_recovery_hint(provider_id: &str, reason: AuthFailureReason) -> Option<String> {
    ...
    let hint = match reason {
        ...
        | AuthFailureReason::CallbackPortUnavailable
        | AuthFailureReason::NonInteractiveTerminal => {
            "Try the manual callback flow from Lyra Agent settings.".to_string()
        }
        AuthFailureReason::ManualInputMissing => {
            "Retry the same flow and paste the full callback URL, authorization code, or required API key when prompted.".to_string()
        }
        AuthFailureReason::SaveFailed => {
            "Check whether Lyra Agent can write its config directory, then retry sign-in.".to_string()
        }
        AuthFailureReason::PostLoginValidationFailed => format!(
            "Credentials were saved, but runtime verification failed. Re-run validation for {} from Lyra Agent settings.",
            provider
        ),
        AuthFailureReason::ImportUnavailable => {
        ...
        AuthFailureReason::RateLimited => format!(
            "The provider accepted the browser callback but rate-limited the token exchange. Wait before retrying, avoid repeated immediate attempts, and keep using existing credentials if they still validate in Lyra Agent settings for {}.",
            provider
        ),
        AuthFailureReason::OAuthExchangeFailed => format!(
            "Retry the OAuth flow for {}, and if it keeps failing use the manual callback flow in Lyra Agent settings.",
            provider
        ),
        AuthFailureReason::Unknown => {
            "Review auth status and diagnostics in Lyra Agent settings.".to_string()
        }
    };
