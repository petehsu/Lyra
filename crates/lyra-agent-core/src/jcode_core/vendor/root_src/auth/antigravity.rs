...
pub fn load_tokens() -> Result<AntigravityTokens> {
    ...
    if path.exists() {
        ...
        return crate::storage::read_json(&path).map_err(|_| {
            anyhow::anyhow!("No Antigravity tokens found. Sign in again from Lyra Agent settings.")
        });
    }
    ...
    anyhow::bail!("No Antigravity tokens found. Sign in again from Lyra Agent settings.");
}
...
pub async fn login(no_browser: bool) -> Result<AntigravityTokens> {
        ...
        && let Ok(listener) = crate::auth::oauth::bind_callback_listener(DEFAULT_PORT)
    {
        ...
        if browser_opened {
            ...
            eprintln!(
                "If the browser lands on a loopback error page instead of returning to Lyra Agent, copy the full URL from the address bar and retry with manual callback paste."
            );
...
async fn manual_login(
    ...
    eprintln!(
        "After approving access, paste the full callback URL (or query string) here so Lyra Agent can verify the login state.\n"
    );
    eprintln!(
