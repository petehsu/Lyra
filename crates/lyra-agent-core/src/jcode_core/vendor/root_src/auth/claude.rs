...
/// Multi-account Lyra Agent auth format.
...
/// Read the Lyra Agent auth file, auto-migrating from legacy format if needed.
pub fn load_auth_file() -> Result<JcodeAuthFile> {
    ...
    let mut auth: JcodeAuthFile = crate::storage::read_json(&path)
        .with_context(|| format!("Could not read Lyra Agent credentials from {:?}", path))?;
...
/// Write the Lyra Agent auth file (multi-account format).
...
pub fn load_credentials() -> Result<ClaudeCredentials> {
    ...
    anyhow::bail!("No Claude OAuth credentials found (checked Claude Code, Lyra Agent, OpenCode)")
}

/// Load credentials for a specific Lyra Agent account by label.
...
/// Load credentials from the active Lyra Agent account (multi-account aware).
fn load_jcode_credentials() -> Result<ClaudeCredentials> {
    let auth = load_auth_file()?;
    if auth.anthropic_accounts.is_empty() {
        anyhow::bail!("No anthropic accounts configured in Lyra Agent auth");
    }
    ...
    let account = auth
        ...
        .context("No anthropic accounts in Lyra Agent auth")?;
