            ...
            crate::provider_catalog::LoginProviderTarget::OpenRouter => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        "OPENROUTER_API_KEY",
                        "openrouter.env",
                        "Lyra Agent config/openrouter.env",
                    ),
            ...
            crate::provider_catalog::LoginProviderTarget::Azure => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        ...
                        "Lyra Agent config/azure-openai.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::OpenAiCompatible(profile) => {
                ...
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        &resolved.api_key_env,
                        &resolved.env_file,
                        format!("Lyra Agent config/{}", resolved.env_file),
                    ),
...
fn env_source(env_key: &str) -> Option<(AuthCredentialSource, String)> {
    lyra_first_env_keys(env_key)
        .into_iter()
        .find(|key| env_var_nonempty(key))
        .map(|key| {
            (
...
fn config_source(
    ...
    lyra_first_env_keys(env_key)
        .into_iter()
        .find(|key| config_file_has_key(file_name, key))
        .map(|key| {
            (
                AuthCredentialSource::AppConfigFile,
                format!("{} ({key})", path_label.into()),
            )
        })
}
...
fn anthropic_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
        ...
        .is_empty()
    {
        return Some((
            ...
            "~/.lyra/modules/agent/auth.json".to_string(),
        ));
    }
...
fn openai_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
        ...
        .is_empty()
    {
        return Some((
            AuthCredentialSource::JcodeManagedFile,
            "~/.lyra/modules/agent/openai-auth.json".to_string(),
        ));
    }
...
fn lyra_first_env_keys(env_key: &str) -> Vec<&str> {
    match env_key {
        "OPENAI_API_KEY" => vec!["LYRA_AGENT_OPENAI_API_KEY", env_key],
        "OPENROUTER_API_KEY" => vec!["LYRA_AGENT_OPENROUTER_API_KEY", env_key],
        "ANTHROPIC_API_KEY" => vec!["LYRA_AGENT_ANTHROPIC_API_KEY", env_key],
        "CURSOR_API_KEY" => vec!["LYRA_AGENT_CURSOR_API_KEY", env_key],
        "COPILOT_GITHUB_TOKEN" => vec!["LYRA_AGENT_COPILOT_API_KEY", env_key],
        _ => vec![env_key],
    }
}
