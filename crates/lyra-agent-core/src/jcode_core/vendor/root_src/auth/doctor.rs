...
pub fn diagnostics(
    ...
    match assessment.state {
        AuthState::NotConfigured => diagnostics.push(format!(
            "{} is not configured for Lyra Agent yet.",
...
pub fn recommended_actions(
    ...
    match assessment.state {
        AuthState::NotConfigured => actions.push(format!(
            "Connect {} from Lyra Agent settings.",
            provider.display_name
        )),
        AuthState::Expired
            if matches!(
        ...
        {
            actions.push(format!(
                "Sign in to {} again from Lyra Agent settings; this provider cannot auto-refresh.",
                provider.display_name
            ));
        }
        AuthState::Expired => actions.push(format!(
            "Refresh or replace the current {} login from Lyra Agent settings.",
            provider.display_name
        )),
        AuthState::Available => {}
    }
        ...
        .and_then(|record| record.last_error.as_deref())
    {
        ...
        if lower.contains("invalid_grant") || lower.contains("refresh token") {
            actions.push(format!(
                "Replace the stale {} OAuth account/token from Lyra Agent settings.",
                provider.display_name
            ));
            ...
            || lower.contains("too many requests")
        {
            ...
            actions.push(format!(
                "Retry credential refresh by running validation again for {} in Lyra Agent settings.",
                provider.display_name
            ));
        }
    }
    ...
    if assessment.state != AuthState::NotConfigured {
        match assessment.last_validation.as_ref() {
            None => actions.push(format!(
                "Run runtime verification for {} from Lyra Agent settings.",
                provider.display_name
            )),
            Some(record) if !record.success => actions.push(format!(
                "Inspect runtime readiness for {} from Lyra Agent settings.",
                provider.display_name
            )),
            Some(record) if validation_is_stale(record.checked_at_ms) => actions.push(format!(
                "Refresh stale runtime verification for {} from Lyra Agent settings.",
                provider.display_name
            )),
            Some(_) => {}
        }
    ...
    if validation_result.is_some_and(|value| value != "validation passed") {
        actions.push(format!(
            "Re-run detailed auth diagnostics for {} from Lyra Agent settings.",
            provider.display_name
        ));
    }
    ...
    {
        actions.push(format!(
            "For browser/callback issues, use the manual callback flow for {} in Lyra Agent settings.",
            provider.display_name
        ));
    }

    actions.push("Review current auth state in Lyra Agent settings.".to_string());
    actions.dedup();
    actions
}
...
mod tests {
    ...
    fn base_assessment() -> ProviderAuthAssessment {
        ProviderAuthAssessment {
            ...
            credential_source_detail: "~/.lyra/modules/agent/auth.json".to_string(),
