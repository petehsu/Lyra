...
impl Agent {
    ...
    pub fn mark_session_saved(&mut self, label: Option<String>) -> Result<()> {
        self.session.mark_saved(label);
        self.log_env_snapshot("save_session");
        self.session.save()?;
        Ok(())
    }

    pub fn unmark_session_saved(&mut self) -> Result<()> {
        self.session.unmark_saved();
        self.log_env_snapshot("unsave_session");
        self.session.save()?;
        Ok(())
    }

    pub fn set_session_archived(&mut self, archived: bool) -> Result<()> {
        self.session.set_archived(archived);
        self.log_env_snapshot("archive_session");
        self.session.save()?;
        Ok(())
    }
