...
impl NotificationDispatcher {
    ...
    pub fn dispatch_permission_request(&self, action: &str, description: &str, request_id: &str) {
        let title = format!("Lyra Agent: permission needed ({})", action);
        let safe_body =
            "An ambient action needs your approval. Open Lyra Agent to review.".to_string();
        let detailed_body = format!(
            "Action: {}\n{}\n\nRequest ID: {}\nReview in Lyra Agent to approve or deny.",
        ...
        let reply_to = self
            ...
            .email_from
            .as_deref()
            .unwrap_or("lyra-agent@localhost");
...
fn format_cycle_body_safe(transcript: &AmbientTranscript) -> String {
    ...
    lines.push("Check Lyra Agent for full details.".to_string());
    lines.join("\n")
}
...
fn format_cycle_body_detailed(transcript: &AmbientTranscript) -> String {
    ...
    if transcript.pending_permissions > 0 {
        lines.push(String::new());
        lines.push(format!(
            "**⚠ {} permission request(s) pending** — review in Lyra Agent",
            transcript.pending_permissions
        ));
    }
...
mod tests {
    ...
    fn test_format_cycle_body_safe() {
        ...
        assert!(body.contains("Check Lyra Agent for full details"));
    ...
    fn test_format_cycle_body_with_pending_permissions() {
        ...
        assert!(safe.contains("Check Lyra Agent for full details"));
