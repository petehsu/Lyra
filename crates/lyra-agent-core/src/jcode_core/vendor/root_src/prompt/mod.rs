...
/// Default system prompt for Lyra Agent (embedded at compile time)
pub const DEFAULT_SYSTEM_PROMPT: &str = include_str!("system_prompt.md");
const SELFDEV_HINT_PROMPT: &str = include_str!("selfdev_hint.txt");
const SELFDEV_MODE_PROMPT: &str = include_str!("selfdev_mode.txt");
...
pub fn build_system_prompt_full(
    ...
    // Add optional prompt overlays from Lyra Agent storage and the project.
...
/// Load optional prompt overlay markdown from Lyra Agent storage and the project.
fn load_prompt_overlay_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".lyra-agent").join("prompt-overlay.md"),
    ...
    if let Ok(global_overlay) = crate::storage::jcode_dir().map(|dir| dir.join("prompt-overlay.md"))
        && let Some((content, size)) = load_file(&global_overlay, "Global Prompt Overlay")
    {
        total_chars += size;
        contents.push(content);
    }
