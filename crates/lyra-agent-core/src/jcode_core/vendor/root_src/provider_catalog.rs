...
fn inline_key_env_name(profile_name: &str) -> String {
    let suffix = profile_name
        ...
        .collect::<String>();
    format!("LYRA_AGENT_PROVIDER_{}_API_KEY", suffix)
}
...
pub fn load_api_key_from_env_or_config(env_key: &str, file_name: &str) -> Option<String> {
    ...
    for key_name in lyra_first_env_keys(env_key) {
...
fn lyra_first_env_keys(env_key: &str) -> Vec<String> {
    let mut keys = Vec::with_capacity(2);
    if let Some(alias) = lyra_agent_api_key_alias(env_key)
        && alias != env_key
    {
        keys.push(alias);
    }
...
fn lyra_agent_api_key_alias(env_key: &str) -> Option<String> {
    if !env_key.ends_with("_API_KEY") {
        return None;
    }
    if let Some(suffix) = env_key.strip_prefix("JCODE_") {
        return Some(format!("LYRA_AGENT_{}", suffix));
    }
    if env_key.starts_with("LYRA_AGENT_") {
        return None;
    }
    Some(format!("LYRA_AGENT_{}", env_key))
}
