...
fn test_load_agents_md_files_uses_sandboxed_global_files() {
    ...
    if let Some(prev_home) = prev_home {
        crate::env::set_var("LYRA_AGENT_HOME", prev_home);
    } else {
...
fn test_session_context_includes_time_timezone_and_system_info() {
    ...
    assert!(context.contains("Lyra Agent core version: "));
}
...
#[test]
fn test_prompt_overlay_files_are_loaded_from_project_and_global_agent_dirs() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("LYRA_AGENT_HOME");
    let temp = tempfile::TempDir::new().unwrap();
    crate::env::set_var("LYRA_AGENT_HOME", temp.path());
    ...
    let project_dir = tempfile::TempDir::new().unwrap();
    std::fs::create_dir_all(project_dir.path().join(".lyra-agent")).unwrap();
    std::fs::write(
        project_dir.path().join(".lyra-agent/prompt-overlay.md"),
