...
fn user_home_path_uses_external_dir_under_jcode_home() {
    ...
    restore_env_var("LYRA_AGENT_HOME", prev_lyra_home);
    restore_env_var("JCODE_HOME", prev_home);
}

#[test]
fn lyra_agent_home_takes_precedence_over_jcode_home() {
    let _guard = lock_test_env();
    let prev_lyra_home = std::env::var_os("LYRA_AGENT_HOME");
    let prev_home = std::env::var_os("JCODE_HOME");
    let lyra_home = tempfile::TempDir::new().expect("create lyra agent home");
    let legacy_home = tempfile::TempDir::new().expect("create legacy jcode home");
    crate::env::set_var("LYRA_AGENT_HOME", lyra_home.path());
    crate::env::set_var("JCODE_HOME", legacy_home.path());

    assert_eq!(jcode_dir().expect("resolve agent home"), lyra_home.path());
    assert_eq!(
        app_config_dir().expect("resolve app config dir"),
        lyra_home.path().join("config").join("lyra-agent")
    );
...
#[test]
fn runtime_dir_uses_lyra_alias_before_jcode_fallback() {
    let _guard = lock_test_env();
    let prev_lyra_runtime = std::env::var_os("LYRA_AGENT_RUNTIME_DIR");
    let prev_runtime = std::env::var_os("JCODE_RUNTIME_DIR");
    let lyra_runtime = tempfile::TempDir::new().expect("create lyra runtime");
    let legacy_runtime = tempfile::TempDir::new().expect("create legacy runtime");
    crate::env::set_var("LYRA_AGENT_RUNTIME_DIR", lyra_runtime.path());
    crate::env::set_var("JCODE_RUNTIME_DIR", legacy_runtime.path());

    assert_eq!(runtime_dir(), lyra_runtime.path());

    restore_env_var("LYRA_AGENT_RUNTIME_DIR", prev_lyra_runtime);
    restore_env_var("JCODE_RUNTIME_DIR", prev_runtime);
}
