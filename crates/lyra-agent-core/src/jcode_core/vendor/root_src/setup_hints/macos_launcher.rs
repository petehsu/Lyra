...
fn macos_launcher_script(terminal: MacTerminalKind, exe_path: &str, app_dir: &Path) -> String {
    ...
    let missing_message = escape_applescript_text(&format!(
        "Lyra Agent could not launch because the executable was not found.\n\nExpected path:\n{}\n\nTry reinstalling Lyra.",
        exe_path
    ));
    let terminal_failure_message = escape_applescript_text(&format!(
        "Lyra Agent could not open {}.\n\nTry reinstalling Lyra.\n\nLauncher log:\n~/.lyra/modules/agent/launcher/macos-launcher.log",
        terminal.label()
    ));
