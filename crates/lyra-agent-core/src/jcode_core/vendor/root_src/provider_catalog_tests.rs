...
#[test]
fn api_key_loader_prefers_lyra_agent_env_aliases() {
    let _lock = crate::storage::lock_test_env();
    let _guard = EnvGuard::save(&[
        "LYRA_AGENT_OPENAI_API_KEY",
        "OPENAI_API_KEY",
        "LYRA_AGENT_PROVIDER_MY_API_KEY",
        "JCODE_PROVIDER_MY_API_KEY",
    ]);
    crate::env::set_var("LYRA_AGENT_OPENAI_API_KEY", "lyra-openai");
    crate::env::set_var("OPENAI_API_KEY", "legacy-openai");
    assert_eq!(
        load_api_key_from_env_or_config("OPENAI_API_KEY", "missing.env").as_deref(),
        Some("lyra-openai")
    );

    crate::env::remove_var("LYRA_AGENT_OPENAI_API_KEY");
    assert_eq!(
        load_api_key_from_env_or_config("OPENAI_API_KEY", "missing.env").as_deref(),
        Some("legacy-openai")
    );

    crate::env::set_var("LYRA_AGENT_PROVIDER_MY_API_KEY", "lyra-profile");
    crate::env::set_var("JCODE_PROVIDER_MY_API_KEY", "legacy-profile");
    assert_eq!(
        load_api_key_from_env_or_config("JCODE_PROVIDER_MY_API_KEY", "missing.env").as_deref(),
        Some("lyra-profile")
    );
}
...
fn named_provider_inline_api_key_is_private_runtime_fallback() {
    ...
    let _guard = EnvGuard::save(&[
        ...
        "JCODE_OPENROUTER_MODEL_CATALOG",
        "JCODE_NAMED_PROVIDER_PROFILE",
        "LYRA_AGENT_PROVIDER_MY_GATEWAY_API_KEY",
    ]);
    ...
    assert_eq!(
        std::env::var("JCODE_OPENROUTER_API_KEY_NAME")
            .ok()
            .as_deref(),
        Some("LYRA_AGENT_PROVIDER_MY_GATEWAY_API_KEY")
    );
    assert_eq!(
        std::env::var("LYRA_AGENT_PROVIDER_MY_GATEWAY_API_KEY")
            .ok()
            .as_deref(),
        Some("inline-secret")
    );
