...
pub struct Session {
    ...
    /// Whether this session is archived in GUI/session picker surfaces.
    #[serde(default)]
    pub archived: bool,
...
impl Session {
    fn session_from_startup_stub(stub: SessionStartupStub) -> Self {
        ...
        session.archived = stub.archived;
    ...
    fn session_from_remote_startup_snapshot(snapshot: RemoteStartupSessionSnapshot) -> Self {
        ...
        session.archived = snapshot.archived;
        session.replay_events.clear();
        session.env_snapshots.clear();
        session.memory_injections.clear();
    ...
    /// Set or clear archived state for history surfaces.
