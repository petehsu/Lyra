...
const KILLALL_PROCESS_NAME: &str = "lyra-agent";
...
pub(crate) fn set_server_title(server_name: &str) {
    set_title(compact_process_title("lyra:s:", Some(server_name)));
}
...
pub(crate) fn set_client_display_title(session_name: &str, is_selfdev: bool) {
    let prefix = if is_selfdev { "lyra:d:" } else { "lyra:c:" };
...
pub(crate) fn set_client_remote_display_title(
    ...
    if server_name.is_empty()
        ...
        || server_name.eq_ignore_ascii_case("lyra")
    {
...
pub(crate) fn initial_title(args: &Args) -> String {
    match &args.command {
        Some(Command::Serve { .. }) => "lyra:server".to_string(),
        Some(Command::Connect) => "lyra:client".to_string(),
        Some(Command::Run { .. }) => "lyra-agent run".to_string(),
        Some(Command::Login { .. }) => "lyra-agent login".to_string(),
        Some(Command::Repl) => "lyra-agent repl".to_string(),
        Some(Command::Update) => "lyra-agent update".to_string(),
        Some(Command::Version { .. }) => "lyra-agent version".to_string(),
        Some(Command::Usage { .. }) => "lyra-agent usage".to_string(),
        Some(Command::SelfDev { .. }) => "lyra:selfdev".to_string(),
        Some(Command::Debug { .. }) => "lyra-agent debug".to_string(),
        Some(Command::Auth(_)) => "lyra-agent auth".to_string(),
        Some(Command::Provider(_)) => "lyra-agent provider".to_string(),
        Some(Command::Memory(_)) => "lyra-agent memory".to_string(),
        Some(Command::Session(_)) => "lyra-agent session".to_string(),
        Some(Command::Ambient(subcommand)) => match subcommand {
            AmbientCommand::RunVisible => "lyra-agent ambient visible".to_string(),
            _ => "lyra-agent ambient".to_string(),
        },
        Some(Command::Pair { .. }) => "lyra-agent pair".to_string(),
        Some(Command::Permissions) => "lyra-agent permissions".to_string(),
        Some(Command::Transcript { .. }) => "lyra-agent transcript".to_string(),
        Some(Command::SetupHotkey {
            listen_macos_hotkey,
        }) => {
            if *listen_macos_hotkey {
                "lyra-agent hotkey listener".to_string()
            } else {
                "lyra-agent hotkey setup".to_string()
            }
        }
        Some(Command::Replay { .. }) => "lyra-agent replay".to_string(),
        Some(Command::Model(_)) => "lyra-agent model".to_string(),
        Some(Command::AuthTest { .. }) => "lyra-agent auth-test".to_string(),
        Some(Command::Restart { .. }) => "lyra-agent restart".to_string(),
        Some(Command::SetupLauncher) => "lyra-agent setup-launcher".to_string(),
        None => {
            if let Some(resume) = args.resume.as_deref().filter(|resume| !resume.is_empty()) {
                ...
                "lyra:selfdev".to_string()
            } else {
                "lyra:client".to_string()
            }
        }
    }
}
...
mod tests {
    ...
    fn initial_title_labels_server() {
        with_selfdev_env_removed(|| {
            ...
            assert_eq!(initial_title(&args), "lyra:server");
        });
    }
    ...
    fn initial_title_labels_resume_client_with_short_name() {
        with_selfdev_env_removed(|| {
            ...
            assert_eq!(initial_title(&args), "lyra:c:fox");
        });
    }
    ...
    fn initial_title_labels_selfdev_command() {
        with_selfdev_env_removed(|| {
            ...
            assert_eq!(initial_title(&args), "lyra:selfdev");
        });
    }
}
