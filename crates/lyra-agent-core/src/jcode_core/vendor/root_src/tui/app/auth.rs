        ...
        match provider.target {
            ...
            crate::provider_catalog::LoginProviderTarget::Google => {
                self.push_display_message(DisplayMessage::error(
                    "Google/Gmail login is available from Lyra Agent settings."
                        .to_string(),
                ));
            }
        }
                ...
                match save_result {
                    Ok(()) => {
                        ...
                        let guidance = if let Some(resolved) = resolved_openai_compatible.as_ref() {
                            ...
                            "You can now use `/model` to switch to Bedrock models. TUI onboarding saved region `us-east-2`; for a different region, update Bedrock settings in Lyra Agent.".to_string()
