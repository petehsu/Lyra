...
pub struct App {
    ...
    // Total session KV cache usage for turns where the provider reported cache usage.
    ...
    // Memory is an always-on internal capability for this session.
    memory_enabled: bool,
    ...
    // Swarm coordination state for this session
    swarm_enabled: bool,
    ...
    // Last turn-level stream error (used by automatic recovery to choose actions)
...
impl App {
    ...
    pub(super) fn record_completed_stream_cache_usage(&mut self) {
        let has_cache_usage = self.streaming_cache_read_tokens.is_some()
