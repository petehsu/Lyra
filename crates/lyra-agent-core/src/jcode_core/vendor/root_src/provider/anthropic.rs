...
impl AnthropicProvider {
    ...
    async fn get_access_token(&self) -> Result<(String, bool)> {
        ...
        {
            anyhow::bail!(
                "Claude OAuth credentials are missing the required user:inference scope (scopes: {}). Sign in again from Lyra Agent settings to mint a fresh Claude.ai OAuth token, or import/use a fresh Claude Code login.",
        ...
        .await
        {
            ...
            Err(e) => {
                        ...
                        Err(refresh_err) => {
                            let _ = tx
                                .send(Err(anyhow::anyhow!(
                                    "{}\n\nAutomatic Claude OAuth refresh failed: {}\nSign in again from Lyra Agent settings, then retry.",
                                    e,
                                    refresh_err
                                )))
                                .await;
                ...
                if is_oauth && is_oauth_auth_error(&error_str) {
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
                            "{}\n\nClaude OAuth authentication failed. Sign in again from Lyra Agent settings, then retry.",
                            e
                        )))
                        .await;
                } else {
