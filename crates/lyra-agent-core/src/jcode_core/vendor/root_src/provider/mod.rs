...
impl MultiProvider {
        ...
        match provider {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anyhow::bail!(
                        "Claude credentials not available. Sign in from Lyra Agent settings first."
                    );
                }
            ...
            ActiveProvider::OpenAI => {
                let Some(openai) = self.openai_provider() else {
                    anyhow::bail!(
                        "OpenAI credentials not available. Sign in from Lyra Agent settings first."
                    );
                };
            ...
            ActiveProvider::Copilot => {
                let Some(copilot) = self.copilot_provider() else {
                    anyhow::bail!(
                        "GitHub Copilot credentials not available. Sign in from Lyra Agent settings first."
                    );
                };
            ...
            ActiveProvider::Antigravity => {
                let Some(antigravity) = self.antigravity_provider() else {
                    anyhow::bail!(
                        "Antigravity credentials not available. Sign in from Lyra Agent settings first."
                    );
                };
            ...
            ActiveProvider::Gemini => {
                let Some(gemini) = self.gemini_provider() else {
                    anyhow::bail!(
                        "Gemini credentials not available. Sign in from Lyra Agent settings first."
                    );
                };
            ...
            ActiveProvider::Cursor => {
                let Some(cursor) = self.cursor_provider() else {
                    anyhow::bail!(
                        "Cursor credentials not available. Sign in from Lyra Agent settings first."
                    );
                };
            ...
            ActiveProvider::OpenRouter => {
                let Some(openrouter) = self.openrouter_provider() else {
                    anyhow::bail!(
                        "OpenRouter/OpenAI-compatible credentials not available. Set the configured API key or sign in from Lyra Agent settings first."
                    );
                };
    ...
    fn set_model_on_openai_compatible_profile(
        ...
        if !crate::provider_catalog::openai_compatible_profile_is_configured(profile) {
            anyhow::bail!(
                "{} credentials not available. Sign in from Lyra Agent settings first.",
