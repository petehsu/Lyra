...
pub trait Provider: Send + Sync {
    ...
    /// Returns true if Lyra should use its own compaction for this provider.
    fn supports_compaction(&self) -> bool {
        false
    }

    /// Returns true if Lyra should proactively run its own summary-based compaction.
    fn uses_lyra_compaction(&self) -> bool {
...
/// Native tool result to send back to provider bridges that delegate tool execution to Lyra.
...
/// Canonical User-Agent for generic outbound Lyra HTTP requests.
pub const LYRA_AGENT_USER_AGENT: &str = concat!("Lyra/", env!("CARGO_PKG_VERSION"));
    ...
    CLIENT
        .get_or_init(|| {
            reqwest::Client::builder()
                .user_agent(LYRA_AGENT_USER_AGENT)
                ...
                .build()
                .unwrap_or_else(|err| {
                    eprintln!("Lyra: failed to build shared provider HTTP client: {err}");
                    reqwest::Client::builder()
                        .user_agent(LYRA_AGENT_USER_AGENT)
                        .build()
                        .expect("fallback Lyra HTTP client should build")
                })
        })
        .clone()
}
...
mod tests {
    ...
    #[test]
    fn canonical_user_agent_identifies_lyra() {
        assert!(LYRA_AGENT_USER_AGENT.starts_with("Lyra/"));
    }
}
