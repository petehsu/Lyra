...
use lyra_config::ConfigLayerEntry;
use lyra_config::ConfigLayerStack;
use lyra_config::ConfigRequirementsToml;
...
use lyra_utils_absolute_path::test_support::PathExt;
...
fn write_user_skill(lyra_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = lyra_home.path().join("skills").join(dir);
...
fn write_plugin_skill(
    ...
    fs::create_dir_all(plugin_root.join(".lyra-plugin")).unwrap();
    fs::create_dir_all(&skill_dir).unwrap();
    fs::write(
...
fn user_config_layer(lyra_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
    let config_path = AbsolutePathBuf::try_from(lyra_home.path().join(CONFIG_TOML_FILE))
...
fn config_stack(lyra_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(lyra_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(lyra_home, user_config_toml),
            ConfigLayerEntry::new(
...
fn new_with_disabled_bundled_skills_removes_stale_cached_system_skills() {
    let lyra_home = tempfile::tempdir().expect("tempdir");
    let stale_system_skill_dir = lyra_home.path().join("skills/.system/stale-skill");
    ...
    assert!(
        !lyra_home.path().join("skills/.system").exists(),
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&lyra_home, "");
    let skills_manager =
        SkillsManager::new(lyra_home.path().abs(), /*bundled_skills_enabled*/ true);

    write_user_skill(&lyra_home, "a", "skill-a", "from a");
    let outcome1 =
    ...
    write_user_skill(&lyra_home, "b", "skill-b", "from b");
    let outcome2 =
        skills_for_config_with_stack(&skills_manager, &cwd, &config_layer_stack, &[]).await;
...
async fn skills_for_cwd_loads_repo_user_and_extra_roots_with_local_fs() {
    ...
    let cwd = tempfile::tempdir().expect("tempdir");
    let extra_root = tempfile::tempdir().expect("tempdir");
    let repo_dot_lyra = cwd.path().join(".lyra");
    fs::create_dir_all(&repo_dot_lyra).expect("create repo config dir");

    write_user_skill(&lyra_home, "user", "user-skill", "from local user root");
    write_user_skill(&extra_root, "extra", "extra-skill", "from extra root");
    let repo_skill_dir = repo_dot_lyra.join("skills/repo");
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![
            user_config_layer(&lyra_home, ""),
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_lyra_folder: repo_dot_lyra.abs(),
                },
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let lyra_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = lyra_home.path().join("skills/.system/bundled-skill");
    ...
    let config_layer_stack = config_stack(&lyra_home, "[skills.bundled]\nenabled = false\n");
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    let lyra_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let skill_dir = lyra_home.path().join("skills").join("demo");
    ...
    let parent_stack = config_stack(&lyra_home, &disabled_skill_config);
    let child_stack =
        config_stack_with_session_flags(&lyra_home, &disabled_skill_config, &enabled_skill_config);
    let skills_manager =
        SkillsManager::new(lyra_home.path().abs(), /*bundled_skills_enabled*/ true);
