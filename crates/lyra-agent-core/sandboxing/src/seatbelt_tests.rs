...
use lyra_network_proxy::ConfigReloader;
use lyra_network_proxy::ConfigState;
use lyra_network_proxy::NetworkMode;
use lyra_network_proxy::NetworkProxy;
use lyra_network_proxy::NetworkProxyConfig;
use lyra_network_proxy::NetworkProxyConstraints;
use lyra_network_proxy::NetworkProxyState;
use lyra_network_proxy::build_config_state;
use lyra_protocol::permissions::FileSystemAccessMode;
use lyra_protocol::permissions::FileSystemPath;
use lyra_protocol::permissions::FileSystemSandboxEntry;
use lyra_protocol::permissions::FileSystemSandboxPolicy;
use lyra_protocol::permissions::FileSystemSpecialPath;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/lyra-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.lyra".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/lyra-readable");
    let unreadable = absolute_path("/tmp/lyra-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
    ...
    assert!(
        args.iter()
            .any(|arg| arg.starts_with("-DWRITABLE_ROOT_0_EXCLUDED_")
                && arg.ends_with("/workspace/.lyra")),
        "expected proactive .lyra carveout for cwd root: {args:#?}",
    );
}
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/lyra-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/lyra-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/lyra-proxy-use";
    let explicit_socket = "/tmp/lyra-browser-use";
    ...
    let network_proxy = NetworkProxy::builder()
        ...
        .managed_by_lyra(/*managed_by_lyra*/ false)
        .build()
        .await?;
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_lyra_subpaths() {
    // Create a temporary workspace with two writable roots: one containing
    // top-level .git and .lyra directories and one without them.
    let tmp = TempDir::new().expect("tempdir");
    let PopulatedTmp {
        vulnerable_root,
    ...
    // write to .lyra/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected cwd .lyra carveout in policy:\n{policy_text}",
    );
    assert!(
        policy_text.contains("WRITABLE_ROOT_1_EXCLUDED_0")
            && policy_text.contains("WRITABLE_ROOT_1_EXCLUDED_1"),
        "expected explicit writable root .git/.lyra carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_lyra_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .lyra/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_lyra_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .lyra is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_lyra_creation_with_exact_and_descendant_carveouts() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_lyra = repo_root.join(".lyra");
    let config_toml = dot_lyra.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_lyra.to_string_lossy().as_ref(),
    ...
    assert!(
        ...
        "expected exact .lyra carveout in policy:\n{policy_text}"
    );
    assert!(
        policy_text.contains("(require-not (subpath (param \"WRITABLE_ROOT_0_EXCLUDED_1\")))"),
        "expected descendant .lyra carveout in policy:\n{policy_text}"
    );
}
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    // `.lyra` checks are done properly for cwd.
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        read_only_access: Default::default(),
        network_access: false,
    ...
    // - write access to WRITABLE_ROOT_0 (but not its .git or .lyra), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
struct PopulatedTmp {
    /// Path containing a .git and .lyra subfolder.
    ...
    /// ran `git commit` themselves, or modified .lyra/config.toml to
    ...
    dot_lyra_canonical: PathBuf,

    /// Path without .git or .lyra subfolders.
    empty_root: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".lyra")).expect("create .lyra");
    fs::write(
        vulnerable_root.join(".lyra").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .lyra/config.toml");
    ...
    let dot_lyra_canonical = vulnerable_root_canonical.join(".lyra");
