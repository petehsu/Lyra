...
use lyra_protocol::models::NetworkPermissions;
...
use lyra_protocol::protocol::NetworkAccess;
...
fn transform_linux_seccomp_request(
    lyra_linux_sandbox_exe: &std::path::Path,
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let lyra_linux_sandbox_exe = std::path::PathBuf::from("/tmp/lyra-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&lyra_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(lyra_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_linux_seccomp_uses_helper_alias_when_launcher_is_not_helper_path() {
    let lyra_linux_sandbox_exe = std::path::PathBuf::from("/tmp/lyra");
    let exec_request = transform_linux_seccomp_request(&lyra_linux_sandbox_exe);

    assert_eq!(exec_request.arg0, Some("lyra-linux-sandbox".to_string()));
}
