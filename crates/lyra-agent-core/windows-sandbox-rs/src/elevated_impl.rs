...
mod windows_impl {
    ...
    /// Creates the sandbox user's Lyra home directory if it does not already exist.
    fn ensure_lyra_home_exists(p: &Path) -> Result<()> {
    ...
    /// Resolves the command runner path, preferring LYRA_HOME/.sandbox/bin.
    fn find_runner_exe(lyra_home: &Path, log_dir: Option<&Path>) -> PathBuf {
        resolve_helper_for_launch(HelperExecutable::CommandRunner, lyra_home, log_dir)
    }

    /// Generates a unique named-pipe path used to communicate with the runner process.
    fn pipe_name(suffix: &str) -> String {
        let mut rng = SmallRng::from_entropy();
        format!(r"\\.\pipe\lyra-runner-{:x}-{}", rng.r#gen::<u128>(), suffix)
    }
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = lyra_home.join(".sandbox");
        ensure_lyra_home_exists(&sandbox_base)?;
        ...
        let caps = load_or_create_cap_sids(lyra_home)?;
        let (psid_to_use, cap_sids) = match &policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => {
                ...
                (
                    ...
                    vec![
                        caps.workspace,
                        crate::cap::workspace_cap_sid_for_cwd(lyra_home, cwd)?,
                    ],
                )
            }
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        let runner_exe = find_runner_exe(lyra_home, logs_base_dir);
        let runner_cmdline = runner_exe
            .to_str()
            .map(ToString::to_string)
            .unwrap_or_else(|| "lyra-command-runner.exe".to_string());
            ...
            let spawn_request = FramedMessage {
                ...
                message: Message::SpawnRequest {
                    payload: Box::new(SpawnRequest {
                        ...
                        lyra_home: sandbox_base.clone(),
                        real_lyra_home: lyra_home.to_path_buf(),
                        cap_sids,
                        timeout_ms,
                        tty: false,
                        stdin_open: false,
