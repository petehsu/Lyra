...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "lyra-command-runner.exe",
        }
    }
...
pub(crate) fn helper_bin_dir(lyra_home: &Path) -> PathBuf {
    sandbox_bin_dir(lyra_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, lyra_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(
    ...
    let destination = helper_bin_dir(lyra_home).join(file_name);
    match copy_from_source_if_needed(&source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(lyra_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), lyra_home.display());
    ...
    let destination = helper_bin_dir(lyra_home).join(kind.file_name());
    log_note(
        &format!(
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let lyra_home = Path::new(r"C:\Users\example\.lyra");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.lyra\.sandbox-bin"),
            helper_bin_dir(lyra_home)
        );
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("lyra-command-runner.exe");
        let runner_destination = helper_bin_dir(&lyra_home).join("lyra-command-runner.exe");
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("lyra.exe");
        let helper = resources_dir.join("lyra-command-runner.exe");
        fs::write(&exe, b"lyra").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved =
            source_path_for_exe(&exe, /*file_name*/ "lyra-command-runner.exe").expect("helper path");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("lyra.exe");
        let sibling_helper = release_dir.join("lyra-command-runner.exe");
        let resource_helper = resources_dir.join("lyra-command-runner.exe");
        fs::write(&exe, b"lyra").expect("write exe");
