...
pub const OFFLINE_USERNAME: &str = "LyraSandboxOffline";
pub const ONLINE_USERNAME: &str = "LyraSandboxOnline";
...
pub fn sandbox_dir(lyra_home: &Path) -> PathBuf {
    lyra_home.join(".sandbox")
}

pub fn sandbox_bin_dir(lyra_home: &Path) -> PathBuf {
    lyra_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(lyra_home: &Path) -> PathBuf {
    lyra_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(lyra_home: &Path) -> PathBuf {
    sandbox_dir(lyra_home).join("setup_marker.json")
}

pub fn sandbox_users_path(lyra_home: &Path) -> PathBuf {
    sandbox_secrets_dir(lyra_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, lyra_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        lyra_home: request.lyra_home.to_path_buf(),
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.lyra_home.to_path_buf());
    log_note(
        &format!(
        ...
        Some(&sandbox_dir(request.lyra_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
fn gather_helper_read_roots(lyra_home: &Path) -> Vec<PathBuf> {
    ...
    let helper_dir = helper_bin_dir(lyra_home);
...
fn gather_legacy_full_read_roots(
    ...
    let mut roots = gather_helper_read_roots(lyra_home);
    roots.extend(
...
pub(crate) fn gather_read_roots(
    ...
    if policy.has_full_disk_read_access() {
        gather_legacy_full_read_roots(command_cwd, policy, lyra_home)
    } else {
        gather_restricted_read_roots(command_cwd, policy, lyra_home)
    }
}
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "LYRA_NETWORK_ALLOW_LOCAL_BINDING";
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let candidate = dir.join("lyra-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }

        // Standalone installs keep Windows helper binaries under
        // `lyra-resources/` next to `lyra.exe`, so elevation needs to probe
        // that sibling folder before falling back to PATH.
        let resource_candidate = dir
            .join("lyra-resources")
            .join("lyra-windows-sandbox-setup.exe");
        if resource_candidate.exists() {
            return resource_candidate;
        }
    }
    PathBuf::from("lyra-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(lyra_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(lyra_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(lyra_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(request.lyra_home);
    ...
    run_setup_exe(&payload, needs_elevation, request.lyra_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, request.lyra_home);
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.lyra_home);
        ...
        gather_read_roots(request.command_cwd, request.policy, request.lyra_home)
    };
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, lyra_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to LYRA_HOME or anything under LYRA_HOME/.sandbox,
    // LYRA_HOME/.sandbox-bin, or LYRA_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let lyra_home_key = canonical_path_key(lyra_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(lyra_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(lyra_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(lyra_home));
...
mod tests {
    ...
    use crate::helper_materialization::helper_bin_dir;
    use crate::policy::SandboxPolicy;
    use lyra_protocol::protocol::ReadOnlyAccess;
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "LYRA_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_lyra_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let lyra_home = user_profile.join("LyraHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&lyra_home).expect("create Lyra home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &lyra_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let lyra_home = tmp.path().join("lyra-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &lyra_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&lyra_home)).expect("canonical helper dir");
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_lyra).expect("canonical extra .lyra"),
                explicit_deny,
            ]
            .into_iter()
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &lyra_home);
