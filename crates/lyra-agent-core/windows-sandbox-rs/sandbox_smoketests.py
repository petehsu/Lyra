...
# Run a suite of smoke tests against the Windows sandbox via the Lyra CLI
...
def _resolve_lyra_cmd() -> List[str]:
    """Resolve the Lyra CLI to invoke `lyra sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Lyra.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "lyra.exe",
        ws_root / "target" / "release" / "lyra.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "lyra.exe",
            cargo_base / "release" / "lyra.exe",
        ])
    ...
    if shutil.which("lyra"):
        return ["lyra"]

    raise FileNotFoundError(
        "Lyra CLI not found. Build it first, e.g.\n"
        "  cargo build -p lyra-app-server --release\n"
        "or for debug:\n"
        "  cargo build -p lyra-app-server\n"
    )

LYRA_CMD = _resolve_lyra_cmd()
print(LYRA_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    argv = [*LYRA_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"LYRA_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\lyra_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.lyra artifacts) denied
    lyra_home = Path(os.environ["USERPROFILE"]) / ".lyra"
    cap_sid_target = lyra_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .lyra\\policy.json"], WS_ROOT)
    add("WS: .lyra cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .lyra policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > lyra_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://lyra-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
