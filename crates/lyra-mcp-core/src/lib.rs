...
fn write_mcp_message(writer: &mut impl Write, payload: &serde_json::Value) -> Result<()> {
    let body = serde_json::to_vec(payload)
        .map_err(|error| to_error(format!("failed to encode MCP payload: {error}")))?;
    writer
        .write_all(format!("Content-Length: {}\r\n\r\n", body.len()).as_bytes())
        .and_then(|_| writer.write_all(&body))
        .and_then(|_| writer.flush())
        .map_err(|error| to_error(format!("failed to write MCP frame: {error}")))
}

fn read_mcp_message(reader: &mut impl Read) -> Result<serde_json::Value> {
    let mut header = Vec::new();
    let mut byte = [0_u8; 1];
    while !header.ends_with(b"\r\n\r\n") {
        let size = reader
            .read(&mut byte)
            .map_err(|error| to_error(format!("failed to read MCP header: {error}")))?;
        if size == 0 {
            return Err(to_error(
                "MCP stream closed before a complete response frame was received",
            ));
        }
        header.push(byte[0]);
        if header.len() > 16_384 {
            return Err(to_error("MCP frame header exceeded maximum size"));
        }
    }

    let header_text = String::from_utf8(header)
        .map_err(|error| to_error(format!("MCP header was not valid utf8: {error}")))?;
    let content_length = header_text
        ...
        .ok_or_else(|| to_error("MCP frame missing Content-Length header"))?;

    let mut body = vec![0_u8; content_length];
    reader
        .read_exact(&mut body)
        .map_err(|error| to_error(format!("failed to read MCP frame body: {error}")))?;
    serde_json::from_slice(&body)
        .map_err(|error| to_error(format!("failed to decode MCP json payload: {error}")))
}
...
fn read_mcp_response(
    ...
    loop {
        let payload = read_mcp_message(reader)?;
        let Some(message) = payload.as_object().cloned() else {
            continue;
        };
        let message_id = message.get("id").and_then(|value| value.as_i64());
        let jsonrpc = message.get("jsonrpc").and_then(|value| value.as_str());
        if message_id != Some(expected_id) || jsonrpc != Some("2.0") {
            continue;
        }
        return Ok(message);
    }
...
fn call_mcp_tool_blocking(request: CallMcpToolRequest) -> Result<CallMcpToolResult> {
    ...
    let mut stdin = child
        .stdin
        .take()
        .ok_or_else(|| to_error("failed to acquire MCP stdin"))?;
    let mut stdout = child
        .stdout
        .take()
        .ok_or_else(|| to_error("failed to acquire MCP stdout"))?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| to_error("failed to acquire MCP stderr"))?;
    ...
    let timeout_thread = thread::spawn(move || match timeout_cancel_rx.recv_timeout(timeout) {
        ...
        Err(mpsc::RecvTimeoutError::Timeout) => {
            ...
            if let Ok(mut guard) = timeout_child.lock() {
                let _ = guard.kill();
            }
        }
    });
...
#[cfg_attr(feature = "node-api", napi(js_name = "readMcpScopeDocumentJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "writeMcpScopeDocumentJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "readMcpSecretStoreJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "writeMcpSecretStoreJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "sanitizeMcpEnvironmentJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "deleteMcpSecretRefsJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "mergeMcpEffectiveConfigJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "validateMcpServerJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "writeMcpManagedManifestJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "readMcpRuntimeStatusesJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "stopMcpRuntimeJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "restartMcpRuntimeJson"))]
...
#[cfg_attr(feature = "node-api", napi(js_name = "shutdownMcpRuntime"))]
...
mod tests {
    ...
    fn trims_setup_values() {
        ...
        assert_eq!(normalized.get("rootPath"), Some(&"/tmp/demo".to_string()));
        assert_eq!(
    ...
    fn normalizes_server_key() {
        ...
        assert!(key.chars().all(|character| character.is_ascii_lowercase()
