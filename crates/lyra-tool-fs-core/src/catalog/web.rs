...
pub(super) fn manifests() -> Vec<ToolManifest> {
    vec![
        super::s(
            ...
            "search",
            "Web search",
            "Search the web — GitHub, docs, community discussions, news, error solutions, API references. SearXNG aggregates engines first; if it fails or returns nothing, Lyra tries keyless Exa, Parallel, Firecrawl, and Keenable, then a short public fallback. Do not fire extra web_search calls for the same query.",
            Some("web_search"),
        ),
        super::s(
            "/tools/web/fetch",
