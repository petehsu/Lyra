...
fn crawl_domain(
    ...
    let same_domain_links = filter_same_domain_links(document.links.clone(), registrable_domain);
    let sitemap_links = discover_sitemap_pages(client, &document.sitemap_urls, registrable_domain);

    let subdomain_candidates = build_subdomain_candidates(
        registrable_domain,
        target,
        &same_domain_links,
        &sitemap_links,
