...
use crate::protocol::LyraErrorInfo;
...
use lyra_utils_string::truncate_middle_chars;
use lyra_utils_string::truncate_middle_with_token_budget;
...
pub type Result<T> = std::result::Result<T, LyraErr>;
...
pub enum LyraErr {
    ...
    #[error(
        "Lyra ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    QuotaExceeded,
    #[error("This provider entitlement is not available for the current credentials.")]
    ...
    #[error("lyra-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for LyraErr {
...
impl LyraErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            | LyraErr::EnvVar(_)
            | LyraErr::Fatal(_)
            | LyraErr::EntitlementUnavailable
            | LyraErr::QuotaExceeded
            | LyraErr::InvalidImageRequest()
            | LyraErr::InvalidRequest(_)
            | LyraErr::RefreshTokenFailed(_)
            | LyraErr::UnsupportedOperation(_)
            | LyraErr::Sandbox(_)
            | LyraErr::LandlockSandboxExecutableNotProvided
            | LyraErr::RetryLimit(_)
            | LyraErr::ContextWindowExceeded
            | LyraErr::ThreadNotFound(_)
            | LyraErr::AgentLimitReached { .. }
            | LyraErr::Spawn
            | LyraErr::SessionConfiguredNotFirstEvent
            | LyraErr::UsageLimitReached(_)
            | LyraErr::ServerOverloaded => false,
            LyraErr::Stream(..)
            | LyraErr::Timeout
            | LyraErr::UnexpectedStatus(_)
            | LyraErr::ResponseStreamFailed(_)
            | LyraErr::ConnectionFailed(_)
            | LyraErr::InternalServerError
            | LyraErr::InternalAgentDied
            | LyraErr::Io(_)
            | LyraErr::Json(_)
            | LyraErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            LyraErr::LandlockRuleset(_) | LyraErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<LyraErr>()` checks continue to compile
    ...
    pub fn to_lyra_protocol_error(&self) -> LyraErrorInfo {
        match self {
            LyraErr::ContextWindowExceeded => LyraErrorInfo::ContextWindowExceeded,
            LyraErr::UsageLimitReached(_)
            | LyraErr::QuotaExceeded
            | LyraErr::EntitlementUnavailable => LyraErrorInfo::UsageLimitExceeded,
            LyraErr::ServerOverloaded => LyraErrorInfo::ServerOverloaded,
            LyraErr::RetryLimit(_) => LyraErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            LyraErr::ConnectionFailed(_) => LyraErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            LyraErr::ResponseStreamFailed(_) => LyraErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            LyraErr::RefreshTokenFailed(_) => LyraErrorInfo::Unauthorized,
            LyraErr::SessionConfiguredNotFirstEvent
            | LyraErr::InternalServerError
            | LyraErr::InternalAgentDied => LyraErrorInfo::InternalServerError,
            LyraErr::UnsupportedOperation(_)
            | LyraErr::ThreadNotFound(_)
            | LyraErr::AgentLimitReached { .. } => LyraErrorInfo::BadRequest,
            LyraErr::Sandbox(_) => LyraErrorInfo::SandboxError,
            _ => LyraErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            lyra_error_info: Some(self.to_lyra_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            LyraErr::RetryLimit(err) => Some(err.status),
            LyraErr::UnexpectedStatus(err) => Some(err.status),
            LyraErr::ConnectionFailed(err) => err.source.status(),
            LyraErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
#[deprecated(note = "use LyraErr")]
pub type CodexErr = LyraErr;
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("lyra")
        {
            return write!(
                f,
        ...
        let message = format!("You've hit your usage limit.{}", retry_suffix(self.resets_at.as_ref()));
...
pub fn get_error_message_ui(e: &LyraErr) -> String {
    let message = match e {
        LyraErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        LyraErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
