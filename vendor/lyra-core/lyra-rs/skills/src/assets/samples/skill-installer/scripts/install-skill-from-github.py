#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $LYRA_HOME/skills."""
...
def _lyra_home() -> str:
    return os.environ.get("LYRA_HOME", os.path.expanduser("~/.lyra"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "lyra")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "lyra-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_lyra_home(), "skills")
