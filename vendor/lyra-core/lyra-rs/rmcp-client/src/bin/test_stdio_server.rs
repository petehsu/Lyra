...
const MEMO_URI: &str = "memo://lyra/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "lyra/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Lyra TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Lyra TUI):
    /// - Build this binary: `cargo build -p lyra-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `lyra mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Lyra TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://lyra/{slug}".to_string(),
            name: "lyra-memo".to_string(),
            title: Some("Lyra Memo".to_string()),
            description: Some(
                "Template for memo://lyra/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Lyra TUI handling of MCP image outputs.
///
