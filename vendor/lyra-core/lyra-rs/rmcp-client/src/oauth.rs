...
//!
//! If the keyring is not available or fails, we fall back to LYRA_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Lyra MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_lyra_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    use lyra_keyring_store::tests::MockKeyringStore;
    ...
    impl TempLyraHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create LYRA_HOME temp dir");
            unsafe {
                std::env::set_var("LYRA_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
