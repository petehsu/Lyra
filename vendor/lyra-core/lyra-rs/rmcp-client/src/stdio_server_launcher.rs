...
use anyhow::Result;
use anyhow::anyhow;
use lyra_config::types::McpServerEnvVar;
use lyra_config::types::ShellEnvironmentPolicyInherit;
use lyra_exec_server::ExecBackend;
use lyra_exec_server::ExecEnvPolicy;
use lyra_exec_server::ExecParams;
#[cfg(unix)]
use lyra_utils_pty::process_group::kill_process_group;
#[cfg(unix)]
use lyra_utils_pty::process_group::terminate_process_group;
...
impl ExecutorStdioServerLauncher {
    ...
    fn remote_env_policy(remote_env_vars: &[String]) -> ExecEnvPolicy {
        let include_only = if remote_env_vars.is_empty() {
            ...
            // environment, not copied from Lyra. Start from `All` only so the
...
mod tests {
    use super::*;
    use lyra_config::shell_environment;
