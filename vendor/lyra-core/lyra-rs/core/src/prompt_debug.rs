...
pub async fn build_prompt_input(
    ...
    let output =
        build_prompt_input_from_session(thread.thread.session_handle.session.as_ref(), input).await;
...
mod tests {
    ...
    async fn build_prompt_input_includes_context_and_user_message() {
        ...
        config.lyra_home =
            AbsolutePathBuf::from_absolute_path(lyra_home.path()).expect("Lyra home is absolute");
