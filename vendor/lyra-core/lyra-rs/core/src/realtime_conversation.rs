...
use lyra_api::RealtimeAudioFrame;
use lyra_api::RealtimeEvent;
use lyra_api::RealtimeEventParser;
use lyra_api::RealtimeSessionConfig;
use lyra_api::RealtimeSessionMode;
use lyra_api::RealtimeWebsocketClient;
use lyra_api::RealtimeWebsocketEvents;
use lyra_api::RealtimeWebsocketWriter;
use lyra_api::map_api_error;
use lyra_app_server_protocol::AuthMode;
use lyra_config::config_toml::RealtimeWsMode;
use lyra_config::config_toml::RealtimeWsVersion;
use lyra_login::LyraAuth;
use lyra_login::default_client::default_headers;
...
use lyra_protocol::protocol::RealtimeConversationClosedEvent;
use lyra_protocol::protocol::RealtimeConversationRealtimeEvent;
use lyra_protocol::protocol::RealtimeConversationSdpEvent;
use lyra_protocol::protocol::RealtimeConversationStartedEvent;
use lyra_protocol::protocol::RealtimeHandoffRequested;
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> LyraResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> LyraResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            session_config,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> LyraResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(LyraErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> LyraResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(text)
            .await
            .map_err(|_| LyraErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> LyraResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> LyraResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> LyraResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> LyraResult<PreparedRealtimeConversationStart> {
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> LyraResult<()> {
    let voices = RealtimeVoicesList::builtin();
    ...
    let allowed = allowed
        ...
        .collect::<Vec<_>>()
        .join(", ");
    Err(LyraErr::InvalidRequest(format!(
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), LyraErrorInfo::BadRequest).await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&LyraAuth>, provider: &ModelProviderInfo) -> LyraResult<String> {
    ...
    if let Some(api_key) = auth.and_then(LyraAuth::api_key) {
        return Ok(api_key.to_string());
    }

    Err(LyraErr::InvalidRequest(
...
fn realtime_request_headers(
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            LyraErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
