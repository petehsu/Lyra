...
//!
//! The history is stored at `~/.lyra/history.jsonl` with **one JSON object per
...
use lyra_config::types::HistoryPersistence;
...
/// Filename that stores the message history inside the Lyra runtime home.
...
fn history_filepath(config: &Config) -> AbsolutePathBuf {
    config.lyra_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.lyra/history.jsonl` and ensure the parent directory exists.
