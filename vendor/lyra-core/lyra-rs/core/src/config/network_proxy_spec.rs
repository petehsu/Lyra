...
use lyra_network_proxy::BlockedRequestObserver;
use lyra_network_proxy::ConfigReloader;
use lyra_network_proxy::ConfigState;
use lyra_network_proxy::NetworkDecision;
use lyra_network_proxy::NetworkPolicyDecider;
...
use lyra_network_proxy::NetworkProxyHandle;
use lyra_network_proxy::NetworkProxyState;
use lyra_network_proxy::build_config_state;
use lyra_network_proxy::host_and_port_from_network_addr;
...
impl NetworkProxySpec {
    ...
    fn apply_requirements(
        ...
        let managed_allowed_domains = if hard_deny_allowlist_misses {
            Some(
                requirements
                    .domains
                    .as_ref()
                    .and_then(lyra_config::NetworkDomainPermissionsToml::allowed_domains)
                    .unwrap_or_default(),
            )
        } else {
            requirements
        ...
        let managed_denied_domains = requirements
            .domains
            .as_ref()
            .and_then(lyra_config::NetworkDomainPermissionsToml::denied_domains);
        ...
        if requirements.unix_sockets.is_some() {
            let allow_unix_sockets = requirements
                .unix_sockets
                .as_ref()
                .map(lyra_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
                .unwrap_or_default();
            config
                .network
