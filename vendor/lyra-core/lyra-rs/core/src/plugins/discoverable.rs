...
use lyra_tools::DiscoverablePluginInfo;

const TOOL_SUGGEST_DISCOVERABLE_PLUGIN_ALLOWLIST: &[&str] = &["computer-use@openai-bundled"];

const TOOL_SUGGEST_DISCOVERABLE_MARKETPLACE_ALLOWLIST: &[&str] = &[OPENAI_BUNDLED_MARKETPLACE_NAME];
...
pub(crate) async fn list_tool_suggest_discoverable_plugins(
    ...
    let plugins_manager = PluginsManager::new(config.lyra_home.to_path_buf());
