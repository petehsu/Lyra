...
use lyra_thread_store::ListThreadsParams;
use lyra_thread_store::LocalThreadStore;
use lyra_thread_store::ThreadSortKey;
...
pub async fn maybe_migrate_personality(
    ...
    let marker_path = lyra_home.join(PERSONALITY_MIGRATION_FILENAME);
    ...
    if !has_recorded_sessions(lyra_home, model_provider_id.as_str()).await? {
        create_marker(&marker_path).await?;
        return Ok(PersonalityMigrationStatus::SkippedNoSessions);
    }
...
async fn has_recorded_sessions(lyra_home: &Path, default_provider: &str) -> io::Result<bool> {
    let store = LocalThreadStore::new(lyra_rollout::RolloutConfig {
        lyra_home: lyra_home.to_path_buf(),
        sqlite_home: lyra_home.to_path_buf(),
        cwd: lyra_home.to_path_buf(),
...
async fn has_threads(store: &LocalThreadStore, archived: bool) -> io::Result<bool> {
    store
        .list_threads(ListThreadsParams {
            ...
            sort_direction: lyra_thread_store::SortDirection::Desc,
