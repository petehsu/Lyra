...
use lyra_config::RequirementsExecPolicy;
...
use lyra_protocol::protocol::GranularApprovalConfig;
...
fn config_stack_for_dot_lyra_folder(dot_lyra_folder: &Path) -> ConfigLayerStack {
    let dot_lyra_folder =
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        lyra_home.join(lyra_config::CONFIG_TOML_FILE),
...
async fn test_config() -> (TempDir, Config) {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .lyra_home(home.path().to_path_buf())
        .build()
        .await
...
async fn returns_empty_policy_when_no_policy_files_exist() {
    ...
    let config_stack = config_stack_for_dot_lyra_folder(temp_dir.path());
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        lyra_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        /*justification*/ None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(lyra_config::Sourced::new(
            lyra_config::RequirementsExecPolicy::new(requirements_exec_policy),
            lyra_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
    let dot_lyra_folder = AbsolutePathBuf::from_absolute_path(temp_dir.path())?;
    let layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project { dot_lyra_folder },
        TomlValue::Table(Default::default()),
    );
...
async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
    ...
    let project_dot_lyra_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
    let layers = vec![ConfigLayerEntry::new_disabled(
        ConfigLayerSource::Project {
            dot_lyra_folder: project_dot_lyra_folder,
        },
        TomlValue::Table(Default::default()),
        "marked untrusted",
    )];
...
async fn request_rule_falls_back_when_prefix_rule_does_not_approve_all_commands() {
    let command = vec![
        "bash".to_string(),
        "-lc".to_string(),
        "cargo install cargo-insta && rm -rf /tmp/lyra".to_string(),
    ];
    let manager = ExecPolicyManager::default();
...
async fn append_execpolicy_amendment_updates_policy_and_file() {
    let lyra_home = tempdir().expect("create temp dir");
    let prefix = vec!["echo".to_string(), "hello".to_string()];
    let manager = ExecPolicyManager::default();

    manager
        .append_amendment_and_update(lyra_home.path(), &ExecPolicyAmendment::from(prefix))
        .await
    ...
    let contents = fs::read_to_string(default_policy_path(lyra_home.path()))
...
async fn append_execpolicy_amendment_rejects_empty_prefix() {
    ...
    let result = manager
        .append_amendment_and_update(lyra_home.path(), &ExecPolicyAmendment::from(vec![]))
        .await;
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let lyra_home = temp.path().join("home_execpolicy_nested");
    let project_root = temp.path().join("project_execpolicy_nested");
    let nested = project_root.join("nested");
    let root_rules = project_root.join(".lyra").join(RULES_DIR_NAME);
    let nested_rules = nested.join(".lyra").join(RULES_DIR_NAME);

    fs::create_dir_all(&lyra_home)?;
    ...
    write_project_trust_config(&lyra_home, &[(&nested, TrustLevel::Trusted)]).await?;
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let nested = project_root.join("nested");
    let rules_dir = project_root.join(".lyra").join(RULES_DIR_NAME);
    ...
    for (name, trust_entries, expected_decision) in cases {
        let lyra_home = temp.path().join(format!("home_execpolicy_{name}"));
        fs::create_dir_all(&lyra_home)?;
        write_project_trust_config(&lyra_home, &trust_entries).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_entries, expect_warning) in cases {
        let lyra_home = temp.path().join(format!("home_execpolicy_warning_{name}"));
        fs::create_dir_all(&lyra_home)?;
        write_project_trust_config(&lyra_home, &trust_entries).await?;
