...
use lyra_execpolicy::AmendError;
use lyra_execpolicy::Decision;
use lyra_execpolicy::Error as ExecPolicyRuleError;
use lyra_execpolicy::Evaluation;
use lyra_execpolicy::MatchOptions;
use lyra_execpolicy::NetworkRuleProtocol;
use lyra_execpolicy::Policy;
use lyra_execpolicy::PolicyParser;
use lyra_execpolicy::RuleMatch;
use lyra_execpolicy::blocking_append_allow_prefix_rule;
use lyra_execpolicy::blocking_append_network_rule;
...
use lyra_shell_command::is_dangerous_command::command_might_be_dangerous;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(lyra_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(lyra_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &lyra_execpolicy::Error) -> String {
...
fn default_policy_path(lyra_home: &Path) -> PathBuf {
    lyra_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
/// - Note: we only use this amendment when the command fails to run in sandbox and lyra prompts the user to run outside the sandbox
