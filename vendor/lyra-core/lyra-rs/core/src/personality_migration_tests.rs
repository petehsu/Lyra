...
use lyra_rollout::SESSIONS_SUBDIR;
...
async fn read_config_toml(lyra_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(lyra_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(lyra_home: &Path) -> io::Result<()> {
...
async fn write_archived_session_with_user_event(lyra_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = lyra_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}
...
async fn write_rollout_with_user_event(dir: &Path, thread_id: ThreadId) -> io::Result<()> {
    ...
    let session_meta = SessionMetaLine {
        meta: SessionMeta {
            ...
            client_name: "test_originator".to_string(),
