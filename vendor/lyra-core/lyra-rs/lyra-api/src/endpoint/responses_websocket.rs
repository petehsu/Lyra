...
use lyra_client::TransportError;
...
const X_LYRA_TURN_STATE_HEADER: &str = "x-lyra-turn-state";
...
async fn connect_websocket(
    ...
    // If a Lyra-specific CA bundle is configured, build an explicit rustls connector so this
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        let payload = json!({
            ...
            "headers": {
                "x-lyra-primary-used-percent": "100.0",
                "x-lyra-primary-window-minutes": 15
            }
        })
        .to_string();
        ...
        assert_eq!(
            headers
                .get("x-lyra-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-lyra-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
