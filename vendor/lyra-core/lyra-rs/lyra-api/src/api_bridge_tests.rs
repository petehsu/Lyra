...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, LyraErr::ServerOverloaded));
}
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("lyra_other"),
    );
    headers.insert(
        "x-lyra-other-limit-name",
        http::HeaderValue::from_static("lyra_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let LyraErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected LyraErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let err = map_api_error(ApiError::Transport(TransportError::Http {
        ...
        url: Some("https://api.example.test/backend-api/models".to_string()),
        headers: Some(headers),
        body: Some(r#"{"detail":"Unauthorized"}"#.to_string()),
    }));

    let LyraErr::UnexpectedStatus(err) = err else {
        panic!("expected LyraErr::UnexpectedStatus, got {err:?}");
    };
