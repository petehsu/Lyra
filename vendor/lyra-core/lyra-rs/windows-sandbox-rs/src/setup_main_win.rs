...
use lyra_windows_sandbox::LOG_FILE_NAME;
use lyra_windows_sandbox::SETUP_VERSION;
use lyra_windows_sandbox::SetupErrorCode;
use lyra_windows_sandbox::SetupErrorReport;
use lyra_windows_sandbox::SetupFailure;
use lyra_windows_sandbox::add_deny_write_ace;
use lyra_windows_sandbox::canonicalize_path;
use lyra_windows_sandbox::convert_string_sid_to_sid;
use lyra_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use lyra_windows_sandbox::ensure_allow_write_aces;
use lyra_windows_sandbox::extract_setup_failure;
use lyra_windows_sandbox::hide_newly_created_users;
use lyra_windows_sandbox::is_command_cwd_root;
use lyra_windows_sandbox::load_or_create_cap_sids;
use lyra_windows_sandbox::log_note;
use lyra_windows_sandbox::path_mask_allows;
use lyra_windows_sandbox::sandbox_bin_dir;
use lyra_windows_sandbox::sandbox_dir;
use lyra_windows_sandbox::sandbox_secrets_dir;
use lyra_windows_sandbox::string_from_sid_bytes;
use lyra_windows_sandbox::to_wide;
use lyra_windows_sandbox::workspace_cap_sid_for_cwd;
use lyra_windows_sandbox::write_setup_error_report;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(lyra_home) = std::env::var("LYRA_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&lyra_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.lyra_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.lyra_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.lyra_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    let workspace_sid_str = workspace_cap_sid_for_cwd(&payload.lyra_home, &payload.command_cwd)?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.lyra`, and `.agents`.
        //
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.lyra_home),
        &payload.real_user,
        &sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.lyra_home).display()
            ),
        ))
    })?;
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.lyra_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.lyra_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.lyra_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.lyra_home).join("sandbox_users.json");
