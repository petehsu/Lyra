        ...
        let add_writable_root =
            |root: PathBuf,
             ...
             add_deny: &mut dyn FnMut(PathBuf)| {
                ...
                for protected_subdir in [".git", ".lyra", ".agents"] {
...
mod tests {
    ...
    #[test]
    fn denies_lyra_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let lyra_dir = command_cwd.join(".lyra");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&lyra_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&lyra_dir).unwrap(),
