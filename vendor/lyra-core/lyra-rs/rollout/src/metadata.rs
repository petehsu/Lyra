...
use lyra_state::BackfillState;
use lyra_state::BackfillStats;
use lyra_state::BackfillStatus;
use lyra_state::DB_ERROR_METRIC;
use lyra_state::DB_METRIC_BACKFILL;
use lyra_state::DB_METRIC_BACKFILL_DURATION_MS;
use lyra_state::ExtractionOutcome;
use lyra_state::ThreadMetadataBuilder;
use lyra_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions(
    runtime: &lyra_state::StateRuntime,
    config: &impl RolloutConfigView,
) {
    let metric_client = lyra_otel::global();
    ...
    let sessions_root = config.lyra_home().join(SESSIONS_SUBDIR);
    let archived_root = config.lyra_home().join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.lyra_home(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(lyra_home: &Path, path: &Path) -> String {
    path.strip_prefix(lyra_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
