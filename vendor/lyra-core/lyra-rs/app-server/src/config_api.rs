...
use lyra_app_server_protocol::ConfigReadResponse;
use lyra_app_server_protocol::ConfigRequirements;
...
use lyra_app_server_protocol::NetworkUnixSocketPermission;
use lyra_app_server_protocol::SandboxMode;
use lyra_core::ThreadManager;
use lyra_core::config::Config;
use lyra_core::config::ConfigService;
use lyra_core::config::ConfigServiceError;
use lyra_core::config_loader::CloudRequirementsLoader;
use lyra_core::config_loader::ConfigRequirementsToml;
use lyra_core::config_loader::LoaderOverrides;
use lyra_core::config_loader::ResidencyRequirement as CoreResidencyRequirement;
use lyra_core::config_loader::SandboxModeRequirement as CoreSandboxModeRequirement;
...
impl ConfigApi {
    ...
    pub(crate) async fn load_latest_config(
        ...
        let mut config = lyra_core::config::ConfigBuilder::default()
            .lyra_home(self.lyra_home.clone())
    ...
    async fn emit_plugin_toggle_events(
        ...
        for (plugin_id, enabled) in pending_changes {
            ...
            let metadata =
                installed_plugin_telemetry_metadata(self.lyra_home.as_path(), &plugin_id).await;
            if enabled {
...
pub(crate) fn protected_feature_keys(
    config_layer_stack: &lyra_core::config_loader::ConfigLayerStack,
) -> BTreeSet<String> {
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(lyra_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(lyra_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
...
) -> lyra_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => lyra_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: lyra_core::config_loader::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(lyra_core::config_loader::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(lyra_core::config_loader::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(lyra_core::config_loader::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: lyra_core::config_loader::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        lyra_core::config_loader::NetworkDomainPermissionToml::Allow => {
            NetworkDomainPermission::Allow
        }
        lyra_core::config_loader::NetworkDomainPermissionToml::Deny => {
...
fn map_network_unix_socket_permission_to_api(
    permission: lyra_core::config_loader::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        lyra_core::config_loader::NetworkUnixSocketPermissionToml::Allow => {
            NetworkUnixSocketPermission::Allow
        }
        lyra_core::config_loader::NetworkUnixSocketPermissionToml::None => {
...
mod tests {
    use super::*;
    use lyra_analytics::AnalyticsEventsClient;
    use lyra_core::config_loader::NetworkDomainPermissionToml as CoreNetworkDomainPermissionToml;
    use lyra_core::config_loader::NetworkDomainPermissionsToml as CoreNetworkDomainPermissionsToml;
    use lyra_core::config_loader::NetworkRequirementsToml as CoreNetworkRequirementsToml;
    use lyra_core::config_loader::NetworkUnixSocketPermissionToml as CoreNetworkUnixSocketPermissionToml;
    use lyra_core::config_loader::NetworkUnixSocketPermissionsToml as CoreNetworkUnixSocketPermissionsToml;
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        ...
        assert_eq!(
            ...
            Some(vec![
                lyra_app_server_protocol::AskForApproval::Never,
                lyra_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(lyra_app_server_protocol::ResidencyRequirement::Us),
        );
        assert_eq!(
            mapped.network,
            Some(NetworkRequirements {
                ...
                domains: Some(std::collections::BTreeMap::from([
                    ("api.example.test".to_string(), NetworkDomainPermission::Allow,),
    ...
    async fn apply_runtime_feature_enablement_keeps_cli_overrides_above_config_and_runtime() {
        ...
        std::fs::write(
            lyra_home.path().join("config.toml"),
            "[features]\napps = false\n",
        )
        .expect("write config");
        ...
        let mut config = lyra_core::config::ConfigBuilder::default()
            ...
            .fallback_cwd(Some(lyra_home.path().to_path_buf()))
            .cli_overrides(vec![(
            ...
            .cloud_requirements(CloudRequirementsLoader::new(async {
                Ok(Some(ConfigRequirementsToml {
                    feature_requirements: Some(
                        lyra_core::config_loader::FeatureRequirementsToml {
    ...
    async fn batch_write_reloads_user_config_when_requested() {
        let lyra_home = TempDir::new().expect("create temp dir");
        let user_config_path = lyra_home.path().join("config.toml");
        ...
        let analytics_config = Arc::new(
            lyra_core::config::ConfigBuilder::default()
                .build()
                .await
                .expect("load analytics config"),
        );
        let auth_manager = AuthManager::from_auth_for_testing(LyraAuth::from_api_key("test"));
        ...
        let response = config_api
            .batch_write(ConfigBatchWriteParams {
                edits: vec![lyra_app_server_protocol::ConfigEdit {
                    key_path: "model".to_string(),
                    value: json!("gpt-5"),
                    merge_strategy: lyra_app_server_protocol::MergeStrategy::Replace,
                }],
        ...
        assert_eq!(
            response,
            ConfigWriteResponse {
                status: lyra_app_server_protocol::WriteStatus::Ok,
                version: response.version.clone(),
                file_path: lyra_utils_absolute_path::AbsolutePathBuf::try_from(
