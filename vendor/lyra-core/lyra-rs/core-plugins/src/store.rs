...
use lyra_utils_plugins::find_plugin_manifest_path;
...
impl PluginStore {
    pub fn new(lyra_home: PathBuf) -> Self {
        Self::try_new(lyra_home)
            .unwrap_or_else(|err| panic!("plugin cache root should be absolute: {err}"))
    }

    pub fn try_new(lyra_home: PathBuf) -> Result<Self, PluginStoreError> {
        let root = AbsolutePathBuf::from_absolute_path_checked(lyra_home.join(PLUGINS_CACHE_DIR))
