...
use crate::lyra_tool_config::LyraToolCallParam;
use crate::lyra_tool_config::LyraToolCallReplyParam;
use crate::lyra_tool_config::create_tool_for_lyra_tool_call_param;
use crate::lyra_tool_config::create_tool_for_lyra_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_lyra_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_lyra_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation {
            name: "lyra-mcp-server".to_string(),
            title: Some("Lyra".to_string()),
        ...
        // Preserve Lyra's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_lyra_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_lyra_tool_call_param(),
                create_tool_for_lyra_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "lyra" => self.handle_tool_call_lyra(id, arguments).await,
            "lyra-reply" => {
                self.handle_tool_call_lyra_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_lyra(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<LyraToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![rmcp::model::Content::text(format!(
                                "Failed to load Lyra configuration from overrides: {e}"
                            ))],
                            structured_content: None,
                            is_error: Some(true),
                            meta: None,
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for Lyra tool: {e}"
                        ))],
                        structured_content: None,
                        is_error: Some(true),
                        meta: None,
            ...
            None => {
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for lyra tool-call; the `prompt` field is required.",
                    )],
                    structured_content: None,
                    is_error: Some(true),
                    meta: None,
        ...
        let running_requests_id_to_lyra_uuid = self.running_requests_id_to_lyra_uuid.clone();

        // Spawn an async task to handle the Lyra session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Lyra session and stream events back to the client.
            crate::lyra_tool_runner::run_lyra_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_lyra_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_lyra_session_reply(
        &self,
        ...
        // parse arguments
        let lyra_tool_call_reply_param: LyraToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<LyraToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Lyra tool call reply parameters: {e}");
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for Lyra tool: {e}"
                        ))],
            ...
            None => {
                tracing::error!(
                    "Missing arguments for lyra-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for lyra-reply tool-call; the `thread_id` and `prompt` fields are required.",
        ...
        let thread_id = match lyra_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let thread_handle = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::lyra_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = lyra_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_lyra_uuid = running_requests_id_to_lyra_uuid.clone();

            async move {
                crate::lyra_tool_runner::run_lyra_tool_session_reply(
                    thread_id,
                    thread_handle,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_lyra_uuid.lock().await;
        ...
        // Obtain the Lyra thread from the server.
        let lyra_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Lyra.
        if let Err(e) = lyra_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: lyra_protocol::protocol::Op::Interrupt,
                trace: None,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to Lyra: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_lyra_uuid
            .lock()
            .await
            .remove(&request_id);
    }
