//! Schema-heavy configuration TOML types used by Lyra.
...
use lyra_app_server_protocol::Tools;
use lyra_app_server_protocol::UserSavedConfig;
...
/// Base config deserialized from `~/.lyra/config.toml`.
...
pub struct ConfigToml {
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          https://github.com/openai/codex/blob/main/lyra-rs/rmcp-client/src/oauth.rs#L2
    /// file: Use a file in the active Lyra runtime home directory.
    ...
    /// Defaults to `$LYRA_SQLITE_HOME` when set. Otherwise uses the active Lyra runtime home.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Lyra writes log files, for example `lyra-tui.log`.
    /// Defaults to the active Lyra runtime home's `log/` directory.
    ...
    /// directories for `.lyra` folders. Defaults to [".git"] when unset.
    #[serde(default)]
