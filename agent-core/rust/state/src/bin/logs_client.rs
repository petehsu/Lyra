...
use chrono::DateTime;
use clap::Parser;
use lyra_state::LogQuery;
use lyra_state::LogRow;
use lyra_state::StateRuntime;
use dirs::home_dir;
use owo_colors::OwoColorize;

#[derive(Debug, Parser)]
#[command(name = "lyra-state-logs")]
#[command(about = "Tail Lyra logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to LYRA_HOME. Defaults to $LYRA_HOME or ~/.lyra.
    #[arg(long, env = "LYRA_HOME")]
    lyra_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --lyra-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let lyra_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(lyra_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let lyra_home = args.lyra_home.clone().unwrap_or_else(default_lyra_home);
    Ok(lyra_state::logs_db_path(lyra_home.as_path()))
}

fn default_lyra_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".lyra");
    }
    PathBuf::from(".lyra")
}
