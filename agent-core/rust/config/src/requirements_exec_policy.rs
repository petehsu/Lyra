...
use lyra_execpolicy::RuleRef;
use lyra_execpolicy::rule::PatternToken;
use lyra_execpolicy::rule::PrefixPattern;
use lyra_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Lyra merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `lyra-execpolicy`.
