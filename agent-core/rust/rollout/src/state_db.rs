...
pub use lyra_state::LogEntry;
use lyra_state::ThreadMetadataBuilder;
use lyra_utils_path::normalize_for_path_comparison;
...
pub type StateDbHandle = Arc<lyra_state::StateRuntime>;
...
pub async fn init(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match lyra_state::StateRuntime::init(
    ...
    let backfill_state = match runtime.get_backfill_state().await {
        ...
        Err(err) => {
            warn!(
                ...
                config.lyra_home.display()
            );
            return None;
        }
    };
    if backfill_state.status != lyra_state::BackfillStatus::Complete {
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = lyra_state::state_db_path(config.sqlite_home());
...
pub async fn open_if_present(lyra_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = lyra_state::state_db_path(lyra_home);
    if !tokio::fs::try_exists(&db_path).await.unwrap_or(false) {
        return None;
    }
    let runtime =
        lyra_state::StateRuntime::init(lyra_home.to_path_buf(), default_provider.to_string())
            .await
            .ok()?;
    require_backfill_complete(runtime, lyra_home).await
}
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == lyra_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<lyra_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&lyra_state::StateRuntime>,
    lyra_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.lyra_home() != lyra_home {
        warn!(
            "state db lyra_home mismatch: expected {}, got {}",
            ctx.lyra_home().display(),
            lyra_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => lyra_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => lyra_state::SortKey::UpdatedAt,
            },
...
pub async fn list_threads_db(
    ...
    match ctx
        .list_threads(
            page_size,
            lyra_state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => lyra_state::SortDirection::Asc,
                    SortDirection::Desc => lyra_state::SortDirection::Desc,
                },
                search_term,
            },
        )
