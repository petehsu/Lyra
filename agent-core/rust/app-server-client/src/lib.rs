...
//!
//! This crate wraps [`lyra_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](lyra_app_server::in_process::InProcessClientHandle),
...
pub use lyra_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use lyra_app_server::in_process::InProcessServerEvent;
use lyra_app_server::in_process::InProcessStartArgs;
use lyra_app_server::in_process::LogDbLayer;
...
use lyra_app_server_protocol::InitializeCapabilities;
use lyra_app_server_protocol::InitializeParams;
use lyra_app_server_protocol::JSONRPCErrorError;
use lyra_app_server_protocol::RequestId;
use lyra_app_server_protocol::Result as JsonRpcResult;
...
use lyra_core::config::ConfigBuilder;
use lyra_core::config_loader::CloudRequirementsLoader;
use lyra_core::config_loader::LoaderOverrides;
pub use lyra_exec_server::EnvironmentManager;
pub use lyra_exec_server::ExecServerRuntimePaths;
...
/// module exists so clients can remove a direct `lyra-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub use lyra_core::Cursor;
    pub use lyra_core::DEFAULT_AGENTS_MD_FILENAME;
    pub use lyra_core::INTERACTIVE_SESSION_SOURCES;
    pub use lyra_core::LOCAL_AGENTS_MD_FILENAME;
    pub use lyra_core::LYRA_DESKTOP_SESSION_SOURCE;
    pub use lyra_core::McpManager;
    pub use lyra_core::PLUGIN_TEXT_MENTION_SIGIL;
    pub use lyra_core::RolloutRecorder;
    pub use lyra_core::TOOL_MENTION_SIGIL;
    pub use lyra_core::ThreadItem;
    pub use lyra_core::ThreadSortKey;
    pub use lyra_core::ThreadsPage;
    pub use lyra_core::append_message_history_entry;
    pub use lyra_core::check_execpolicy_for_warnings;
    pub use lyra_core::find_thread_meta_by_name_str;
    pub use lyra_core::find_thread_name_by_id;
    pub use lyra_core::find_thread_names_by_ids;
    pub use lyra_core::format_exec_policy_error_with_source;
    pub use lyra_core::grant_read_root_non_elevated;
    pub use lyra_core::lookup_message_history_entry;
    pub use lyra_core::message_history_metadata;
    pub use lyra_core::path_utils;
    pub use lyra_core::read_session_meta_line;
    pub use lyra_core::web_search_detail;

    pub mod config {
        pub use lyra_core::config::*;

        pub mod edit {
            pub use lyra_core::config::edit::*;
        }
    }

    pub mod config_loader {
        pub use lyra_core::config_loader::*;
    }
    ...
    pub mod review_format {
        pub use lyra_core::review_format::*;
    }

    pub mod review_prompts {
        pub use lyra_core::review_prompts::*;
    }

    pub mod skills {
        pub use lyra_core::skills::*;
    }

    pub mod test_support {
        pub use lyra_core::test_support::*;
    }

    pub mod util {
        pub use lyra_core::util::*;
    }

    pub mod windows_sandbox {
        pub use lyra_core::windows_sandbox::*;
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    /// Whether auth loading should honor the `LYRA_API_KEY` environment variable.
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_lyra_api_key_env: self.enable_lyra_api_key_env,
            initialize,
...
#[derive(Clone, Debug)]
pub struct EmbeddedClientDefaults {
    pub session_source: SessionSource,
    pub client_name: String,
    pub client_version: String,
    pub experimental_api: bool,
...
impl EmbeddedClientDefaults {
    pub fn lyra(client_version: impl Into<String>) -> Self {
        Self {
            session_source: SessionSource::Custom(
                lyra_core::LYRA_DESKTOP_SESSION_SOURCE.to_string(),
            ),
...
/// - [`lyra_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    pub async fn start_with_embedded_defaults(defaults: EmbeddedClientDefaults) -> IoResult<Self> {
        let config = match ConfigBuilder::default().build().await {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides(Vec::new()).await.map_err(|error| {
                IoError::new(
                    ErrorKind::InvalidData,
                    format!("failed to load default embedded config: {error}"),
                )
            })?,
        };
        ...
        Self::start(InProcessClientStartArgs {
            ...
            session_source: defaults.session_source,
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            lyra_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
mod tests {
    use super::*;
    use lyra_app_server_protocol::ConfigRequirementsReadResponse;
    use lyra_app_server_protocol::ConfigWarningNotification;
    ...
    use lyra_app_server_protocol::ServerNotification;
    use lyra_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    #[test]
    fn lyra_embedded_defaults_use_lyra_identity() {
        ...
        assert!(matches!(
            ...
            SessionSource::Custom(ref value) if value == "lyra"
        ));
    }
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            lyra_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            lyra_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(lyra_app_server_protocol::ItemCompletedNotification {
            thread_id: "thread".to_string(),
            turn_id: "turn".to_string(),
            item: lyra_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(lyra_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<lyra_app_server_protocol::ThreadReadResponse>(
    ...
    async fn forward_in_process_event_preserves_transcript_notifications_under_backpressure() {
        ...
        assert!(matches!(
            ...
            InProcessServerEvent::ServerNotification(ServerNotification::ItemCompleted(
                ...
                &notification.item,
                lyra_app_server_protocol::ThreadItem::AgentMessage { text, .. } if text == "hello"
            )
        ));
        assert!(matches!(
            &events[4],
            InProcessServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                notification
            )) if notification.turn.status == lyra_app_server_protocol::TurnStatus::Completed
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        let websocket_url = start_test_remote_server(|mut websocket| async move {
            ...
            let JSONRPCMessage::Request(request) = read_websocket_message(&mut websocket).await
                ...
                panic!("expected config/mcpServer/reload request");
            };
            ...
            write_websocket_message(
                ...
                JSONRPCMessage::Response(JSONRPCResponse {
                    id: request.id,
                    result: serde_json::to_value(McpServerRefreshResponse {})
        ...
        let response: McpServerRefreshResponse = client
            ...
            .await
            .expect("typed request should succeed");
        assert_eq!(response, McpServerRefreshResponse {});
    ...
    async fn remote_duplicate_request_id_keeps_original_waiter() {
        ...
        assert_eq!(
            ...
            "config/mcpServer/reload transport error: duplicate remote app-server request id `1`"
        );
        ...
        assert_eq!(first_response, McpServerRefreshResponse {});
    ...
    async fn remote_notifications_arrive_over_websocket() {
            ...
            write_websocket_message(
                ...
                JSONRPCMessage::Notification(
                    serde_json::from_value(
                        serde_json::to_value(ServerNotification::ConfigWarning(
                            ConfigWarningNotification {
                                summary: "config warning".to_string(),
        ...
        assert!(matches!(
            event,
            AppServerEvent::ServerNotification(ServerNotification::ConfigWarning(_))
        ));
        ...
        for event in &remaining_events {
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    == lyra_app_server_protocol::TurnStatus::Completed =>
                {
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                lyra_app_server_protocol::ServerNotification::TurnCompleted(
                    lyra_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: lyra_app_server_protocol::Turn {
                            id: "turn".to_string(),
                            items: Vec::new(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                lyra_app_server_protocol::ServerNotification::AgentMessageDelta(
                    lyra_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                lyra_app_server_protocol::ServerNotification::ItemCompleted(
                    lyra_app_server_protocol::ItemCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item: lyra_app_server_protocol::ThreadItem::AgentMessage {
                            id: "item".to_string(),
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                lyra_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    lyra_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
