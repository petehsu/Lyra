...
use lyra_config::ConfigLayerStackOrdering;
use lyra_config::default_project_root_markers;
use lyra_config::merge_toml_values;
use lyra_config::project_root_markers_from_config;
...
use lyra_utils_plugins::plugin_namespace_for_skill_path;
use dirs::home_dir;
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // User skills location (`$LYRA_HOME/skills`).
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                ...
                // Embedded system skills are cached under `$LYRA_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/lyra/` on Unix, so treat
                // `/etc/lyra/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
...
async fn discover_skills_under_root(
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by Lyra itself.
