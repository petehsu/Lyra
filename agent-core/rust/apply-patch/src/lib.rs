...
/// Special argv[1] flag used when the Lyra executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `lyra-apply-patch` (to avoid forcing
/// `lyra-arg0` to depend on `lyra-core`), it remains part of the Lyra core
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const CODEX_CORE_APPLY_PATCH_ARG1: &str = "--lyra-run-as-apply-patch";
