...
mod tests {
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd agent-core/rust && rg --files"),
            vec![ParsedCommand::ListFiles {
                cmd: "rg --files".to_string(),
                path: None,
            }],
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        let inner = r#"cd /Users/pakrym/code/lyra && rg -n "lyra_api" agent-core/rust -S | head -n 50"#;
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n lyra_api agent-core/rust -S".to_string(),
                query: Some("lyra_api".to_string()),
                path: Some("agent-core/rust".to_string()),
            }],
        );
    }
