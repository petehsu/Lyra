...
use lyra_login::AuthEnvTelemetry;
use lyra_login::AuthManager;
use lyra_login::LyraAuth;
use lyra_login::collect_auth_env_telemetry;
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "lyra_otel.log_only",
            tracing::Level::INFO,
            event.name = "lyra.api_request",
            ...
            auth.env_lyra_api_key_present = self.auth_env.lyra_api_key_env_present,
            auth.env_lyra_api_key_enabled = self.auth_env.lyra_api_key_env_enabled,
...
impl ModelsManager {
    ...
    ///
    /// Uses `lyra_home` to store cached model metadata and initializes with bundled catalog
    ...
    pub fn new_with_provider(
        ...
        let cache_path = lyra_home.join(MODEL_CACHE_FILE);
    ...
    async fn refresh_available_models(&self, refresh_strategy: RefreshStrategy) -> CoreResult<()> {
        ...
        if auth_mode.is_none() && !self.provider.info().has_command_auth() {
            if matches!(
                refresh_strategy,
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        let _timer =
            lyra_otel::start_global_timer("lyra.remote_models.fetch_update.duration_ms", &[]);
        let auth_manager = self.provider.auth_manager();
        let lyra_api_key_env_enabled = auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.lyra_api_key_env_enabled());
        let auth = self.provider.auth().await;
        let auth_mode = auth.as_ref().map(LyraAuth::auth_mode);
        let api_provider = self.provider.api_provider().await?;
        let api_auth = self.provider.api_auth().await?;
        let auth_env = collect_auth_env_telemetry(self.provider.info(), lyra_api_key_env_enabled);
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            lyra_otel::start_global_timer("lyra.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::client_version_to_whole();
    ...
    fn build_available_models(&self, mut remote_models: Vec<ModelInfo>) -> Vec<ModelPreset> {
        ...
        presets = ModelPreset::filter_api_supported(presets);
