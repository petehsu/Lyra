...
async fn refresh_available_models_uses_provider_auth_token() {
    ...
    let lyra_home = tempdir().expect("temp dir");
    let auth_manager = AuthManager::from_auth_for_testing(LyraAuth::from_api_key("unused"));
...
#[tokio::test]
async fn refresh_available_models_skips_network_without_cached_credentials() {
    let server = MockServer::start().await;
    ...
    manager
        .refresh_available_models(RefreshStrategy::Online)
        .await
        .expect("refresh should no-op without provider auth");
    let cached_remote = manager.get_remote_models().await;
    assert!(
        !cached_remote
            .iter()
...
fn models_request_telemetry_emits_auth_env_feedback_tags_on_failure() {
    ...
    let telemetry = ModelsRequestTelemetry {
        ...
        auth_env: lyra_login::AuthEnvTelemetry {
            ...
            lyra_api_key_env_present: false,
