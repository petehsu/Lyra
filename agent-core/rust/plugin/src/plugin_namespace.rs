...
pub const PLUGIN_MANIFEST_PATH: &str = ".lyra-plugin/plugin.json";
...
/// plugin manifest (same `name` rules as full manifest loading in lyra-core).
...
mod tests {
    ...
    fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".lyra-plugin")).expect("mkdir manifest");
        fs::write(
