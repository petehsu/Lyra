...
#[cfg(test)]
use lyra_protocol::protocol::NetworkSandboxPolicy;
#[cfg(test)]
...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/lyra-arg0-session/lyra-linux-sandbox".to_string(),
    );
    assert_eq!(
        argv,
        vec![
...
fn rewrites_inner_command_path_when_bwrap_lacks_argv0() {
    ...
    assert!(
        argv.windows(2)
            .any(|window| { window == ["--", "/tmp/lyra-arg0-session/lyra-linux-sandbox"] })
    );
}
...
fn rewrites_bwrap_helper_command_not_nested_user_command_when_current_exe_appears_later() {
    ...
    let mut argv = vec![
        ...
        "--lyra-run-as-apply-patch".to_string(),
        "patch".to_string(),
    ];
...
fn split_only_filesystem_policy_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        lyra_protocol::permissions::FileSystemSandboxEntry {
            path: lyra_protocol::permissions::FileSystemPath::Special {
                value: lyra_protocol::permissions::FileSystemSpecialPath::CurrentWorkingDirectory,
            },
            access: lyra_protocol::permissions::FileSystemAccessMode::Write,
        },
        lyra_protocol::permissions::FileSystemSandboxEntry {
            path: lyra_protocol::permissions::FileSystemPath::Path { path: docs },
            access: lyra_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);
...
fn root_write_read_only_carveout_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        lyra_protocol::permissions::FileSystemSandboxEntry {
            path: lyra_protocol::permissions::FileSystemPath::Special {
                value: lyra_protocol::permissions::FileSystemSpecialPath::Root,
            },
            access: lyra_protocol::permissions::FileSystemAccessMode::Write,
        },
        lyra_protocol::permissions::FileSystemSandboxEntry {
