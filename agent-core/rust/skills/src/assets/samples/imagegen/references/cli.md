...
Set a stable path to the skill CLI (default `LYRA_HOME` is `~/.lyra`):

```
export LYRA_HOME="${LYRA_HOME:-$HOME/.lyra}"
export IMAGE_GEN="$LYRA_HOME/skills/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/lyra-network.md`
