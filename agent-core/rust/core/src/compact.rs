...
use futures::prelude::*;
use lyra_analytics::CodexCompactionEvent;
use lyra_analytics::CompactionImplementation;
use lyra_analytics::CompactionPhase;
use lyra_analytics::CompactionReason;
use lyra_analytics::CompactionStatus;
use lyra_analytics::CompactionStrategy;
use lyra_analytics::CompactionTrigger;
use lyra_analytics::now_unix_seconds;
use lyra_features::Feature;
use lyra_protocol::error::LyraErr;
use lyra_protocol::error::Result as LyraResult;
use lyra_protocol::items::ContextCompactionItem;
...
async fn run_compact_task_inner_impl(
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(LyraErr::Interrupted) => {
                return Err(LyraErr::Interrupted);
            }
            Err(e @ LyraErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
pub(crate) fn compaction_status_from_result<T>(result: &LyraResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(LyraErr::Interrupted | LyraErr::TurnAborted) => CompactionStatus::Interrupted,
