...
use lyra_app_server_protocol::Config as ApiConfig;
use lyra_app_server_protocol::ConfigBatchWriteParams;
use lyra_app_server_protocol::ConfigLayerMetadata;
...
use lyra_app_server_protocol::OverriddenMetadata;
use lyra_app_server_protocol::WriteStatus;
...
impl ConfigService {
    ...
    pub fn new_with_defaults(lyra_home: PathBuf) -> Self {
        Self {
            lyra_home,
    ...
    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(lyra_home: PathBuf) -> Self {
        Self::new(
            lyra_home,
            Vec::new(),
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<lyra_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_thread_agnostic_config()
            .await
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.lyra_home);
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), &self.lyra_home).map_err(
                |err| {
                    ConfigServiceError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .lyra/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_lyra_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_lyra_folder.display(),
        ),
