...
impl ShellSnapshot {
    ...
    fn spawn_snapshot_task(
        ...
        tokio::spawn(
            async move {
                let timer = session_telemetry.start_timer("lyra.shell_snapshot.duration_ms", &[]);
                let snapshot =
                    ShellSnapshot::try_new(&lyra_home, session_id, &session_cwd, &snapshot_shell)
                        .await
                        .map(Arc::new);
                ...
                session_telemetry.counter("lyra.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_new(
        ...
        let temp_path = lyra_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let lyra_home = lyra_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&lyra_home, cleanup_session_id).await {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            lyra_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(
    ...
    let snapshot_dir = lyra_home.join(SNAPSHOT_DIR);
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(lyra_home, session_id).await?;
