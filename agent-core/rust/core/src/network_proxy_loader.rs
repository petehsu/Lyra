...
use lyra_config::permissions_toml::overlay_network_domain_permissions;
...
use lyra_network_proxy::NetworkProxyConstraintError;
...
use lyra_network_proxy::validate_policy_against_constraints;
...
async fn build_config_state_with_mtimes() -> Result<(ConfigState, Vec<LayerMtime>)> {
    let lyra_home = find_lyra_home().context("failed to resolve Lyra runtime home")?;
        ...
        .filter_map(|layer| {
            let path = match &layer.name {
                ...
                ConfigLayerSource::Project { dot_lyra_folder } => {
                    Some(dot_lyra_folder.join(CONFIG_TOML_FILE))
                }
...
fn apply_exec_policy_network_rules(
    ...
    for host in allowed_domains {
        upsert_network_domain(
            config,
            host,
            lyra_network_proxy::NetworkDomainPermission::Allow,
        );
    }
    for host in denied_domains {
        upsert_network_domain(
            config,
            host,
            lyra_network_proxy::NetworkDomainPermission::Deny,
        );
    }
}
...
fn upsert_network_domain(
    config: &mut NetworkProxyConfig,
    host: String,
    permission: lyra_network_proxy::NetworkDomainPermission,
) {
    config
        .network
