...
use anyhow::Context;
use anyhow::Result;
use lyra_windows_sandbox::ErrorPayload;
use lyra_windows_sandbox::ExitPayload;
use lyra_windows_sandbox::FramedMessage;
use lyra_windows_sandbox::Message;
use lyra_windows_sandbox::OutputPayload;
use lyra_windows_sandbox::OutputStream;
use lyra_windows_sandbox::PipeSpawnHandles;
use lyra_windows_sandbox::SandboxPolicy;
use lyra_windows_sandbox::SpawnReady;
use lyra_windows_sandbox::SpawnRequest;
use lyra_windows_sandbox::StderrMode;
use lyra_windows_sandbox::StdinMode;
use lyra_windows_sandbox::allow_null_device;
use lyra_windows_sandbox::convert_string_sid_to_sid;
use lyra_windows_sandbox::create_readonly_token_with_caps_from;
use lyra_windows_sandbox::create_workspace_write_token_with_caps_from;
use lyra_windows_sandbox::decode_bytes;
use lyra_windows_sandbox::encode_bytes;
use lyra_windows_sandbox::get_current_token_for_restriction;
use lyra_windows_sandbox::hide_current_user_profile_dir;
use lyra_windows_sandbox::log_note;
use lyra_windows_sandbox::parse_policy;
use lyra_windows_sandbox::read_frame;
use lyra_windows_sandbox::read_handle_loop;
use lyra_windows_sandbox::spawn_process_with_pipes;
use lyra_windows_sandbox::to_wide;
use lyra_windows_sandbox::write_frame;
use std::ffi::c_void;
use std::fs::File;
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.lyra_home.clone();
    hide_current_user_profile_dir(req.lyra_home.as_path());
    log_note(
        &format!(
            "runner start cwd={} cmd={:?} real_lyra_home={}",
            req.cwd.display(),
            req.command,
            req.real_lyra_home.display()
        ),
        Some(&req.lyra_home),
    );
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, conpty) = lyra_windows_sandbox::spawn_conpty_process_as_user(
            h_token,
            &req.command,
            &effective_cwd,
            &req.env,
