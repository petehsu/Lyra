...
impl MessageProcessor {
    ...
    async fn handle_lyra_config_providers_catalog_read(
        &self,
        request_id: ConnectionRequestId,
    ) {
    ...
    async fn refresh_loaded_threads_after_persona_change(&self) {
        for thread_id in self.thread_manager.list_thread_ids().await {
            ...
            let merged = self.lyra_runtime_api.merge_developer_instructions(
                strip_persona_context_block(thread.developer_instructions_snapshot().await),
            );
...
fn api_dynamic_tool_to_core(tool: lyra_app_server_protocol::DynamicToolSpec) -> CoreDynamicToolSpec {
    CoreDynamicToolSpec {
        name: tool.name,
