use crate::lyra_message_processor::ApiVersion;
use crate::lyra_message_processor::read_rollout_items_from_rollout;
use crate::lyra_message_processor::read_summary_from_rollout;
use crate::lyra_message_processor::summary_to_thread;
...
use lyra_app_server_protocol::AdditionalPermissionProfile as V2AdditionalPermissionProfile;
use lyra_app_server_protocol::AgentMessageDeltaNotification;
use lyra_app_server_protocol::ApplyPatchApprovalParams;
use lyra_app_server_protocol::ApplyPatchApprovalResponse;
use lyra_app_server_protocol::LyraErrorInfo as V2CodexErrorInfo;
use lyra_app_server_protocol::CollabAgentState as V2CollabAgentStatus;
...
use lyra_app_server_protocol::CommandExecutionRequestApprovalResponse;
use lyra_app_server_protocol::CommandExecutionSource;
use lyra_app_server_protocol::CommandExecutionStatus;
use lyra_app_server_protocol::DeprecationNoticeNotification;
use lyra_app_server_protocol::DynamicToolCallOutputContentItem;
use lyra_app_server_protocol::DynamicToolCallParams;
use lyra_app_server_protocol::DynamicToolCallStatus;
use lyra_app_server_protocol::ErrorNotification;
use lyra_app_server_protocol::ExecCommandApprovalParams;
use lyra_app_server_protocol::ExecCommandApprovalResponse;
use lyra_app_server_protocol::ExecPolicyAmendment as V2ExecPolicyAmendment;
use lyra_app_server_protocol::FileChangeApprovalDecision;
use lyra_app_server_protocol::FileChangeOutputDeltaNotification;
use lyra_app_server_protocol::FileChangeRequestApprovalParams;
use lyra_app_server_protocol::FileChangeRequestApprovalResponse;
use lyra_app_server_protocol::FileUpdateChange;
use lyra_app_server_protocol::GrantedPermissionProfile as V2GrantedPermissionProfile;
use lyra_app_server_protocol::HookCompletedNotification;
use lyra_app_server_protocol::HookStartedNotification;
...
use lyra_app_server_protocol::McpServerElicitationRequestResponse;
use lyra_app_server_protocol::McpServerStartupState;
use lyra_app_server_protocol::McpServerStatusUpdatedNotification;
use lyra_app_server_protocol::McpToolCallError;
use lyra_app_server_protocol::McpToolCallResult;
use lyra_app_server_protocol::McpToolCallStatus;
use lyra_app_server_protocol::MemoryFrozenUpdatedNotification;
use lyra_app_server_protocol::MemoryPromptCacheUpdatedNotification;
use lyra_app_server_protocol::MemorySharedUpdatedNotification;
use lyra_app_server_protocol::MemoryTrimmedNotification;
use lyra_app_server_protocol::ModelReroutedNotification;
use lyra_app_server_protocol::NetworkApprovalContext as V2NetworkApprovalContext;
use lyra_app_server_protocol::NetworkPolicyAmendment as V2NetworkPolicyAmendment;
use lyra_app_server_protocol::NetworkPolicyRuleAction as V2NetworkPolicyRuleAction;
use lyra_app_server_protocol::PatchApplyStatus;
use lyra_app_server_protocol::PermissionsRequestApprovalParams;
use lyra_app_server_protocol::PermissionsRequestApprovalResponse;
use lyra_app_server_protocol::PlanDeltaNotification;
use lyra_app_server_protocol::ReasoningSummaryPartAddedNotification;
use lyra_app_server_protocol::ReasoningSummaryTextDeltaNotification;
use lyra_app_server_protocol::ReasoningTextDeltaNotification;
...
use lyra_app_server_protocol::SkillsChangedNotification;
...
use lyra_app_server_protocol::ToolRequestUserInputOption;
...
use lyra_app_server_protocol::TurnPlanStep;
use lyra_app_server_protocol::TurnPlanUpdatedNotification;
...
use lyra_core::persist_thread_item_to_lyra_memory_truth;
use lyra_core::review_format::format_review_findings_block;
use lyra_core::review_prompts;
...
use lyra_protocol::dynamic_tools::DynamicToolResponse as CoreDynamicToolResponse;
use lyra_protocol::items::parse_hook_prompt_message;
use lyra_protocol::plan_tool::UpdatePlanArgs;
use lyra_protocol::protocol::LyraErrorInfo as CoreCodexErrorInfo;
...
use lyra_protocol::protocol::TurnCompleteEvent;
use lyra_protocol::protocol::TurnDiffEvent;
...
use lyra_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
use lyra_protocol::request_user_input::RequestUserInputAnswer as CoreRequestUserInputAnswer;
use lyra_protocol::request_user_input::RequestUserInputResponse as CoreRequestUserInputResponse;
    ...
    match msg {
        ...
        EventMsg::McpStartupUpdate(update) => {
            if let ApiVersion::V2 = api_version {
                let (status, error) = match update.status {
                    lyra_protocol::protocol::McpStartupStatus::Starting => {
                        (McpServerStartupState::Starting, None)
                    }
                    lyra_protocol::protocol::McpStartupStatus::Ready => {
                        (McpServerStartupState::Ready, None)
                    }
                    lyra_protocol::protocol::McpStartupStatus::Failed { error } => {
                        (McpServerStartupState::Failed, Some(error))
                    }
                    lyra_protocol::protocol::McpStartupStatus::Cancelled => {
                    ...
                    Err(err) => {
                        ...
                        if let Err(err) = conversation
                            .submit(Op::ResolveElicitation {
                                ...
                                request_id: request.id,
                                decision: lyra_protocol::approvals::ElicitationAction::Cancel,
                                content: None,
                                meta: None,
                            })
                            .await
        ...
        EventMsg::CollabAgentSpawnEnd(end_event) => {
            ...
            let status = match &end_event.status {
                lyra_protocol::protocol::AgentStatus::Errored(_)
                | lyra_protocol::protocol::AgentStatus::NotFound => V2CollabToolCallStatus::Failed,
        ...
        EventMsg::CollabWaitingEnd(end_event) => {
            let status = if end_event.statuses.values().any(|status| {
                matches!(
                    status,
                    lyra_protocol::protocol::AgentStatus::Errored(_)
                        | lyra_protocol::protocol::AgentStatus::NotFound
                )
            }) {
        ...
        EventMsg::AgentMessageContentDelta(event) => {
            let lyra_protocol::protocol::AgentMessageContentDeltaEvent { item_id, delta, .. } =
                event;
        ...
        EventMsg::Error(ev) => {
            ...
            let lyra_error_info = ev.lyra_error_info.clone();
            ...
            let turn_error = TurnError {
                message: ev.message,
                lyra_error_info: ev.lyra_error_info.map(V2CodexErrorInfo::from),
        ...
        EventMsg::ItemCompleted(item_completed_event) => {
            ...
            outgoing
                .send_server_notification(ServerNotification::ItemCompleted(notification))
                .await;
            match persist_thread_item_to_lyra_memory_truth(
                thread_id.as_str(),
                turn_id.as_str(),
                &item,
            )
            .await
            {
                Ok(outcome) => {
                    if matches!(api_version, ApiVersion::V2) {
                        if outcome.trimmed {
                            outgoing
                                .send_server_notification(ServerNotification::MemoryTrimmed(
                                    MemoryTrimmedNotification {
                                        thread_id: thread_id.clone(),
                                        turn_id: turn_id.clone(),
                                    },
                                ))
                        ...
                        if outcome.shared_updated {
                            outgoing
                                .send_server_notification(ServerNotification::MemorySharedUpdated(
                        ...
                        if outcome.frozen_updated {
                            outgoing
                                .send_server_notification(ServerNotification::MemoryFrozenUpdated(
                ...
                Err(err) => {
                    warn!(
                        "failed to persist item into Lyra memory truth for thread {} turn {}: {err}",
                        conversation_id,
                        event_turn_id
                    );
                }
        ...
        EventMsg::ExecCommandBegin(exec_command_begin_event) => {
            if matches!(api_version, ApiVersion::V2)
                && matches!(
                    ...
                    lyra_protocol::protocol::ExecCommandSource::UnifiedExecInteraction
                )
            {
                        ...
                        match read_rollout_items_from_rollout(rollout_path.as_path()).await {
                            Ok(items) => {
                                ...
                                thread.status = thread_watch_manager
                                    ...
                                    .await;
                                match find_thread_name_by_id(lyra_home, &conversation_id).await {
                                    Ok(name) => {
                                        thread.name = name;
                                    }
                                    Err(err) => {
...
pub(crate) async fn maybe_emit_hook_prompt_item_completed(
    ...
    conversation_id: ThreadId,
    turn_id: &str,
    item: &lyra_protocol::models::ResponseItem,
    outgoing: &ThreadScopedOutgoingMessageSender,
) {
    ...
    let lyra_protocol::models::ResponseItem::Message {
        role, content, id, ..
    } = item
    else {
        return;
    ...
    let notification = ItemCompletedNotification {
        ...
        item: ThreadItem::HookPrompt {
            ...
            fragments: hook_prompt
                .fragments
                .into_iter()
                .map(lyra_app_server_protocol::HookPromptFragment::from)
                .collect(),
        },
    };
    outgoing
...
async fn handle_token_count_event(
    ...
    let TokenCountEvent { info, rate_limits: _ } = token_count_event;
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `lyra_thread`.
    let response =
...
async fn on_mcp_server_elicitation_response(
    server_name: String,
    request_id: lyra_protocol::mcp::RequestId,
    pending_request_id: RequestId,
    receiver: oneshot::Receiver<ClientRequestResult>,
    conversation: Arc<LyraThread>,
    thread_state: Arc<Mutex<ThreadState>>,
...
fn collab_resume_begin_item(
    begin_event: lyra_protocol::protocol::CollabResumeBeginEvent,
) -> ThreadItem {
...
fn collab_resume_end_item(end_event: lyra_protocol::protocol::CollabResumeEndEvent) -> ThreadItem {
    let status = match &end_event.status {
        lyra_protocol::protocol::AgentStatus::Errored(_)
        | lyra_protocol::protocol::AgentStatus::NotFound => V2CollabToolCallStatus::Failed,
        _ => V2CollabToolCallStatus::Completed,
...
mod tests {
    ...
    use lyra_protocol::items::build_hook_prompt_message;
    ...
    fn turn_aborted_event(turn_id: &str) -> TurnAbortedEvent {
        TurnAbortedEvent {
            ...
            reason: lyra_protocol::protocol::TurnAbortReason::Interrupted,
            completed_at: Some(TEST_TURN_COMPLETED_AT),
            duration_ms: Some(TEST_TURN_DURATION_MS),
        }
    }
    ...
    fn collab_resume_end_maps_to_item_completed_resume_agent() {
        let event = CollabResumeEndEvent {
            ...
            receiver_agent_nickname: None,
            receiver_agent_role: None,
            status: lyra_protocol::protocol::AgentStatus::NotFound,
        };
        ...
        let expected = ThreadItem::CollabAgentToolCall {
            ...
            agents_states: [(
                receiver_id,
                V2CollabAgentStatus::from(lyra_protocol::protocol::AgentStatus::NotFound),
            )]
            .into_iter()
            .collect(),
        };
    ...
    async fn test_handle_error_records_message() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                lyra_error_info: Some(V2CodexErrorInfo::InternalServerError),
                additional_details: None,
            },
            &thread_state,
        )
    ...
    async fn test_handle_turn_complete_emits_failed_with_error() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                lyra_error_info: Some(V2CodexErrorInfo::Other),
                additional_details: None,
            },
            &thread_state,
        )
    ...
    async fn test_handle_turn_complete_emits_error_multiple_turns() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                lyra_error_info: Some(V2CodexErrorInfo::BadRequest),
                additional_details: None,
            },
            &thread_state,
        )
            ...
            OutgoingMessage::AppServerNotification(ServerNotification::ItemCompleted(
                ...
                assert_eq!(
                    ...
                    ThreadItem::HookPrompt {
                        ...
                        fragments: vec![
                            lyra_app_server_protocol::HookPromptFragment {
