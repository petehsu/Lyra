...
use lyra_core::config_loader::ConfigLayerStackOrdering;
use lyra_core::config_loader::LoaderOverrides;
use lyra_login::AuthManager;
use lyra_utils_cli::CliConfigOverrides;
...
use lyra_app_server_protocol::TextPosition as AppTextPosition;
use lyra_app_server_protocol::TextRange as AppTextRange;
use lyra_core::ExecPolicyError;
use lyra_core::check_execpolicy_for_warnings;
use lyra_core::config_loader::ConfigLoadError;
use lyra_core::config_loader::TextRange as CoreTextRange;
use lyra_exec_server::EnvironmentManager;
use lyra_exec_server::ExecServerRuntimePaths;
use lyra_protocol::protocol::SessionSource;
use lyra_state::log_db;
use tokio::sync::mpsc;
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_lyra_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_lyra_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport(
    ...
    let environment_manager = Arc::new(EnvironmentManager::from_env_with_runtime_paths(Some(
        ExecServerRuntimePaths::from_optional_paths(
            arg0_paths.lyra_self_exe.clone(),
            arg0_paths.lyra_linux_sandbox_exe.clone(),
        )?,
    )));
    ...
    if let Some(warning) =
        lyra_core::config::system_bwrap_warning(config.permissions.sandbox_policy.get())
    {
        config_warnings.push(ConfigWarningNotification {
            summary: warning,
            details: None,
    ...
    let otel = lyra_core::otel_init::build_provider(
        &config,
        env!("CARGO_PKG_VERSION"),
        Some("lyra-app-server"),
        default_analytics_enabled,
    )
    .map_err(|e| {
        std::io::Error::new(
    ...
    let state_db = lyra_state::StateRuntime::init(
    ...
    let processor_handle = tokio::spawn({
        ...
        let auth_manager =
            AuthManager::shared_from_config(&config, /*enable_lyra_api_key_env*/ false);
